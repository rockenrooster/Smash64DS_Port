...
Estimator results (2026-09-05, corrected accounting): `estimate_fighter_pack.py --ledger --strict` enumerates 793 kind sets but returns **STOP** for eight remaining source-location disagreements; the figures remain provisional. The original 302 disagreements included compiler alignment, section tails, parent-relative names/comments and a stale Ness DObjDLLink comment. The parser now checks all 81 unique closure O2Rs against manifest hashes, protects actual relocation slots/targets, accounts 279 B of proven zero alignment, and resolves relative locations from named source parents; Ness's full typed DL-link record and raw chain establish its location. All 81 member payloads are fully indexed. Remaining: two Sector texel labels, Link's `gap_0x40EC`, three first-split item labels, and two Kirby palette labels still lack sufficient reconciliation. Profile A's worst set remains Captain/Link/Pikachu/Yoshi at **312,921 B**, including resident MainMotion commands, or **137,317 B above** the optimistic ceiling before other growth. Profile B adds complete raw motion members without assumed compression: Captain/Pikachu/Ness/Kirby is worst at **1,929,961 B**. Kirby's 188 motion members total **399,264 B**, separate from 10,924 B of core commands. Legal common/team costumes include ID 4 for Mario/Donkey/Samus/Kirby and 4/5 for Captain. The per-kind unresolved u32 totals remain dominated by live readers, not orphaned data. Exact four-slot VRAM occupancy and compact motion encoding remain unmeasured. The 59 focused host tests pass; no runtime RAM gain or Kirby admission is established.
