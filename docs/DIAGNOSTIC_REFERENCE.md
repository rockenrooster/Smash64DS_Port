...
workflow, use `docs/VERIFYING.md`.
...
The executable registry contains only the retained runtime profiles:
...
Boundary is canonical `battle_playable_realtime` mode `163`. Latest adds the
normal `smash64ds.nds` runtime check. Focused mode-163 checks remain available
for platform semantics, DamageFall recovery, throws, Fireball rendering, camera
containment, crowd timing, CPU/lifecycle, and renderer forensics. Normal-launch
opening and title boundary scripts also remain available directly.

Bounded harness modes `1`-`162`, their verifier wrappers, and the regression
fleet were retired on 2026-07-15 after natural mode 163 superseded them. Marker
definitions below are retained only as diagnostic/history reference; they are
not executable profile membership.
...
These marker groups came from the retired bounded wrapper pair. The retained
mode-163 verifier still uses the direct gcRunAll script internally as its
shared engine; do not invoke it as a legacy harness.
...
These marker groups are historical evidence from the retired bounded live
preview wrappers. Canonical live input now runs in mode 163.
...
These marker groups are historical evidence from the retired bounded draw and
collision wrappers. Current stage/fighter draw coverage is mode 163.
