...
- **THE REALTIME RED IS A GDB-STUB CEILING, NOT A ROM HANG — do not chase the ROM.** HANDOFF's old wording was wrong twice: `scVSBattleStartBattle` **hits**, and the stop is breakpoint **2**, `ndsRendererHardwareArmBattleStaticTextures` (the Wait→GO arm; gdb misreports its file as `nds_renderer.c:15504`). **600 s also times out** (`Invoke-GdbMarkerScript`, floor `Max(-RendererBenchmarkTimeoutSeconds,120)`, `…gcrunall-loop-harness.ps1:1367`). The ROM is fine: booted standalone it reaches a live match — **45 s wall = TIME 00:36, FPS 28.0, DMG 18/0**; at 14 s it is still on `Original boot: PARTIAL`, so boot-to-GO is ~21 s unstubbed and does not finish inside 600 s under the stub. **Ceiling NOT raised: 600 failed, and a number picked without measuring boot-to-GO under the stub is a guess.** Next cycle: measure it via the loop harness directly (`ValidateRange(5,3600)`), then set the floor from the measurement and make `gdb-markers.ps1` report elapsed on success.
...
- **THE P95 EXCESS IS 92.1% SIMULATION — `…/2026-08-14_runtime2-p95-closure/MARGINAL_OWNERS.md`, no build.** On the 80 frames that literally set `WORK-H` P95 (P95 of 1,600 IS the 80th-largest), excess over a 2-VBlank frame is **+520,718**: `SRC` **+479,816**, **all of it inside `GCRA`** (`SRC−GCRA` = **−68**, and −64 on the cadence set — two populations both put it at zero). Inside it `SINT` +178,455 (`SITR` +171,234, `SCPU` +7,222) · **`SHDT` +119,920 at 19.2x presence** · `SPHD` +112,833 · `SPRM` +49,377. **`FTR` 1.03x, `STG` 1.00x, `MISC` 1.15x — the draw side is 4.8%**, so a whole-match `icache_fill` lane is a P50 lever. Nesting: `SRC ⊃ GCRA ⊃ {SINT ⊃ SCPU, SHDT, SPHD/SPHC, SCAT, SPRM}` — never add a parent to its child. **Cut 91,844 at the 80th-largest frame, not 16,000 on average.**
