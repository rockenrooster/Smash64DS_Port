...
Updated: 2026-08-15 — **THE PACK IS RESIDENT AND ITS COUNTER HAS BEEN READ ON A LIVE ROM** (`3963b8b14ea`, `…/2026-08-15_battlepack-resident/BATTLEPACK_RESIDENT.md`). Gate arm, whole match, `gNdsBattlePackHits` **0 → 197** with the total acquisition count **identical at 357** on both arms; static image +896 B, not +288,288; `gNdsTaskmanArenaChosenSize` 0x150000 / `AllocFailCount` 0. **The marker-2 Boundary hang was the arena and is GONE** (27.1 s of a 120 s ceiling, twice). **Flag 1 is still RED on ONE different assert** — the FPS-HUD self-consistency check, `FPS_HUD=289,14,15,16856768` byte-identical twice; flag 0 GREEN. `NDS_R2_BATTLEPACK` stays **default 0** until phase 8. **Next is slice 1 phase 8 (gate) — and it must first explain +12.4% battle VBlanks on the proof arm.** **THE GATE IS RE-BANKED ON THE SETTLED HEAD: raw 1,184,064 / net 1,159,117**
