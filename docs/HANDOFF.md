...
- **NEXT OWNER: HOT CODE FOOTPRINT REDUCTION — fewer bytes of hot code, not better-arranged bytes.** 404,608 B executed non-ITCM text against 8,192 B of cache. `ndsR2AnimValueQ` spends **81.4% of its own cycles fetching itself**, `…MtxMulAffine20p12` 70.4%, `…CommitNativeStageSegment` 57.1%. `.text.hot`+`.text.hot.draw` = 9,844 B is **already 1.2x the cache**, so the curated set cannot be resident as it stands — know that before adding to it.
