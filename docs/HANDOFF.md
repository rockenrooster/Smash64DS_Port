...
1. `P2-1a`…`P2-1f` are GREEN (descriptor seam, scene manager, UI kit, splash/
   title/menu/rules, character select, stage select, and the three audio-pack
   splits 1c-1, 1d-1, 1e-1). The shell now runs the WHOLE loop — boot → splash
   → title → main menu → VS menu → character select → stage select → battle →
   results → character select → … — walked three laps with every per-kind arena
   high-water flat. Take the board's remaining reds: `P2-1f-1` (render FGM 159
   `nSYAudioFGMStageSelect`, the stage select's only unpacked cue, `MSMISS
   ring=1 id0=159`), then `P2-1g` (loop verifier, which becomes the new
   Boundary and carries the owner visual pass for every shell screen).
