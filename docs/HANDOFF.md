...
**Boundary is RED (below); the owner's order is bugs first, then P95.**
...
- **THE REALTIME RED IS A REGRESSION, NOT A CEILING — the old "GDB-STUB CEILING, do not chase the ROM" wording is REFUTED (2026-08-14, `GATE_ARM_OWNERS.md` §1).** **1,800 s times out exactly as 120 s and 600 s did**, so no ceiling may be raised; and the **control kills the stub theory**: on the same slot 2, same stub, same DLDI, same HEAD, the gate tick-HUD ROM booted and finished a LARGER workload (2,039 frames, 1,600 samples, 17 ring stops) in **123 seconds**. Breakpoint 1 `scVSBattleStartBattle` **hits**; breakpoint 2 is at `0x020250a0`, the **first instruction** of `ndsRendererHardwareArmBattleStaticTextures` (`nds_renderer.c:15535` — the "misreports :15504" note was wrong), with exactly one live call site, in `syTaskmanRunTask` at the Wait→GO edge. After bp1 the presented-frame marker never fires again in-window. **Boundary was green in the DEFAULT config on 2026-08-13 22:01** (`…/2026-08-13_c-bore36-bgstretch/boundary.log`; also 17:20 and 18:50), so the bisect window is the **five 2026-08-14 code commits** `697303ed77c · 33d7cc5d3b7 · 54d7d7862e4 · 9b6c9e72a25 · 813207773c1`. `gdb-markers.ps1` now prints elapsed on success; a probe that may be killed must also `set logging enabled on` or gdb's buffered stdout is lost.
...
|---|---|---:|---:|
| `battle_playable_realtime` (`BOTH_CPU 0`) | shipped, verifier now RED | 899,136 | 1,087,616 |
