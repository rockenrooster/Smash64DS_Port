...
**STATE THE ARM WITH THE HEADROOM.** 211,936 was `build-battle-playable-proof-hwtri-harness` (now 208,672), NOT the shipped ROM; published `smash64ds-battle-playable-hwtri` reads `0x02260724` = **213,216 -> 359,776**. **"+146,560 makes the full pack fit by 6,536" is the COMBINED pool only** — `.rodata` full pack still short 292,152, items-off short 193,920; arena pack still 299,968 alone. The 3-file coupling (arena constant, `0x130000` floor, Task 36 guard) is still required.
