...
material color/alpha selection. Two-cycle color output now falls back through
`COMBINED` for `G_CC_*, G_CC_PASS2` style lists, and blend alpha-memory is now
an exact cycle-aware two-bit field instead of a cycle-1-only bit test.
Current captures:
`artifacts\renderer-stage-gcdrawall-hw-combined-color.png` and
