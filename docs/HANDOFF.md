...
- **UNITS: 2 profile cycles = 1 tick.** The dcache census compared cycles against tick floors and overstated everything 2x; the DMA0CNT poll is **8,315 ticks, UNDER the ±8,544 floor**, not "at the bankable bar". Both docs now carry unit headers.
- **RETRACTED: "88.9% of data-load excess is cacheable" was the classifier FAILING OPEN.** `census-dcache-working-set.py` mapped an unresolved base to `cacheable`; fail-closed it reads **cacheable 0.1% (239 cyc/fr), unknown 91.7% (285,853)**. Only 0.1% was ever PROVEN cacheable. The STOP verdict stands and is stronger; the stated basis does not.
- **CODE PLACEMENT CLOSED 2026-08-14 — hot set is 33.6x the I-cache** (`…/2026-08-14_pgo-code-placement/PGO_CODE_PLACEMENT.md`, `scripts/census-icache-placement.py`; nothing built). I-cache 8 KB / 32 B line (only the line size is confirmed in-toolchain, `libnds .../cache.h:74`) / 4-way / **64 sets, period 2048 B**. Executed non-ITCM text **404,608 B**; distinct lines fetched **8,596 = 33.6x cache**; hottest 2,000 lines fill **64/64 sets**, median 32 vs ideal 31.2 — **already within 2.5% of optimal, so there is no conflict pathology to reorder away**. Two hottest functions alone = 1.25x the whole cache. Holds at 4/8/16/32 KB (67.2x/33.6x/16.8x/8.4x). Phases 3+5 deliberately not built.
- **NEXT ARCHITECTURAL OWNER: shrink the hot FOOTPRINT, not its order.** `icache_fill` 1,525,043 ≈ `issue` 1,522,083 — fetching the code costs about what running it does; 404,608 B of executed text against 8 KB of cache is a CODE SIZE problem wearing a cache costume. **First, ADOPT THE v3 STALL ATTRIBUTOR**: shipped `melonDS.exe` emits `profile-v2` with no stall partition (`strings` confirms); v3 is `melonDS-Accurate` branch `r2-stall-attributor` commit `4a1abf61`, never adopted. **Three lanes this week (D-cache layout, call-frame, placement) each ended needing a stall class the shipped profile lacks** — one adoption unblocks all three.
