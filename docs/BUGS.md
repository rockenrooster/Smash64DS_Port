...
-saffron city: transparency around the pokemon garage door hazard thing is missing. REMOVE WHITE HAZE. — **2026-09-09: the gate owner is now a SILENT empty draw. `DIAG_OWNERTRI` slot 3 (owner_specs `gate`, stage_actors 0x08A0) reads 0 triangles while `gNdsRendererStageOwnerRejectCount` also reads 0, so the door draws nothing and records nothing. That is worse than the 09-08 reading of seen=480 reject=480, not better: a surface that cannot be drawn natively must fail loudly. Saffron's native-failure count is 0, so the failure instrument does not see this — do not read `all nine stages clean` as covering it.** — **Door transparency remains open; remove only the source haze panel as authorized, preserving other geometry and native rendering.**
