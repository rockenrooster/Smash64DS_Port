...
-Hyrule: Do tornadoes spawn at random locations like source and have a lifetime for each cycle??? If not then fix. — **Source lifecycle imported; verify randomized map-position selection, active lifetime, capture/release, cleanup, and repeated cycles on the native ROM.**
...
-saffron city: transparency around the pokemon garage door hazard thing is missing. REMOVE WHITE HAZE. — **Door transparency remains open; remove only the source haze panel as authorized, preserving other geometry and native rendering.**