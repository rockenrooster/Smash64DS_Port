...
-Pikachu
    ...
    -Pokeball Spawn Intro not playing VFX. **Ball VISIBLE (owner). RAYS: construct and submit -- req=2 null=0 cand=100, free-min 48,216, latch never fired. Read that as `not starved and not refused`, NOT as `drawn`: the counters are construction and submission, and nothing here proves a pixel. Needs your exact scenario -- which stage, and which mode.**
