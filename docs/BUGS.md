...
-The rematched match is drawn wrong: duplicated fighters over corrupted stage
  ...
  **The second-entry framing is CORRECT. A 2026-07-31 attempt to reframe it as
  progressive was wrong and is withdrawn** — recorded because the mistake is
  cheap to repeat. The harness's `*-entry.png` is captured at the `entered`
  stage, which is the TIE CHECK: `TIME 00:00` (match 1's clock expired, which is
  what produced the 0-0 tie) and `DMG 0%`. **It is match 1's final frame, before
  Sudden Death renders anything** — not the second entry's first frame. Comparing
  it against `*-watch.png` 60s into Sudden Death compares match 1 to match 2 and
  proves nothing about degradation over time. Read the HUD before reading the
  ...
  picture unchanged). Both are genuine eliminations and survive the withdrawal.
