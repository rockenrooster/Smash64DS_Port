...
| Task 12 renderer code placement | EXACT / corrected HUD packet ready; hardware verdict pending | Renderer | Phase A shrinks main renderer text 79,068 → 60,694 bytes while keeping renderer object/final ITCM exactly 24,364/28,088 bytes; ARM/Thumb frame delta is 0/49,152. melonDS regresses loop 1,680,448 → 2,240,640 and draw 1,150,944 → 1,765,152, which is calibration evidence only. Phase B places the measured 1,772-byte raw-run and 5,268-byte fighter-root kernels in a sorted 7,040-byte region, stays 0/49,152, and saves 2,112 draw/active ticks in melonDS. The corrected three-ROM packet is under `builds/task12-hardware-hud-pair/`; all three visibly expose UPD/DRW/ACT/LOOP/SLIP/GIT and passed the focused config/codegen/launch gates | Hardware is the only keep referee. Phase A uses ARM control vs Thumb; Phase B uses Thumb vs hot-text. If blanket Thumb loses on device, pin the hottest non-ITCM functions ARM before abandoning Phase A. Hardware cells remain `PENDING-HW`; never infer them from melonDS |
...
| Focused/checkpoint verification | Boundary PASS / natural attack-hit channel/effect PASS | QA | Published battle candidate `41E457A6...` passes the 06:33 Boundary checkpoint in 137 seconds with normal runtime, mode-163 CPU setup/proc/target 1/33/33, GBI fixtures, two-ROM publication, Task16 1/1/1 at 28,820-byte ITCM, zero renderer rejects, and dated visual analysis. The retained focused route proves exact natural ID-32 channel/effect output and 187,152-byte reserve | Keep this development checkpoint. Full one-minute post-pack reserve, exact-ROM ear check, and the remaining cue audit are the restart packet |
| Cut G capture / final dated capture / manual retest | Automated exactness pass / manual current-ROM retest pending | QA + user | Latest capture is `2026-07-18_canonical_fast_063233-1948850-p57288.png`; its paired frame has 489/49,152 meaningful changes, 100% overlap, and all named-region/detail gates pass. Task 6 C/D, Task 8 G2, reserve repair, Task 9/16 state identity, source-light parity, both emitter splits, raw-corner cut, Down+A, and the common-contact checkpoint remain retained | Manually retest exact ROM `41E457A6...`; automated common-contact A/V closure is focused, not global |
