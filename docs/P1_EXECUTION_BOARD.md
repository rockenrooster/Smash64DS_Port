...
| Gameplay + QA | Playtest review open / Down+A reproduced | shared live tree / disjoint files | Fox Down-Air advances exactly six updates and then stalls at the first ID-190 FGM event with the PC in NitroFS I/O; the tiny-GO source point sample is an isolated KEEP with canonical locks still stale | no runner active |
...
| Down+A CPU-stall report | RED / reproducible Fox stall, root fix open | Gameplay + QA | A verifier-only pre-spawn human-P2 Fox enters status 213 / motion 188 with exact `FTFoxAnim129` / `0x303`, then advances exactly six callback updates in independent 30-second and 120-second runs before stopping. Timeout PCs are in `nitroromReadIter` / `_lseek_r`; source motion tick 7 reaches the first `PlayFGMStoreInfo` for ID 190. The separate WIP common-bank repair restores omitted files 642..661 / `0x282..0x295` and passes its static source-identity checker. Canonical mode 163 remains level-3 CPU Fox | Trace ID 190 through `func_800269C0_275C0` and the shared DS pack/load seam with the audio qualification workflow; fix the root cause, then rerun Fox P2, Mario, and one CPU-on Current gate before marking fixed |
...
| Countdown/3-2-1/GO top presentation | YELLOW / isolated tiny-GO KEEP, canonical lock refresh pending | Renderer + QA | The existing source-backed point sample changes 49 atlas pixels, all inside the exact 12x9 `ShadowGo` rectangle and zero outside. Its isolated runtime remains 10/10/10 draws/queued/emitted with 57,344 texture bytes, 608 palette bytes, and zero hot conversion/upload; the candidate reads `GO`, while the accepted reference does not. The canonical verifier still pins the old crop hash/count | Refresh only the exact crop/count locks, recapture against the canonical ROM, and keep the prepare-once source path |
