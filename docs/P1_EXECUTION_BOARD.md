...
| Renderer implementation | Tasks 8 and 9, emitter splits, source-light repair, capture reset, and raw-corner cut banked | shared live tree / focused lab builds | E/F/G2, unchanged hot libgcc ITCM, measured raw/cross and raw texture-class splits, exact generic/fast light-state parity, scalar-only display-capture reset, and plain dense-ID raw corners are retained | no runner active |
...
| Common visual effects | Focused Fireball pass / broader attack-hit audit reopened | Renderer + Gameplay | Natural Fireball rebound created 3 bounded source effects; 13 hardware submissions emitted 96 triangles with kind mask `0x45`, zero texture/reject faults, and 176,464-byte reserve. Tyler reports that some attack/hit visuals do not play in the current ROM; the Fireball proof does not qualify that broader surface | Trace each affected original attack/hit event to its exact effect asset and natural runtime draw before restoring a pass claim |
...
| Task 9 bit-exact soft-float | Pass / 16.54% owner gain | Renderer + Gameplay | Six unchanged GCC 15.2.0 objects add exactly 1,952 ITCM bytes. The two-update source owner moves 311,744/312,960 → 260,192/261,312 P50/P95; 3,892 full-state rows match exactly. After the later emitter splits, source-light repair, and raw-corner cut, canonical ITCM is 28,052/32,768 and renderer ownership is 23,540 bytes | Keep Phase 1; skip custom Phase 2. Main-RAM coroutine stack finding is report-only |
...
| Required FGM, attack/hit sounds, and Mario/Fox voices | REOPENED / user reports missing playback | Audio + Gameplay | The 107,536-byte AOT pack maps 18 exact source IDs / 16 unique samples plus 11 collision cues, but pack membership and the prior 14-play route do not prove every attack/hit trigger. Tyler reports that some current-ROM attack/hit sounds do not play at all | Next checkpoint: trace original event IDs through the DS mapping/mixer/channel path and qualify each affected natural attack/hit event audibly; retain continuous-pitch IDs 429/435 and fork voice 685 as explicit debt unless they are reproduced |
...
| Focused/checkpoint verification | Latest PASS / focused Down-Air PASS | QA | Published battle candidate `DA8282BB...` passes the 21:24 Latest checkpoint with normal runtime, mode-163 CPU setup/proc/target 1/33/33, GBI fixtures, two-ROM publication, 28,052-byte ITCM placement, and visual analysis. The exact no-build profile passed in 201.2 seconds after the disconnected-session capture fallback was qualified. Focused Fox-P2 and Mario routes both complete Down-Air naturally with source assets and reserve above 200 KiB | Keep this shared-runtime checkpoint; the countdown and Down+A findings are fixed |
| Cut G capture / final dated capture / manual retest | Automated exactness pass / manual current-ROM retest pending | QA + user | Latest capture is `2026-07-17_canonical_fast_212428-0396036-p48468.png`; its paired frame has 1,844/49,152 meaningful changes, 100% overlap, and all named-region/detail gates pass under the disconnected-session `PrintWindow` fallback. Task 6 C/D, Task 8 G2, reserve repair, Task 9 state identity, source-light parity, both emitter splits, raw-corner cut, and Down+A remain retained | Manually retest exact ROM `DA8282BB...`; automated Down+A closure is complete |
