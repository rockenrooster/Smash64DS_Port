...
| Gameplay + QA | Playtest review open / Down+A data abort localized | shared live tree / disjoint files | The freeze-diagnostic target reproduces human-P2 Fox at status 213, motion 188, tic 6 and now captures the real exception context. PC `0x020631f6` is `gmCollisionGetFighterPartsWorldPosition` dereferencing `main_dobj`; the saved caller `0x02088873` is `ftMainProcPhysicsMap` immediately after passing the collider's invalid small `joint` pointer. The earlier `armWaitForIrq` sample was Calico's historical idle-thread context, not the gameplay caller. The shared attack-collider joint assignment/decode seam remains unmapped. The countdown full gate passes | no runner active |
...
| Down+A CPU-stall report | RED / data abort at attack-collider joint transform | Gameplay + QA | A verifier-only pre-spawn human-P2 Fox enters status 213 / motion 188 with exact `FTFoxAnim129` / `0x303`; observer-free snapshots stop at tic 6 / animation frame 7. The exception-handler breakpoint captures flags `2`, context PC `0x020631f6`, and saved caller `0x02088873`: `gmCollisionGetFighterPartsWorldPosition` faults on `ldr r0, [r0, #4]` after `ftMainProcPhysicsMap` loads `attack_coll->joint` and calls it. Repeats supplied invalid small values including `0x1a0`, `0x1ac`, and `0x206`. Scheduler state proves the earlier `0x01ffebe4` WFI PC belongs to Calico's seeded idle thread, not this gameplay fault. Both ID-190 calls return before `soundPlaySample`; canonical mode 163 remains level-3 CPU Fox | Trace every live write to `FTAttackColl.joint` and its motion-event joint decode/layout, compare against BattleShip source, and fix the shared assignment seam rather than guarding the collision transform or special-casing Down-Air. Before closure, pass Fox P2, Mario, and CPU-on Current |
