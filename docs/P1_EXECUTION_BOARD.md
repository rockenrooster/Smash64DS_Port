...
| Gameplay + QA | Playtest review open / Down+A verifier under audit | shared live tree / disjoint files | Source trace maps the six observed Fox Down-Air callbacks to normal animation frames 2..7 through the second ID-190 event. Repeated-breakpoint runs did not observe a successor, but GDB convenience-variable and asynchronous-stop failures leave a target stall unproven. The countdown full gate now passes after refreshing its stale post-light-repair large-GO crop lock | no runner active |
...
| Down+A CPU-stall report | RED / report open, stopped loop unproven | Gameplay + QA | A verifier-only pre-spawn human-P2 Fox enters status 213 / motion 188 with exact `FTFoxAnim129` / `0x303`. Source trace proves callbacks 1..6 are normal animation frames 2..7 and that callback 6 completes the second ID-190 event; Fox can then transition on the same-frame map step, including to status 219 `LandingAirNull`. Both ID-190 calls return before `soundPlaySample`, and the NitroFS PCs were attach locations. Repeated-breakpoint runs did not observe the successor, but GDB convenience-variable encoding warnings and failed asynchronous CLI interrupts make the stopped-target conclusion ambiguous. The common-bank repair restores files 642..661 / `0x282..0x295`; canonical mode 163 remains level-3 CPU Fox | At the first Down-Air entry, disable all observer breakpoints, continue, use GDB/MI `-exec-interrupt`, and sample global frame counters plus Fox status/motion/PC/LR. If the target advanced, repair the verifier; if not, map the first stopped owner. Do not add ID 190 as a stall fix. Before closure, pass Fox P2, Mario, and CPU-on Current |
