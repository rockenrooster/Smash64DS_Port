...
14,613,504 bytes
SHA-256 FE0C8893C37F43934DB4BEEB8169F52BB0AADEB97C5C4BA07B69416A37B743A9
```
...
| Gameplay + QA | Playtest review open / Down+A reproduced | shared live tree / disjoint files | Fox Down-Air still stops after six updates, but the exact trace rules out ID-190 file I/O: both ID-190 calls and both tick-7 attack refreshes return normally, while the NitroFS PCs were attach locations rather than terminal samples. The tiny-GO count/crop locks are refreshed; its full gate now reaches an unrelated large-GO hash mismatch | no runner active |
...
| Down+A CPU-stall report | RED / reproducible Fox stall, audio hypothesis disproven | Gameplay + QA | A verifier-only pre-spawn human-P2 Fox enters status 213 / motion 188 with exact `FTFoxAnim129` / `0x303`, then advances exactly six callback updates in independent 30-second and 120-second runs before stopping. ID 190 is absent from the 18-entry resident FGM pack: `ndsAudioFgmPlayAtPan` increments the unsupported count and returns before `soundPlaySample`; pack file I/O occurs only at the fenced scene load. Exact tracing observed two ID-190 returns, both tick-7 attack refreshes, and entry into the following effect-script `SyncWait`. The reported `nitroromReadIter` / `_lseek_r` PCs were GDB attach locations, not terminal samples. The common-bank repair restores omitted files 642..661 / `0x282..0x295` and passes its static source-identity checker. Canonical mode 163 remains level-3 CPU Fox | Continue after the second ID-190/effect-forward event with the target watchdog or one target-owned breadcrumb while observer breakpoints are disabled. Do not add ID 190 to the pack as a stall fix; its unqualified pitch schedule/custom-FX fidelity is separate debt. After the shared root fix, rerun Fox P2, Mario, and one CPU-on Current gate before marking fixed |
...
| Countdown/3-2-1/GO top presentation | YELLOW / tiny-GO locks refreshed, full gate pending | Renderer + QA | The existing source-backed point sample changes 49 atlas pixels, all inside the exact 12x9 `ShadowGo` rectangle and zero outside. Its isolated runtime confirms the new 70-pixel `ShadowGo` count and crop hash across all five GO frames while retaining 10/10/10 draws/queued/emitted, 57,344 texture bytes, 608 palette bytes, and zero hot conversion/upload. The full verifier now reaches a separate large-GO crop mismatch (`8dbde0ad...` expected, `d968b0cc...` actual) | Keep the exact tiny-GO locks, inspect the independent large-GO delta before changing its lock, then recapture against the canonical ROM |
...
| Focused/checkpoint verification | Boundary PASS / forensic parity PASS | QA | Published candidate `FE0C8893...` passes Boundary with 828 triangles, owner packet/hierarchy checks, GBI fixtures, exact generic/fast profile-2 parity, two-ROM publication, and 28,020-byte ITCM placement. State effect 14 is admitted only for exact `G_MW_LIGHTCOL` commands | Keep the checkpoint; the two new playtest findings remain open and are not hidden by this pass |
| Cut G capture / final dated capture / manual retest | Automated exactness pass / manual current-ROM retest pending | QA + user | Current Boundary capture is `2026-07-17_canonical_fast_125115-2819034-p54940.png`; Task 6 C/D, Task 8 G2, reserve repair, Task 9 state identity, source-light parity, the M2 capture-reset cut, and raw-corner cut remain retained | Recheck exact ROM `FE0C8893...` after the Down+A and tiny-GO packets; neither finding is marked fixed |
