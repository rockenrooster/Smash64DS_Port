...
Updated: 2026-07-18 06:18 Central
...
| Task 16 extended bit-exact soft-float | QUALIFIED / compare, i2f, add/sub KEEP; fmul REVERT; publication enable pending | Gameplay + Toolchain | The three exact candidates coexist in one link and move source-update 215,104/216,512 → 200,000/201,088 P50/P95 (-15,104/-15,424; -7.02% median). Combined ITCM is 28,820/32,768 with 3,948 bytes free and zero fill; all 3,892/3,892 six-field state rows match with zero overflow. The exact-zero fmul wrapper passed 100M host and 1,050,880 literal ARM9 pairs but regressed its representative microbench 5.34% and natural source-update 1.64%, so all fmul code/tools/link changes were removed | Keep global selectors default OFF for lab controls, but enable compare/i2f/addsub intrinsically in the published target as TASK 16's qualified shipping result; then run its canonical Boundary/publication gate. Keep stock 408-byte `__aeabi_fmul`; retain `verify-task16-combined-state-hash-ab.ps1` |
