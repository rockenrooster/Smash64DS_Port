...
14,612,480 bytes
SHA-256 A89F143B447F7511D80AAB12D423BF8B70EDA4844E4396750F6CEAABF6F295DC
```
...
| Renderer implementation | Tasks 8 and 9 complete | shared live tree / focused lab builds | E/F/G2 are banked; unchanged hot libgcc objects are promoted to ITCM with supreme state identity | no runner active |
...
| Task 9 bit-exact soft-float | Pass / 16.54% owner gain | Renderer + Gameplay | Six unchanged GCC 15.2.0 objects add exactly 1,952 ITCM bytes. The two-update source owner moves 311,744/312,960 → 260,192/261,312 P50/P95; 3,892 full-state rows match exactly. Canonical ITCM is 28,128/32,768 and renderer ownership remains 23,640 bytes | Keep Phase 1; skip custom Phase 2. Main-RAM coroutine stack finding is report-only |
...
| Focused/checkpoint verification | Integrated Current + focused + lifecycle PASS | QA | Published candidate `A89F143B...` passes normal runtime and Boundary; Task 9 exact-object, 3,892-row state, DevFast, Current, source countdown, visual-effects, FGM, and CPU-on expiry→Results gates pass; ITCM is 28,128/32,768 | Tyler's exact-ROM visual/acoustic eyeball remains the release gate |
| Cut G capture / final dated capture / manual retest | Automated exactness pass / manual current-ROM retest pending | QA + user | Task 6 C/D, Task 8 G2, reserve repair, and Task 9 state identity preserve deterministic output; current Boundary capture is dated | Tyler still owes the requested 30-second visual/acoustic eyeball of exact ROM `A89F143B...` |
