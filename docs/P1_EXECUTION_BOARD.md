...
| Gameplay + QA | Playtest review open | shared live tree / disjoint files | tiny traffic-box GO is unreadable; Down+A CPU-stall report has a concrete missing Fox animation-asset seam but no runtime classification yet | no runner active |
...
| Down+A CPU-stall report | YELLOW / asset seam fixed, Fox runtime proof open | Gameplay + QA | BattleShip's Fox files 751..771 are now coherently mapped as `FTFoxAnim109..129` / `0x2ef..0x303`; Down-Air is exactly `FTFoxAnim129` / `0x303`, not the previously shifted file. The source-identity checker passes. Mario status 213 / motion 188 completed eight updates across four presents with clean relocation reads. A verifier-only pre-spawn human-P2 Fox receives controller-1 jump input and reaches KneeBend, but has not yet reached Down-Air before timeout; canonical mode 163 remains level-3 CPU Fox | Instrument the natural KneeBend update counter/length in the WIP Down-Air verifier, complete Fox P2 status-213 proof, rerun Mario, then use one CPU-on Current gate before marking fixed |
...
| Countdown/3-2-1/GO top presentation | RETEST / tiny GO unreadable | Renderer + QA | O2R decode and the large RGB555+A1 GO are correct, but the separate 12x9 traffic-box `ShadowGo` remains unreadable in the current 0.8x A3I5 runtime capture. Preparation still proves 31,168 OBJ + 57,344 texture + 608 palette bytes and zero hot conversion/upload | Keep the prepare-once source path; timebox one source-backed DS-legibility experiment on the tiny traffic-box state |
