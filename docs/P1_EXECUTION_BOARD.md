...
3. **Disjoint lane:** Task 20R is closed with no placement; Task 22R is next.
   Retail hardware remains authoritative for direct-VRAM/DMA performance.
...
|---|---|---|---|---|
| Integration/release | Task 20R closed / Task 22R next | live tree | Retained Task-26 segment 0, Task-27 Phase-A fighter certificate, rejected Task-20 DTCM placements, static-residency/audio lifecycle repairs, integrated battle ROM, and current terminal lifecycle | no runner active; Task 22R owns only its retained wallpaper census/evidence surfaces |
...
| Focused/checkpoint verification | Task 20R complete / Task 22R queued | QA | The current profile-0 and profile-1 ELF layouts pass the exact 0/152-byte DTCM owner/boundary gate. The intact stack is statically too large; no scratch or stack placement was made. Boundary rebuilds the exact green battle ROM `757ED786...` and passes fixtures, registry, Task-9/16, renderer, and DTCM placement, then exits 1 in emulator launch transport without a new runtime artifact. Three focused lifecycle attempts likewise failed before ROM execution and supplied no game evidence | Commit and Lean-snapshot the checker/report checkpoint, then reconcile Task 22R's retained census and rejected threshold writer without requesting a retail repeat; the attack/hit audio-visual cue audit remains required before Task 30 |
