...
2. **Selected next M2 lane:** Task 21R is closed; Task 27 is next. Task 21C
   retains only the compact-table/manifest foundation, not its rejected runtime
   consumer and not the completed generated fighter architecture.
...
|---|---|---|---|---|
| Integration/release | Task 21R complete / Task 27 Phase A next | live tree | Retained Task-26 segment 0, Task-21 compact fighter tables and consumed-field manifest, exact-pixel rejected runtime cuts, static-residency/audio lifecycle repairs, integrated battle ROM, and current terminal lifecycle | no runner active; renderer TU remains one-writer for Task 27 |
...
| Focused/checkpoint verification | Task 21R closed / retained ROM remains full-Boundary green | QA | Restored battle ROM `757ED786...` and public ROM `D0632348...` are byte-identical to the 00:25 full-Boundary checkpoint. Task-21 same-slot screenshots are exactly 0/49,152 changed pixels. Two fresh no-build runs independently pass the 589-field manifest, Task-26 subset, GBI/registry/ITCM gates, canonical mode-163 lifecycle smoke, and two-ROM publication; both then encounter the same unattended host-only capture outage because melonDS exposes no top-level window handle | Exclude the failed duplicate capture as transport evidence; the exact prior full-Boundary ROM and synchronized Task-21 screenshots already cover it. Start Task 27 Phase A. The attack/hit audio-visual cue audit remains required before Task 30; no emulator or retail repeat is requested |
