...
3. **Selected next disjoint lane:** Task 20R -> Task 22R, with retail hardware deciding DTCM and
...
|---|---|---|---|---|
| Integration/release | Task 27 closed / Task 20R next | live tree | Retained Task-26 segment 0, Task-27 Phase-A fighter certificate, rejected Mario runtime expansion, static-residency/audio lifecycle repairs, integrated battle ROM, and current terminal lifecycle | no runner active; Task 20R owns only its measured memory/stack surfaces |
...
| Focused/checkpoint verification | Task 27 complete / Task 20R queued | QA | Phase A deterministically certifies 25/14/18/30 Mario events and eight mutation/falsifier checks. The candidate preserves 70/686 and 60/320/306 ownership but regresses draw +2,624/+2,560 P50/P95, so it is removed before Fox or pixel promotion. Focused gates, canonical lifecycle smoke, and publication pass; production restores the exact green Boundary pair `757ED786...` / `D0632348...`. The wrapper's only miss is the duplicate unattended-host capture-window failure | Commit and Lean-snapshot this checkpoint, then start Task 20R without requesting another retail repeat; the attack/hit audio-visual cue audit remains required before Task 30 |
