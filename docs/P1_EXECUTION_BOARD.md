...
| Gameplay + QA | Playtest review open / Down+A root mismatch mapped, fix unstarted | shared live tree / disjoint files | Human-P2 Fox reaches status 213, motion 188, tic 6 and faults when `ftMainProcPhysicsMap` passes the refreshed collider's erased `joint` to `gmCollisionGetFighterPartsWorldPosition`. BattleShip clears only `attack_state`; the port `ftParamClearAttackCollAll` shim also zeroes `joint` and the reusable payload. Fox's source script makes IDs 0/1 on joint 20, clears them, waits one tick, then refreshes them without rebuilding the payload. The live `FTAttackColl` ABI and event decode match BattleShip, so neither is the defect. The earlier WFI sample was Calico's idle-thread context. The countdown full gate passes | no runner active |
...
| Down+A CPU-stall report | RED / source mismatch mapped, unpatched | Gameplay + QA | A verifier-only pre-spawn human-P2 Fox enters status 213 / motion 188 with exact `FTFoxAnim129` / `0x303`; observer-free snapshots stop at tic 6 / animation frame 7. The exception handler captures flags `2`, PC `0x020631f6`, and saved caller `0x02088873`: `gmCollisionGetFighterPartsWorldPosition` faults after `ftMainProcPhysicsMap` loads the refreshed collider's `joint`. ABI/DWARF and linked code agree on the 0xC4 collider stride, joint ID +8, joint +20, and parser assignment from `fp->joints[20]`. Fox's source motion makes IDs 0/1 on joint 20, calls ClearAll, waits one tick, and refreshes both IDs. BattleShip ClearAll changes only `attack_state`; the port shim clears the entire payload, including `joint`, while Refresh intentionally restores only state and hit records. Earlier reported small register values are retired because the exception labels do not preserve the caller registers reliably; the stack and linked instructions establish the erased-payload path. The WFI PC remains Calico's idle-thread context, not this fault | Delete the non-source payload clearing from `ftParamClearAttackCollAll`, retain only state-off plus `is_attack_active = FALSE`, and change the existing damage-common diagnostic to require its seeded damage 7 to survive the clear. Do not guard the transform or special-case Down-Air. Before closure, pass Fox P2, Mario, and CPU-on Current |
