...
| Gameplay + QA | Playtest review open / real Down+A target stall proven | shared live tree / disjoint files | GDB/MI observer-free snapshots remain identical after 4 and 12 seconds: Fox stops after 6 logic / 3 present frames at status 213, motion 188, tic 6, animation frame 7. The target-owned interrupted owner is not mapped yet. The countdown full gate passes after refreshing its stale post-light-repair large-GO crop lock | no runner active |
...
| Down+A CPU-stall report | RED / observer-free target stall proven | Gameplay + QA | A verifier-only pre-spawn human-P2 Fox enters status 213 / motion 188 with exact `FTFoxAnim129` / `0x303`. After the first Down-Air entry, the GDB/MI arm disables all five observer breakpoints and resumes the target. Both 4-second and 12-second interrupts report the same 6 logic / 3 present / 8 input-read deltas, status tic 6, animation frame 7, motion frame 0, and untouched 3600/0 timer. This is a real target freeze, not the prior nine-callback verifier dependency. The melonDS stub's interrupt `$pc=0x01ffeca8` and `$lr=0x4000003f` are malformed, so the stuck owner remains unmapped. Both ID-190 calls return before `soundPlaySample`; the common-bank repair restores files 642..661 / `0x282..0x295`; canonical mode 163 remains level-3 CPU Fox | Reuse the existing target-owned freeze watchdog, stop at `ndsFreezeDiagnosticsStallMarker`, and map its captured interrupted PC/LR plus breadcrumb ring before changing gameplay source. Do not add ID 190 as a stall fix. Before closure, pass Fox P2, Mario, and CPU-on Current |
