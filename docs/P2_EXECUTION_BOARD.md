...
| P2-4 Stage production | **all 8 selectable; four layer packets wired in source** | Dream Land, Yoster, Castle and Jungle pass packet/source checks. New runtime wiring is unbuilt; matrix kind 48 still needs its source MVP behavior. The five remaining stages need DLLinks/head-order support. Independent native actors and runtime/visual acceptance remain open. Detail: `docs/p2/P2-4-stage-production.md`. |
...
| P2-4n1 | Native stage packet, all stages | **PAUSED FOR APP UPDATE — 4/9 layer packets wired** | Next: matrix kind 48 and DLLinks/head ordering (Sector, Hyrule, Zebes, Yamabuki, Inishie). Castle/Jungle exposed and corrected fixed asset-count, material-commit and binding-29 assumptions. Native actors, scene-owned blob and runtime acceptance remain. Resume detail: `docs/p2/P2-4-stage-production.md`. |
