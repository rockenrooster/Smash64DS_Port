...
| P2-4 Stage production | **all 8 VS stages plus the five 1P arenas: fourteen native packets wired in source (unbuilt)** | Every packet passes the checker, emitter `--check` and DLLink tests; 25 bonus boards in flight. Collision loads from the source's 41-row table since 09-05. Seven mechanical actors have no render path (`docs/p2/stages/stage-actor-census-2026-09-05.md`); admission/acceptance open. |
...
| P2-6 1P Game | **~55% — driver, bridge for every ladder stage, tally, bonus stages, menus, intro, challenger, ending, credits (with DS glyphs), boss and the five arenas in source (unbuilt)** | `NDS_P2_1P_GAME ?= 0`, no target sets it. The bridge boots the fight task and admits every venue, Giant DK and Metal Mario; it still refuses the Polygon team (admitted 857bafd9398, owner export out), the bonus boards and Race (25 packets written, reloc rows staged, registration owed) and Master Hand (admission agent out). |
