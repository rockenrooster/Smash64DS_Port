...
| P2-4h1 | A Castle build's scripted lap does not finish | **OPEN — cause not established** | Measured 2026-09-03 on `build-p2-shell-loop-castle`: no abort, `LOOPCFG gkind=00` so Castle is committed, `walkloops=1` so the confirm happened — and then the battle entry never appears. A 240 s run ends sitting after stage select; a 3,000 s run was still executing at 38 min with melonDS at full CPU, so the guest is running, not hung on input. **Two open explanations, neither proven:** the stage draws through the generic renderer with no native packet (P2-4n1) and may simply be too slow to finish a lap, or the scripted stage-select step (`kNdsMenuWalkSss`, `nds_menu_shell_core.c:561`, three fixed inputs written when Dream Land was the only unlockable ground) lands somewhere unintended. Next: read the presented-frame counters at a mid-run stop rather than theorising. Boundary is unaffected — it runs flag-off. |
