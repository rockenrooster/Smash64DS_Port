...
| P2-4h1 | Stages froze, aborted or dropped their fighters | **CLOSED - 7 of 8 stages boot and play** | Three defects of ONE shape: a hardcoded list of stage kinds standing in for "does this stage have valid data". (1) `ndsRelocNormalizeGroundMapAsset` swapped camera-bound word halves for three assets only, so every other stage's left bound sat past its right and `gmCameraSetBoundsPosition`'s `while (TRUE)` clamp oscillated - the Castle freeze. (2) `mpCollisionInitGroundData` had a copied arm per stage and four stages had none, so `gMPCollisionGroundData` stayed NULL and they aborted in `grWallpaperMakeCommon`. (3) Collision was gated on gkind in TWO places, so every stage off the list got no line groups and no floor query - fighters fell through, which no verifier assertion could see and the OWNER heard. All three now test the data. Sweep on one all-stages ROM: Castle, Jungle, Yoster, Sector, Hyrule, Yamabuki, Inishie PASS; Zebes still aborts. |
