...
| P2-5u1 | Item Switch and VS Options screens | **RED -- the last P2-5 slice** | Commit rule landed (`ndsMatchConfigItemTogglesFromRows`); `damage_ratio` added to the descriptor, the one field VS Options needed. 37 surfaces baked and gate-verified (`LOOPKIT` hash unchanged, `mismatch=0`); the cursor is 146x10 after scaling, over the 64 px OBJ cell limit, and is being split into three abutting cells. Screen ids 5 and 6 reserved and the verifier reads e5/e6/x5/x6. What remains is the two screen modules and the router dispatch; the VS OPTIONS row refuses today (`nds_menu_shell_mode_vs.c:612`). |
