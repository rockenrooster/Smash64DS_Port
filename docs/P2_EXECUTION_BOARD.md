...
| P2-6 1P Game | **~5% — bonus stages imported (unbuilt), driver in flight** | `NDS_P2_1P_GAME ?= 0` still, no target sets it. Step 5 TUs landed behind the flag (`battleship_sc1pbonusstage*.c`); their blockers are listed in the commit: GRStruct bonus members, ~60 `ll*` reloc rows, `sc1PManagerCheckUnlockSoundTest`, one stub gating in `title_backend.c`. Step 1 (driver) is an agent task. Order: `docs/p2/P2-6-one-player.md`. |
...
| P2-3c1 | Q12 pose clock shifts landing-lag End ticks ±1 on shipped fighters | **RED — kernel proven, wiring owed** | Differential (`test_pose_clock_differential.py`): Mario/Fox 635 live mismatches / 44 speeds — Fox dair/uair End 40→41, Mario/Luigi dair 35→36, Mario Super Jump Punch landing 26→25, actionable; no Q closes it. Fix kernel `include/nds/nds_f32_exact.h` PROVEN vs the host IEEE adder, 0/1.12G ops. Plan `nds_ft_pose.c:74-130`: 32 `wait_q`/`frame_q`/`speed_q` sites become float bits; held-body catch-up becomes `held` sequential subtracts (the multiply is not exact). Owner ruling owed (P1 hot path). |
