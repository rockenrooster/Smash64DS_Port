...
| P2-4h1 | The shell walk's stage-select step is coupled to which stages are unlocked | **OPEN — blocks every flagged stage's lap** | `kNdsMenuWalkSss` in `src/nds/nds_menu_shell_core.c:561` is `RIGHT, UP, A`, written when the cursor could only open on Dream Land's slot 6. Unlock any other stage and those two moves land elsewhere: a Castle build's lap alternates character select and stage select and never enters the battle (measured 2026-09-03, 240 s and 3,000 s runs both). Boundary is unaffected because it runs flag-off. Fix is a stage-agnostic step — seek the intended ground before confirming — so the lap tests the shell rather than whichever stage owns slot 0. |
