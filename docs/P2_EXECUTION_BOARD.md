...
| P2-3f47 | Roster close: Ness, Jigglypuff and Kirby admitted opt-in | **IN PROGRESS** | Kirby both-CPU smoke RED 2026-09-04, and **not** Jigglypuff's fixup defect: an arena refusal. `ndsSyMallocOverflowHalt` on Fox's 115,440 after Kirby's setup succeeds; `arena=1318912` against a Jigglypuff-only lab's 1,597,440, ELF no larger. Remaining: that 278,528, Jigglypuff's fixup, Ness smoke, CSS capture, Boundary, stress arm with P2-3f46. Detail: `docs/p2/fighters/{ness,jigglypuff,kirby}.md`. |
