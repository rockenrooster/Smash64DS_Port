...
| P2-4s3 | Congo Jungle (Jungle), stage 3 | **GAMEPLAY COMPLETE opt-in, unbooted** | Stage half plus the barrel cannon's fighter half - `ProcUpdate`, `ProcInterrupt`, `ShootFighter`, `ftParamGetGroundHazardKnockback`. Music LANDED (`nSYAudioBGMJungle = 5`): the doubling suspicion was wrong, and the 50.06% loop start is what `render-audio-bgm.py:271-273` produces for a song whose last note runs past one period. Remaining: a boot, native packet. |
| P2-4h1 | Peach's Castle hangs inside its battle | **OPEN - localised to one call, cause still unknown** | Four runs, banked in `artifacts/p2-4/`. The bumper spawns and validates, so the maker, the `DObjDesc` walk and the attribute decode are all fine (and P2-5i1's acceptance is met). `gcDrawAll` never enters and `gcRunAll` never runs again, so it is not the renderer and frame 2 never begins. `gcPlayAnimAll` is entered twice and never returns, and the second caller is `itProcessProcItemMain` -- the ITEM's own per-frame process, not the stage's. **A malformed descriptor table was RULED OUT on the ROM**: `gNdsItSetupDObjCount=2`, `gNdsItSetupDObjOrphanCount=0`, so the tree is well formed and the fail-closed check (kept, it is right regardless) never fires. Note `DOBJ_PARENT_NULL` is `(DObj*)1`, so the ascent at `sys/objanim.c:1456-1469` spins on any parent chain that misses that sentinel; the MObj list `gcAddMObjAll` builds from `p_mobjsubs` is the other walk inside the same function. Next: `probe-trace-symbols.ps1 -StepOnLimit` names the frozen PC instead of guessing. |
