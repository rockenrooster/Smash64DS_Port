...
| P2-3f50 | PurinMain's external pointer fixups fail, so his parts container is never resolved | **FIX LANDED 2026-09-03 (`95c5b8133ef`); RUNTIME CONFIRMATION OWED** | The fixup pass bailed at the `extern_count == 0` guard for asset 233 (PurinMain) because `ndsRelocLoadExternTreeAsset`'s already-registered early-out finalized a resident entry that declared a 41-id extern chain and carried no id table. A scan of all 2,132 corpus files found none that pairs a chain with zero ids and none that references asset 233 at all, so such an entry is unfinalizable by construction; the early-out now takes it only when the entry has ids or declares no chain, and otherwise reloads through the known-table path. `gNdsRelocExternTreeReloadCount` is new and is zero on a healthy boot. **What produced the zero-count registration is still unpinned** -- every file-backed path either yields 41 or records a failure, which is why the failure count was 1 and not 2. Next: build a Jigglypuff lab and run `scripts/probe-battle-progress.ps1`; the abort should be gone and the reload counter should say whether the gate fired. |
| P2-3f51 | A fighter that borrows another fighter's animation files gets dependency rows but no animation-token rows | **OPEN — reported 2026-09-03 by the P2-3f50 investigation, NOT yet verified** | Claim as filed: a `NDS_P2_PURIN=1`-only build cannot resolve 77 borrowed `FTKirbyAnim` tokens, because Jigglypuff's generated manifest carries dependency rows for those files and no token rows, so the forced-load path falls back to heap residue after setup rather than failing closed. If true this is a generator defect, not a Jigglypuff one, and it affects every fighter pair that shares animation files. First action is to verify the claim against `nds_fighter_production.generated.h`, `scripts/fighters/generate_fighter_production_manifest.py` and the loader, and to enumerate which fighter pairs share files at all. A fallback that reads uninitialised memory instead of failing is the more serious half. |
