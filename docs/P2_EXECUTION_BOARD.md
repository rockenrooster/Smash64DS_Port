...
| P2-5 Items | **IN PROGRESS — all 20 common items and 5 of 13 Pokémon in the ROM** | System landed; item switch UI and the remaining 8 Pokémon outstanding. Stress = items ON. |
...
| ID | Slice | Status | Next / evidence |
|---|---|---|---|
| P2-5i1 | Item manager core, and the twenty common items | **ALL 20 IN THE ROM** | The Poke Ball closed the set. Its explosion tables closed a second gap: `ndsItGetAttackEvent` was written for Link's bomb and refused every other kind, so all four containers dereferenced NULL and aborted the ARM9 the first time items were turned on. It now decodes any kind, cached by (file, symbol), validated against an oracle transcribed from `251_ITCommonData.c`. `include/it/item.h` carries all 384 `itvars.h` constants and all 25 item-vars members. `scripts/items/check-item-import-fidelity.py` checks reloc offsets, literals, redefined macros and the macro-glob comment trap; `scripts/check-audio-ordinals.py` checks all 510 audio ordinals against the decomp enum. Measured on Castle, `-ItemRate 3`: 5 spawned, evnull=0, evrej=0, zero faults. Traps: `docs/p2/P2-5-items.md`. |
| P2-5i2 | The 13 Poke Ball Pokemon | **5 OF 13 IN THE ROM** | Kabigon, Tosakinto, Nyars, Dogas and Mew, verbatim from `src/it/itmonster/`. The dispatch table was sized `nITKindGBumper + 1`, below every Poke Ball kind AND below `nITKindMBall`, so none of them could be reached however they were rolled; it now runs to `nITKindMew`. Shared plumbing landed with them: `itGetMonsterAnimNode`, the monster SFX/voice block, the coin/hydro/smog weapon-vars members, `itDisplayCheckItemVisible` published, `G_RM_AA_ZB_TEX_EDGE`, and `include/nds/nds_obj_anim.h` for the animation helpers nine TUs had each redeclared. **Arena, measured rather than extrapolated:** the whole batch cost the taskman arena 2 pages (1,449,984 to 1,441,792 B); spawned items cost only 776 B of peak (38,944 B free floor items off, 38,168 on), so the floor tracks BINARY size. Monster TUs measure ~870 B each, so the remaining 8 are ~7,200 B, about 2 more pages, landing the floor near 30,700 against the 32,768 P2-1 reserve. Land them in two batches of four and measure; if the last batch does not fit, reclaim ~4 KiB rather than lowering the reserve, which is an owner decision. |
