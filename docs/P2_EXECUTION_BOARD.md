...
| P2-4 Stage production | **REOPENED — native admission repaired `ab3a8f083e4`; art/hazard acceptance open** | All eight stages declined with reason 6 (missing maxima header, segment-0 flag); all eight admit, Zebes entry abort fixed. Per-stage acceptance, actors (barrel path, clouds, gate, acid, Arwing) and pipes remain; `docs/BUGS.md` holds the owner's list. |
...
| P2-4n1 | Native stage packet and actors | **40 packets connected; actor arms live 2026-09-07; acceptance open** | Host loader/actor tests pass. Yoster vapor fixed in `ad39124faa4`. The ground-vars union aliased every stage actor onto Dream Land's map GObjs, so no actor arm ever ran; fixed, and barrel, clouds, gate and acid now draw through their arms (probe counters). Lakitu/Bronto and visual acceptance open; barrel visibility in play under A/B (`docs/p2/BUG_NOTES.md`). |
