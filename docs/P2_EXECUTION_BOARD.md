...
| P2-3f47 | Roster close: Ness, Jigglypuff and Kirby admitted opt-in | **IN PROGRESS** | Generated by `admit_fighter.py` + `derive_native_owner_tables.py`: manifest rows, reloc/effect/entry/HUD/CSS seams, native owners (slots 9/10/11), status promotion, three audio banks. The ten-flag `build-roster` ROM builds and reaches gameplay; remaining: both-CPU smokes (proof fighter 11/10/8), CSS capture, Boundary, stress arm on the new argmax with P2-3f46. **Kirby needing every donor flag is correct, not a defect**: `dFTKirbySpecialNStatusList` (`ftcommon/ftcommonspecialn.c:9-38`) is an unconditional `[copy_id]` call with no presence check, so his slice is smoke-tested on the ten-flag ROM. Detail: `docs/p2/fighters/{ness,jigglypuff,kirby}.md`. |
...
| P2-4s3 | Congo Jungle (Jungle), stage 3 | **GAMEPLAY COMPLETE opt-in (`NDS_P2_STAGE_JUNGLE`), unbooted** | `e6be3d6ec8c`+`623f6a1ab89`: stage half plus the barrel cannon's fighter half — `ProcUpdate`, `ProcInterrupt`, `ShootFighter` and `ftParamGetGroundHazardKnockback`, none of which the port had. Offsets from decomp `reloc_data.us.h`, the authoritative symbol table; payload closure checked against the map's own extern list (`0x5c`, `0x6c`, `0x9e`). `ftParamGetGroundHazardKnockback` also unblocks Hyrule's tornado. Remaining: music (`nSYAudioBGMJungle = 5`), a boot, native packet. |
| P2-4h1 | A Castle build's scripted lap does not finish | **OPEN — walk coupling REFUTED; hang or slowness inside the battle** | Measured 2026-09-03 on `build-p2-shell-loop-castle`: no abort, `gkind=00`, and `walkloops=1` at scene entry 7 — so the stage select confirmed and the transition to the battle ran. The walk is not the problem. What never appears is a further scene entry: 600 s and 3,000 s runs both end with the emulator at full CPU and the guest inside the Castle battle. Two explanations left: the stage draws through the generic renderer with no native packet (P2-4n1) and may be too slow to finish a one-minute match, or something in Castle's battle setup loops. Next: let a full-ceiling lap run to completion, or read the presented-frame counter at a stop. Boundary is unaffected — flag-off. |
