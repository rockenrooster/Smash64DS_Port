...
| P2-4 Stage production | **all 8 VS stages plus the five 1P arenas: fourteen native packets wired in source (unbuilt)** | Every packet passes `check_nds_native_stage.py` (runtime_wiring=wired), the emitter `--check` and the DLLink tests; the 25 bonus boards are an agent brief in flight. Native actors and admission/acceptance remain open. Detail: `docs/p2/P2-4-stage-production.md`. |
...
| P2-6 1P Game | **~55% — driver, bridge for every ladder stage, tally, bonus stages, menus, intro, challenger, ending, credits (with DS glyphs), boss and the five arenas in source (unbuilt)** | `NDS_P2_1P_GAME ?= 0`, no target sets it. The bridge refuses loudly the stages whose seams are landing: Giant DK and Metal Mario (admission in flight), the Polygon team (brief queued), the bonus boards (brief in flight), Race (no course yet), Master Hand (ftmain kind wiring, GC wallpaper enumerators). Header promotion of the shimmed scene types is in flight. |
| P2-7 Modes & meta | **~60% — items 1-9 in source (unbuilt)** | Save data, Options/Backup Clear/Sound Test, unlock message, Training, DATA menus, the attract demo and How to Play (wired from the title idle timer), the shell bridge and the header widening landed; the VS Options/Item Switch art audit passes. Save-driven unlock masks replacing the forced-open cartridge are an agent in flight. |
