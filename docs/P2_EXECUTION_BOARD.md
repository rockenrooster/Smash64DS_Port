...
| P2-4h1 | Peach's Castle hangs inside its battle | **ROOT-CAUSED; fix built, acceptance run pending** | `DOBJ_PARENT_NULL` is `(DObj*)1`, not NULL (`sys/objtypes.h:32`), and `gcPlayAnimAll`'s ascent (`sys/objanim.c:1455`) terminates only on that sentinel -- so a DObj built with a NULL parent walks forever. `itManagerSetupItemDObjs` makes one whenever a descriptor's id is non-zero before slot id-1 exists: `gcAddChildForDObj(NULL, dl)`. Three traces, all banked in `artifacts/p2-4/`: the bumper spawns (`MakeCount=1`, `AttrValidCount=1`); `gcDrawAll` never enters, so not the renderer; `gcPlayAnimAll` hits twice then stops, and the second `lr=0x20a1109` resolves to `itProcessProcItemMain` -- the ITEM's own per-frame process, not the stage's. The table check now fails closed with `gNdsItSetupDObjOrphanCount/Index/ID`, the same shape as the Hyrule and Inishie admission arms. |
