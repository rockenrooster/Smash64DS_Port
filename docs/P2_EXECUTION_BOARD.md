...
| P2-3 Fighter production | **ALL NINE FIGHTERS ZERO NATIVE FAILURES (09-09)** | Compiled config has Ness/Purin/Kirby=0. 09-09 long wave, 1,200 presents, shell ROM `5203F631`: `native failures=0` across all nine in 269.7 s. the last row closed was the Thunder Jolt **effect** (asset 342 root 0x2170), the air jolt's shell with word 17 turned into a segment-0xE hook. Battle acceptance, Ness smoke, Kirby heap and roster acceptance remain. Details: `docs/p2/fighters/`; pose clock: P2-3c1. |
| P2-4 Stage production | **ALL NINE STAGES ZERO NATIVE FAILURES (09-09); owner visual rows open** | 09-09 waves, 1,200 presents: `native failures=0` across all nine, held after the Zebes acid subdivision. Three same-day RETRACTIONS, all `DIAG_OWNERTRI` misreadings or scope slips: Saffron's `silent empty draw`, Mushroom Kingdom's side-platform row, and the emission claim below. Open per `docs/BUGS.md`: Castle roof, Yoster floor/clouds, Sector Arwing — and 09-09 narrowed the first two to ONE shape: both DECLARE their full triangle count, both pass every static and runtime decline gate, and both record zero native failures. The stronger claim that they "emit losslessly" is RETRACTED: `DIAG_OWNERTRI` accumulates `run->triangle_count`, not `emitted_triangles`, so emission is untested and a per-run emitted-vs-declared comparison is the next reading. BUG_NOTES names the runtime witnesses; watch the silent `inside_count == 0` cull. |
| P2-5 Items | **45 of 45 kinds in code; DRAW is the blocker: ~40 kinds have no native owner** | 09-09 audit (HIGH): spawn law, switch mask, frequency and the mball chain are all live and source-faithful, 45/45 makers registered -- the spawn half is not what is missing. Under the native-only contract an ownerless item spawns and records NO_PROGRAM instead of appearing, which is already proven on four of Saffron's five Pokemon. Second gap: every fighter pickup animation FileID reads `0u`, so pickup poses cannot load; fix is `admit_fighter.py` staging plus a manifest regenerate, not a hand edit. |
...
| P2-5i3 | Stage-spawned kinds | **8 OF 10 IN THE ROM; two behind the 1P flag** | POW block, Piranha and Saffron's five ship; Target and TaruBomb link behind `NDS_P2_1P_GAME`. Native owners exist for 1 of 42 distinct item shapes. |
