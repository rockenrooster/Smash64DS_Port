...
| P2-6 1P Game | **~40% — steps 1, 2, 5, 8 and the intro/challenger imported behind the flag (unbuilt)** | `NDS_P2_1P_GAME ?= 0`, no target sets it. Driver, Link/Hyrule bridge, tally, bonus stages, five 1P menus, intro and challenger landed; the 1P venues, ending/credits, boss and the variants probe are agents in flight. `stage_reloc_file.py` staged every 1P/menu sprite reloc file (`--extend` fills hand tables). |
| P2-7 Modes & meta | **~45% — items 1-5, the shell bridge (item 9) and the header widening landed (unbuilt)** | Mode Select routes 1P GAME/OPTION/DATA to registered source scenes through `ndsSeamRunSourceMenuScene`; `mndef.h`/`mntypes.h`/`gmsound.h` carry every enum and ordinal the imports name; the mixer owns mono/stereo and the BGM fade. Attract demo and the Item Switch art audit are queued briefs; masks still forced open in the harness. |
