...
| P2-6 1P Game | **~20% — steps 1, 2 (table) and 5 imported behind the flag (unbuilt)** | `NDS_P2_1P_GAME ?= 0`, no target sets it. Driver, Link/Hyrule bridge, stage-clear table and bonus stages landed; tally screen, intro, challenger, ending, credits and 1P menus are agent imports in flight. `stage_reloc_file.py` staged all 46 1P/menu sprite reloc files. Blockers are in the commits. |
| P2-7 Modes & meta | **~15% — save data landed; Options/Backup Clear/Sound Test imported (unbuilt)** | Item 1: `nds_backup.c` FAT-backed SRAM image + transcribed `lbbackup.c`. Item 5: three source TUs behind the 1P flag, unreachable until item 9 wires the shell; their compile needs the `mndef.h`/`gmsound.h` widening and three DS audio functions (briefs queued). Training, records, unlocks, attract: agents in flight. Masks still forced open (item 9). |
