...
| P2-5 Items | **IN PROGRESS — all 20 common items and 5 of 13 Pokémon in the ROM** | System landed; item switch UI and the remaining 8 Pokémon outstanding, the latter blocked on arena headroom. Stress = items ON. |
...
| P2-5i2 | The 13 Poke Ball Pokemon | **5 OF 13 IN THE ROM** | Kabigon, Tosakinto, Nyars, Dogas and Mew, verbatim from `src/it/itmonster/`. The dispatch table was sized `nITKindGBumper + 1`, below every Poke Ball kind AND below `nITKindMBall`, so none of them could be reached however they were rolled; it now runs to `nITKindMew`. Shared plumbing landed with them: `itGetMonsterAnimNode`, the monster SFX/voice block, the coin/hydro/smog weapon-vars members, `itDisplayCheckItemVisible` published, `G_RM_AA_ZB_TEX_EDGE`, and `include/nds/nds_obj_anim.h` for the animation helpers nine TUs had each redeclared. **BLOCKER for the remaining 8:** arena free floor fell 45,600 to 37,068 B against a 32,768 minimum across six item TUs; eight more will not fit. Size it before landing them. |
