...
| P2-3c1 | Q12 pose clock crosses command boundaries late for non-dyadic speeds | **OPEN — live, differential test in flight** | Exactness claim retracted (`a5861ddbeef`, repro in `docs/reviews/`). Non-dyadic speeds ARE live: every up-special landing lag (Mario 0.28, Fox 0.34, DK 0.3, Falcon/Link 0.65, Ness 0.17/0.24, Pikachu/Samus 0.4), aerial smooth-landing `flag1*0.01`, rebound `16/(damage*1.62+4)`. Test compares float32 vs Q12/16/20/24 crossing ticks over that set; no fix until it reports. |
...
| P2-5u1 | Item Switch and VS Options screens | **LANDED — runtime lap and art audit owed** | VS Options module `nds_menu_shell_vsoptions.c` (`7e69c9a66d1`) and the OPTIONS row opens it (`nds_menu_shell_mode_vs.c:617`); Item Switch cursor is three abutting cells at source coordinates (`936d37a0fd0`). Both are linked in the shell ELF. Owed: a scripted lap that reaches Item Switch through VS Options, and the `item_switch` ScreenSpec audit (agent out). |
