...
| P2-6 1P Game | **~35% — steps 1, 2, 5 and 8 imported behind the flag (unbuilt)** | `NDS_P2_1P_GAME ?= 0`, no target sets it. Driver, Link/Hyrule bridge, tally, bonus stages and the five 1P menus landed; intro/challenger and the 1P venues are agents in flight; ending, credits, boss and the variants are queued briefs. `stage_reloc_file.py` staged all 1P/menu sprite reloc files (`--extend` fills hand tables). |
| P2-7 Modes & meta | **~40% — items 1-5 imported and the shell bridge (item 9) landed (unbuilt)** | Mode Select routes 1P GAME/OPTION/DATA to registered source scenes through `ndsSeamRunSourceMenuScene`; ten kinds registered. Compile needs the `mndef.h`/`gmsound.h` widening and three DS audio functions (briefs running); attract demo queued; masks still forced open in the harness. |
