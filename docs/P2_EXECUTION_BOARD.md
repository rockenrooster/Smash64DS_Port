...
| P2-4h1 | Peach's Castle hangs inside its battle | **OPEN - isolated to Castle; both other explanations refuted** | The control settles it: the same shell-loop ROM on **Dream Land passes a full lap in 105 s** (11 scene entries, high-waters flat, free floor 171,008 B, zero faults). Castle reaches scene entry 7, confirms with `gkind=00` and `walkloops=1`, then produces no further entry in a **3,000 s** run. So not the scripted walk and not renderer speed. Ranked candidates: the GBumper spawn's data-dependent `DObjDesc` walk (`battleship_item_link_core.c:729`, verbatim against `itmanager.c:190-226`) and the platform joint animation. Next: `probe-trace-symbols.ps1` over `grCastleInitAll`/`itManagerMakeItem`/`grCastleBumperProcUpdate`. |
