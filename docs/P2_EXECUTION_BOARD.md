...
| P2-4s2 | Peach's Castle (Castle), stage 2 | **LINKS AND IS REACHABLE; CRASHES IN GROUND-DATA INIT** | `65f3ac0edc4`+`66e03203368`: wrapper including `grcastle.c` verbatim, admission arm, ground-data load, SSS mask bit, item core (`NDS_P2_ITEM_CORE` now also turns on for this flag), GBumper landed. **Measured 2026-09-03** on `build-p2-shell-loop-castle`: the shell lap selects it — `LOOPCFG gkind=00`, because Castle is SSS slot 0 and the cursor starts there, so **unlocking Castle changes which stage the scripted lap plays** — and then `LOOPABORT n=7 pc=01fffbe8 lr=0206ca4e`. `nm` names those `__excpt_entry` and **`mpCollisionInitGroundData` + 0xb2**. All three nitro payloads staged (`GRCastleMap`, `ExternDataBank106`, `MiscDataBank156`), so it is not a missing asset. Next: read `gNdsRelocFileSizeFallbackCount/Token/Asset` and `gNdsStageOptInAssetMask` at the fault — the 68-byte `sizeof(Sprite)` fallback writing a full map into a short allocation is the shape to rule out first. Acceptance recipe and globals: `docs/p2/P2-4-stage-production.md`. |
