...
| P2-3 Fighter production | **IN PROGRESS — nine enabled in the current public ROM** | The compiled config has Ness/Purin/Kirby=0; Purin has opt-in smoke evidence but is not enabled by default. Yoshi CSS preview is native again (commonpart flags lane, raw identity, pre-list fold) and Pikachu's ears draw natively (clamped sub-pow2 tile padding; `docs/BUGS.md`). Ness smoke, Kirby heap, Link integration, battle acceptance of both repairs and roster acceptance remain. Details: `docs/p2/fighters/`; pose clock: P2-3c1. |
| P2-4 Stage production | **REOPENED — owner reports missing art, geometry, collision and hazards** | All eight non-Dream-Land VS stages are affected; full reports remain in `docs/BUGS.md`. Packet-generation checks and completed laps did not establish visual/gameplay closure. Determine actual native/fallback routes; sub-15 FPS alone does not identify the renderer. |
