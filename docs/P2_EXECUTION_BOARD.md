...
| P2-4h1 | Stages froze, aborted or dropped their fighters | **CLOSED - all 8 stages boot and play** | Four defects of ONE shape, a hardcoded stage list standing in for "does this stage have valid data": camera-bound normalisation covered 3 of 9 assets; `mpCollisionInitGroundData` had a copied arm per stage and 4 had none; collision was gkind-gated in TWO places, so fighters fell through (owner-heard, invisible to every assertion); and three wrappers took the ADDRESS of `llGR<X>MapMapHeader` where the source uses it as an offset. All four now test the data. |
...
| ID | Slice | Status | Next / evidence |
|---|---|---|---|
| P2-5i1 | Item manager core, and the twenty common items | **ALL 20 IN THE ROM** | The Poke Ball closed the set. `ndsItGetAttackEvent` was Link's-bomb-only and returned NULL for every other kind, so all four containers dereferenced it and the first detonation aborted the ARM9; it now decodes any kind, cached by (file, symbol), against an oracle from `251_ITCommonData.c`. Checkers: `scripts/items/check-item-import-fidelity.py` (offsets, literals, redefined macros, the macro-glob comment trap, item-switch row order) and `scripts/check-audio-ordinals.py` (517 ordinals). Detail and traps: `docs/p2/P2-5-items.md`. |
| P2-5i2 | The 13 Poke Ball Pokemon | **ALL 13 IN THE ROM** | Dispatch proved by `gNdsItMonsterMakerMask` = `1fff`, read off the table rather than from a roll: a ball opens only when thrown or hit, so a 60 s CPU match can spawn five and open none. Pippi's Metronome table needs all twelve others, so the batch lands as a set. Item particle effects are invisible (`gITManagerParticleBankID` has no pack) -- presentation, not gameplay. |
...
| P2-5i4 | The fighter half of pickup | **LANDED** | `battleship_ftcommon_get.c` from `ftcommonget.c:12-348`. Nothing called `itMainSetFighterHold`, every Get proc was a weak stub, and `ftCommonGetCheckInterruptCommon` was a shim returning FALSE, so items were scenery. Heavy items included -- cutting them would make all four containers unpickable. Measured search=1 found=1 status=1 hold=1. A picked-up barrel never explodes, so the battle arena high-water fell 194,440 B and every pre-pickup arena figure is pessimistic. |
| P2-5u1 | Item Switch and VS Options screens | **RED -- the last P2-5 slice** | State and commit rule landed (`ndsMatchConfigItemTogglesFromRows`); the 37 surfaces are sized in `docs/p2/P2-5-items.md`. Entry point is the VS OPTIONS row, which refuses today (`nds_menu_shell_mode_vs.c:612`). |
