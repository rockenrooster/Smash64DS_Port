...
| P2-4 Stage production | **all 8 selectable; new native geometry unbuilt** | Yoster's packet/capture are wired; source fixes now cover stage identity, final admission, per-run matrices, animated bindings and Dream Land-only replay/culling. Seven descriptors remain; Castle is in progress. Runtime/visual acceptance is deferred by the code-first constraint. Detail: `docs/p2/P2-4-stage-production.md`. Sector Z and Congo BGM are wired. |
...
| P2-4n1 | Native stage packet, all stages | **Yoster source integration corrected; Castle descriptor in progress** | Seven descriptors remain. For each: `native_stage_descriptors/<stage>.py`, packet source check, adapter/capture rows and registry entry. Both checker arms now audit the shared C integration. Scene-owned blob once the payload sum bites (~95 KB at nine). Detail: `docs/p2/P2-4-stage-production.md`. |
