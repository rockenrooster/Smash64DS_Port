...
| P2-3f49 | The ten-flag ROM exhausts the arena before its first battle frame | **OPEN — re-measured with a real allocation census 2026-09-03; the cause is NOT fighter data** | Halt is now readable (`f0a8fb025ac` flushes the dcache before the spin, which is what the probe's "treat a row of zeroes as unreadable" paragraph was working around). Failing request: `ftManagerSetupFilesMainKind` loading **Fox's 115,440-byte file tree with 40,636 bytes of headroom**, arena `chosen=1,376,256` (up 28,672 from the dense-normal imaging), `allocfail=86` (was 93). **Census of the whole arena for a two-fighter battle** (`gNdsAllocLedgerTop*`, ranked): `ndsR2AnimCacheArenaEnsure` **451,776**; `ftManagerSetupFilesMainKind` 317,552; `scVSBattleSetupFiles` 208,672; `mpCollisionInitGroundData` 202,816; `ndsBaseEFManagerInitEffects` 52,736 + 28,352; `ftManagerAllocFighter` 33,152; `itManagerInitItems` 14,464. Total requested 1,419,796 across 38 callers. **Two findings change the plan.** (1) The largest single consumer is the animation cache reservation, 33% of the arena: 287,936 of battlepack carve plus 163,840 of behind-pack raw cache. Declining the carve returns ~193,728 and falls back to the standalone 258,048 raw cache — but that is a **measured performance trade** on a heavily tuned path, and four-CPU performance work is owner-parked, so it is not being taken unilaterally. (2) `ftManagerSetupFilesMainKind`'s 317,552 is the eager per-kind ladder in `battleship_mnplayersvs.c:496-527`, which loads **every admitted fighter's file closure** — ten of them for a two-fighter match. That is a scaling defect rather than a tuned choice and is the next non-trading lever. Static levers still sized and unspent: Mario/Fox owner tables ~64,147 B, prepared-dense to slot scratch ~65,784 B. |
