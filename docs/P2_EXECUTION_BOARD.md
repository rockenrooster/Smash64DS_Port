...
| P2-4s2 | Peach's Castle (Castle), stage 2 | **GAMEPLAY HALF LANDED opt-in (`NDS_P2_STAGE_CASTLE`, default 0); bumper waits on P2-5i1** | `65f3ac0edc4`: wrapper including `grcastle.c` verbatim, admission arm, ground-data load (bounds from source `MPGroundData`), SSS mask bit, five Makefile sites; flag-off `make` green. Payload derived from the bank-number rule — map 259=0x103, geometry 106=0x6a, nodes 156=0x9c, wallpaper 90=0x5a (`MVOpeningRoomWallpaper`, borrowed from the opening movie). **Flag-on will not link** until the item core defines `itManagerMakeItemSetupCommon`; the source's own `grcastle.c:17` NULL guard means the stage plays bumperless until then. Remaining: item core, native stage packet (law 8), SSS art, music (`nSYAudioBGMCastle = 6`), collision parity sweep, stress measurement. Pins: `docs/p2/P2-4-stage-production.md`. |
