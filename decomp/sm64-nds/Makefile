...
# NDS: use NitroFS for sound data to save ~2.3 MB in the binary
ifeq ($(TARGET_NDS),1)
NITROFS_DIR := $(BUILD_DIR)/nitrofs
endif # TARGET_NDS
...
# Prefer clang as C preprocessor if installed on the system (skip for NDS, uses devkitARM CPP)
ifneq ($(TARGET_NDS),1)
...
C_DEFINES += -D__NDS__
DEF_INC_CFLAGS += -D__NDS__

# Re-evaluate CPPFLAGS now that DEF_INC_CFLAGS is complete. The early := at
# the top of the Makefile captured it before the include paths were added,
# which made the 00_sound_player.s preprocessing fail silently (cannot find
# seq_macros.inc, error swallowed by the pipe into $(AS)) and produced an
# EMPTY sequence 0 - i.e. a ROM with no sound effects at all.
CPPFLAGS := -P -Wno-trigraphs -D_LANGUAGE_ASSEMBLY $(DEF_INC_CFLAGS)

TARGET_CFLAGS := -march=armv5te -mtune=arm946e-s -Wno-error=incompatible-pointer-types -Wno-error=implicit-function-declaration -Wno-error=int-conversion $(foreach dir,$(LIBDIRS),-I$(dir)/include) -DTARGET_NDS -DARM9 -D_LANGUAGE_C #-DENABLE_FPS
...
LDFLAGS := -ldswifi9 -lfat -lnds9 -lfilesystem -specs=ds_arm9.specs -g -mthumb -mthumb-interwork $(foreach dir,$(LIBDIRS),-L$(dir)/lib) $(TARGET_CFLAGS)

ARM7_CFLAGS := -fno-strict-aliasing -fwrapv $(OPT_FLAGS) $(ARM7_TARGET_CFLAGS) $(DEF_INC_CFLAGS)
ARM7_LDFLAGS := -ldswifi7 -lnds7 -specs=ds_arm7.specs -g -mthumb-interwork $(foreach dir,$(LIBDIRS),-L$(dir)/lib) $(ARM7_TARGET_CFLAGS)
...
ifeq ($(TARGET_NDS),1)
$(SOUND_BIN_DIR)/sound_data.o:        $(SOUND_BIN_DIR)/bank_sets.inc.c
else
...
$(BUILD_DIR)/%.inc.c: $(BUILD_DIR)/%
	...
	$(V)$(PYTHON) -c "import sys; data=open(sys.argv[1],'rb').read(); print(','.join('0x%X'%b for b in data)+',')" $< > $@
	$(V)echo >> $@
...
# ====================================================================
# NDS Segment Overlay Build System
# ====================================================================
# Data segments are externalized to NitroFS and loaded on demand.
# Code/script/geo segments stay in the main binary (resident).
#
# Overlay data .o files are linked at segment addresses to produce
# ELFs with correct internal pointers, then extracted to .bin files
# for NitroFS. Symbol addresses are extracted and provided to the
# main binary link via PROVIDE directives.
# ====================================================================

NDS_OVERLAY_LD := $(DEVKITARM)/bin/arm-none-eabi-ld
NDS_OVERLAY_NM := $(DEVKITARM)/bin/arm-none-eabi-nm

# Texture bins to externalize (have SEGMENT_ADDRESS in Makefile.split)
NDS_BIN_SEGMENTS := segment2 title_screen_bg debug_level_select \
    spooky generic outside snow effect cave inside water fire sky machine mountain grass

# Skyboxes to externalize
NDS_SKYBOX_SEGMENTS := $(addsuffix _skybox,$(notdir $(basename $(wildcard textures/skyboxes/*.png))))

# All data .o files to externalize to NitroFS
NDS_OVERLAY_O_FILES := \
    $(ACTOR_GROUPS:%=$(BUILD_DIR)/actors/%.o) \
    $(LEVEL_FILES:%=$(BUILD_DIR)/levels/%.o) \
    $(NDS_BIN_SEGMENTS:%=$(BUILD_DIR)/bin/%.o) \
    $(NDS_SKYBOX_SEGMENTS:%=$(BUILD_DIR)/bin/%.o)

# Disable LTO for overlay data files (raw ld can't handle LTO objects)
$(NDS_OVERLAY_O_FILES): CFLAGS = -fno-strict-aliasing -fwrapv -fno-lto -O2 -ffast-math $(TARGET_CFLAGS) $(DEF_INC_CFLAGS)

# Main binary .o files = everything except overlay data
NDS_MAIN_O_FILES := $(filter-out $(NDS_OVERLAY_O_FILES),$(O_FILES))

# Overlay ELF files (linked at segment addresses)
NDS_OVERLAY_ELFS := \
    $(ACTOR_GROUPS:%=$(BUILD_DIR)/actors/%.elf) \
    $(LEVEL_FILES:%=$(BUILD_DIR)/levels/%.elf) \
    $(NDS_BIN_SEGMENTS:%=$(BUILD_DIR)/bin/%.elf) \
    $(NDS_SKYBOX_SEGMENTS:%=$(BUILD_DIR)/bin/%.elf)

# Binary files extracted from overlay ELFs
NDS_OVERLAY_BINS := $(NDS_OVERLAY_ELFS:.elf=.bin)

# Generated segment stubs (NitroFS paths / empty for resident)
NDS_SEGMENT_STUBS_C := $(BUILD_DIR)/nds_segment_stubs.c
NDS_SEGMENT_STUBS_O := $(NDS_SEGMENT_STUBS_C:.c=.o)

# Overlay symbol linker script (PROVIDE directives)
NDS_OVERLAY_SYMS_LD := $(BUILD_DIR)/nds_overlay_syms.ld

# --- Build overlay ELFs at segment addresses ---

# Actor data overlays (no external symbol dependencies)
$(BUILD_DIR)/actors/%.elf: $(BUILD_DIR)/actors/%.o
	$(call print,Linking NDS overlay:,$<,$@)
	$(V)$(NDS_OVERLAY_LD) -e 0 -Ttext=$(SEGMENT_ADDRESS) -o $@ $<

# Level data overlays (need texture bin symbols for cross-references)
.SECONDEXPANSION:
$(BUILD_DIR)/levels/%/leveldata.elf: $(BUILD_DIR)/levels/%/leveldata.o $(BUILD_DIR)/bin/$$(TEXTURE_BIN).elf
	$(call print,Linking NDS overlay:,$<,$@)
	$(V)$(NDS_OVERLAY_LD) -e 0 -Ttext=$(SEGMENT_ADDRESS) --just-symbols=$(BUILD_DIR)/bin/$(TEXTURE_BIN).elf -o $@ $<

# Bin/texture overlays
$(BUILD_DIR)/bin/%.elf: $(BUILD_DIR)/bin/%.o
	$(call print,Linking NDS overlay:,$<,$@)
	$(V)$(NDS_OVERLAY_LD) -e 0 -Ttext=$(SEGMENT_ADDRESS) -o $@ $<

# --- Extract binary from overlay ELFs ---
$(BUILD_DIR)/actors/%.bin: $(BUILD_DIR)/actors/%.elf
	$(call print,Extracting overlay bin:,$<,$@)
	$(V)$(OBJCOPY) -O binary $< $@
...
# --- Generate segment stubs ---
$(NDS_SEGMENT_STUBS_C): levels/level_defines.h tools/nds_gen_segment_stubs.py
	$(call print,Generating segment stubs:,$<,$@)
	$(V)$(PYTHON) tools/nds_gen_segment_stubs.py $< $@

$(NDS_SEGMENT_STUBS_O): $(NDS_SEGMENT_STUBS_C)
	$(call print,Compiling:,$<,$@)
	$(V)$(CC) -c $(CFLAGS) -o $@ $<

# --- Extract overlay symbols for PROVIDE directives ---
$(NDS_OVERLAY_SYMS_LD): $(NDS_OVERLAY_ELFS) tools/nds_extract_overlay_syms.py
	$(call print,Extracting overlay symbols:,$(words $(NDS_OVERLAY_ELFS)) ELFs,$@)
	$(V)$(PYTHON) tools/nds_extract_overlay_syms.py $(NDS_OVERLAY_NM) $@ $(NDS_OVERLAY_ELFS)

# --- Copy overlay bins to NitroFS ---
$(NITROFS_DIR)/data/actors/%.bin: $(BUILD_DIR)/actors/%.bin
	@mkdir -p $(dir $@)
	@cp $< $@

$(NITROFS_DIR)/data/levels/%/leveldata.bin: $(BUILD_DIR)/levels/%/leveldata.bin
	@mkdir -p $(dir $@)
	@cp $< $@

$(NITROFS_DIR)/data/bin/%.bin: $(BUILD_DIR)/bin/%.bin
	@mkdir -p $(dir $@)
	@cp $< $@

# NitroFS data files for overlay segments
NDS_NITROFS_DATA := \
    $(ACTOR_GROUPS:%=$(NITROFS_DIR)/data/actors/%.bin) \
    $(LEVEL_FILES:%=$(NITROFS_DIR)/data/levels/%.bin) \
    $(NDS_BIN_SEGMENTS:%=$(NITROFS_DIR)/data/bin/%.bin) \
    $(NDS_SKYBOX_SEGMENTS:%=$(NITROFS_DIR)/data/bin/%.bin)

# ====================================================================
# NDS Binary Linking and ROM Build
# ====================================================================
...
# ARM9 binary - main code + resident data, with overlay symbol definitions
$(ARM9): $(GFX_O_FILES) $(NDS_MAIN_O_FILES) $(NDS_SEGMENT_STUBS_O) $(ULTRA_O_FILES) $(GODDARD_O_FILES) $(NDS_OVERLAY_SYMS_LD)
	@$(PRINT) "$(GREEN)Linking ARM9 binary:  $(BLUE)$@ $(NO_COL)\n"
	$(V)$(LD) -L $(BUILD_DIR) -T $(NDS_OVERLAY_SYMS_LD) -o $@ $(GFX_O_FILES) $(NDS_MAIN_O_FILES) $(NDS_SEGMENT_STUBS_O) $(ULTRA_O_FILES) $(GODDARD_O_FILES) $(LDFLAGS)

# NitroFS sound data
$(NITROFS_DIR)/sound:
	@mkdir -p $@

$(NITROFS_DIR)/sound/sound_data.ctl: $(SOUND_BIN_DIR)/sound_data.ctl | $(NITROFS_DIR)/sound
	@cp $< $@

$(NITROFS_DIR)/sound/sound_data.tbl: $(SOUND_BIN_DIR)/sound_data.tbl | $(NITROFS_DIR)/sound
	@cp $< $@

$(NITROFS_DIR)/sound/sequences.bin: $(SOUND_BIN_DIR)/sequences.bin | $(NITROFS_DIR)/sound
	@cp $< $@

NDS_NITROFS_FILES := \
	$(NITROFS_DIR)/sound/sound_data.ctl \
	$(NITROFS_DIR)/sound/sound_data.tbl \
	$(NITROFS_DIR)/sound/sequences.bin

# Build ROM with NitroFS (sound + overlay data)
