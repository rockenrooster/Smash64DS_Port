...
#ifdef TARGET_NDS
#include "../nds/nds_sample_cache.h"
#endif
...
f32 *gCurrentLeftVolRamping;
f32 *gCurrentRightVolRamping;
...
u64 *synthesis_do_one_audio_update(s16 *aiBuf, s32 bufLen, u64 *cmd, s32 updateIndex) {
    ...
    if (gSynthesisReverb.useReverb == 0) {
        ...
        if (gReverbDownsampleRate == 1) {
            ...
            aMix(cmd++, 0,  0x8000 + gSynthesisReverb.reverbGain,  DMEM_ADDR_WET_LEFT_CH,
            ...
            aSetBuffer(cmd++, 0, 0, 0, DEFAULT_LEN_2CH);
            aMix(cmd++, 0,  0x8000 + gSynthesisReverb.reverbGain,  DMEM_ADDR_LEFT_CH,  DMEM_ADDR_LEFT_CH);
...
u64 *synthesis_process_notes(s16 *aiBuf, s32 bufLen, u64 *cmd) {
    ...
    for (noteIndex = 0; noteIndex < gMaxSimultaneousNotes; noteIndex++) {
        ...
        if (note->noteSubEu.enabled == FALSE) {
            ...
            note->samplePosFrac = samplesLenFixedPoint & 0xFFFF;
#else
            ...
            if (note->sound == NULL) {
                ...
                for (curPart = 0; curPart < nParts; curPart++) {
                    ...
                    if (noteSubEu->bookOffset) {
                        curLoadedBook = euUnknownData_80301950;
                    }
#endif
...
u64 *load_wave_samples(u64 *cmd, struct NoteSubEu *noteSubEu, struct NoteSynthesisState *synthesisState, s32 nSamplesToLoad) {
    ...
    s32 repeats;
    s32 i;
    aSetBuffer(cmd++,  0,  DMEM_ADDR_UNCOMPRESSED_NOTE,  0,  128);
    ...
    if (a3 < nSamplesToLoad) {
        ...
        for (i = 0; i < repeats; i++) {
            aDMEMMove(cmd++,
                       ...
                       DMEM_ADDR_UNCOMPRESSED_NOTE + (1 + i) * 128,
                       128);
        }
    }
    return cmd;
...
u64 *load_wave_samples(u64 *cmd, struct Note *note, s32 nSamplesToLoad) {
    s32 a3;
    s32 i;
    aSetBuffer(cmd++,  0,  DMEM_ADDR_UNCOMPRESSED_NOTE,  0,
                sizeof(note->synthesisBuffers->samples));
    ...
    if (a3 < nSamplesToLoad) {
        for (i = 0; i <= (nSamplesToLoad - a3 + 63) / 64 - 1; i++) {
            aDMEMMove(cmd++,  DMEM_ADDR_UNCOMPRESSED_NOTE,  DMEM_ADDR_UNCOMPRESSED_NOTE + (1 + i) * sizeof(note->synthesisBuffers->samples),  sizeof(note->synthesisBuffers->samples));
        }
    }
    return cmd;
}
...
#ifdef VERSION_EU
u64 *final_resample(u64 *cmd, struct NoteSynthesisState *synthesisState, s32 count, u16 pitch, u16 dmemIn, u32 flags) {
    aSetBuffer(cmd++,  0, dmemIn,  DMEM_ADDR_TEMP, count);
...
u64 *note_apply_headset_pan_effects(u64 *cmd, struct Note *note, s32 bufLen, s32 flags, s32 leftRight) {
    ...
    aMix(cmd++, 0,  0x7fff,  DMEM_ADDR_NOTE_PAN_TEMP,  dest);
...
#ifdef VERSION_JP
    note->targetVolLeft = (u16)(velocity * volLeft) & ~0x80FF;
