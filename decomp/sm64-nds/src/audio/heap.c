...
void audio_reset_session(void) {
...

    {
        ...
        static struct Note sNdsNotes[20] __attribute__((aligned(32)));
        u32 *p;
        gNotes = sNdsNotes;

        for (p = (u32 *) gNotes; p < (u32 *) ((u8 *) gNotes + sizeof(sNdsNotes)); p++) {
            *p = 0;
        }
    }
#else
