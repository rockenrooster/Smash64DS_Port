...
static inline void nds_emit_tint(u32 packed) {
    gDisplayListHead->words.w0 = ((uintptr_t) G_SPECIAL_1 << 24) | ((packed >> 24) & 0xFF);
    gDisplayListHead->words.w1 = (uintptr_t) (packed & 0xFFFFFF);
    gDisplayListHead++;
}
...
NDS_DTCM_BSS Mat4 gMatStack[32];
NDS_DTCM_BSS Mtx *gMatStackFixed[32];
...
#ifdef F3DEX_GBI_2
    Mtx lMtx;
    guLookAtReflect(&lMtx, &lookAt, 0, 0, 0,  0, 0, 1,  1, 0, 0 );
#endif
...
static void geo_process_level_of_detail(struct GraphNodeLevelOfDetail *node) {
    ...
    s16 distanceFromCam = (s32) -mtx->m[3][2];
#else

    Mtx *mtx = gMatStackFixed[gMatStackIndex];
    s16 distanceFromCam = -GET_HIGH_S16_OF_32(mtx->m[1][3]);
#endif
...
static void geo_process_camera(struct GraphNodeCamera *node) {
    ...
    mtxf_mul_to_fixed(mtx, gMatStack[gMatStackIndex + 1], cameraTransform, gMatStack[gMatStackIndex]);
    gMatStackIndex++;
...
static void geo_process_translation_rotation(struct GraphNodeTranslationRotation *node) {
    ...
    mtxf_rotate_zxy_and_translate_fixed(mtxf, translation, node->rotation);
    mtxf_mul_fixed_to_fixed(mtx, gMatStack[gMatStackIndex + 1], mtxf, gMatStackFixed[gMatStackIndex]);
    gMatStackIndex++;
    gMatStackFixed[gMatStackIndex] = mtx;
...
static void geo_process_translation(struct GraphNodeTranslation *node) {
    ...
    vec3s_to_vec3f(translation, node->translation);
    mtxf_rotate_zxy_and_translate_fixed(mtxf, translation, gVec3sZero);
    mtxf_mul_fixed_to_fixed(mtx, gMatStack[gMatStackIndex + 1], mtxf, gMatStackFixed[gMatStackIndex]);
    gMatStackIndex++;
    gMatStackFixed[gMatStackIndex] = mtx;
    if (node->displayList != NULL) {
...
static void geo_process_rotation(struct GraphNodeRotation *node) {
    ...
    mtxf_rotate_zxy_and_translate_fixed(mtxf, gVec3fZero, node->rotation);
    mtxf_mul_fixed_to_fixed(mtx, gMatStack[gMatStackIndex + 1], mtxf, gMatStackFixed[gMatStackIndex]);
    gMatStackIndex++;
    gMatStackFixed[gMatStackIndex] = mtx;
    if (node->displayList != NULL) {
...
static void geo_process_animated_part(struct GraphNodeAnimatedPart *node) {
    ...
    mtxf_rotate_xyz_and_translate_fixed(matrix, translation, rotation);
    mtxf_mul_fixed_to_fixed(matrixPtr, gMatStack[gMatStackIndex + 1], matrix, gMatStackFixed[gMatStackIndex]);
    gMatStackIndex++;
...
static s32 obj_is_in_view(struct GraphNodeObject *node, Mat4 matrix) {
    ...
    if (geo != NULL && geo->type == GRAPH_NODE_TYPE_CULLING_RADIUS) {
        cullingRadius =
            (f32)((struct GraphNodeCullingRadius *) geo)->cullingRadius;
    } else {
        cullingRadius = 300;
    }
...
static void geo_process_object(struct Object *node) {
    ...
    if (node->header.gfx.areaIndex == gCurGraphNodeRoot->areaIndex) {
        if (node->header.gfx.throwMatrix != NULL) {
            ...
            s32 localFixed[4][4];
            mtxf_rotate_zxy_and_translate_fixed(localFixed, node->header.gfx.pos,
                                                node->header.gfx.angle);
            mtxf_mul_fixed_raw(objFixed, gMatStack[gMatStackIndex + 1], localFixed,
                               gMatStackFixed[gMatStackIndex]);
            useFixed = TRUE;
        }

        if (useFixed) {
            mtxf_scale_fixed(objFixed, gMatStack[gMatStackIndex + 1], node->header.gfx.scale);
        } else {
        ...
        struct GraphNode *ndsSavedChild = NULL;
        struct AnimInfo ndsSavedAnimInfo;
        struct GraphNode *ndsAppGeo = NULL;
        struct AnimInfo *ndsAppAnim = NULL;
        u32 ndsTint = 0;
        s32 ndsAppFlags = 0;
        if (gNdsLocalAppearanceModel || gNdsLocalAppearanceColor || gNdsNetPlayPuppetActive) {
            ndsAppFlags = nds_object_render_appearance(node, &ndsAppGeo, &ndsTint, &ndsAppAnim);
            if (ndsAppFlags & 1) {
                ndsSavedChild = node->header.gfx.sharedChild;
                node->header.gfx.sharedChild = ndsAppGeo;
            }
            if (ndsAppFlags & (2 | 8)) {
                ndsSavedAnimInfo = node->header.gfx.animInfo;
                if (ndsAppFlags & 8) {

                    node->header.gfx.animInfo = *ndsAppAnim;
                } else {
                    node->header.gfx.animInfo.curAnim = NULL;
                }
            }
        }
#endif
...
#if defined(TARGET_NDS) && defined(ARM9)
        if (ndsAppFlags & 1) {
            node->header.gfx.sharedChild = ndsSavedChild;
        }
        if (ndsAppFlags & (2 | 8)) {
            node->header.gfx.animInfo = ndsSavedAnimInfo;
        }
#endif
...
NDS_ITCM_CODE void geo_process_node_and_siblings(struct GraphNode *firstNode) {
