...
s32 net_classify_attack(struct MarioState *m, struct Object *o) {
    ...
    if (interaction & (INT_FAST_ATTACK_OR_SHELL | INT_SLIDE_KICK
                       | INT_GROUND_POUND_OR_TWIRL | INT_HIT_FROM_ABOVE)) {
        return 2;
    }
    return 0;
}
...
void net_knockback_mario(struct MarioState *m, s16 pushYaw, f32 force) {
    ...
    if (!(m->action & (ACT_FLAG_AIR | ACT_FLAG_MOVING | ACT_FLAG_STATIONARY))) {
        return;
    }
    if (force < 8.0f) {
        force = 8.0f;
    ...
    m->faceAngle[1] = pushYaw + 0x8000;
    m->faceAngle[2] = 0;
    if (m->action & ACT_FLAG_AIR) {
        action = ACT_BACKWARD_AIR_KB;
        if (m->vel[1] < 20.0f) {
            m->vel[1] = 20.0f;
        }
    } else {
        action = ACT_SOFT_BACKWARD_GROUND_KB;
    }
    mario_set_forward_vel(m, -force);
