...
static struct ObjBone *gGdTempBone = NULL;
...
static struct ObjJoint *sJointArr[10];
static struct GdVec3f sJointArrVecs[10];
static s32 sJointArr2Len;
static struct ObjJoint *sJointArr2[10];
...
void grabbable_joint_update_func(struct ObjJoint *self) {
    ...
    if (self->header.drawFlags & OBJ_PICKED) {
        ...
        if (gGdCtrl.trgR == FALSE) {

            self->velocity.x -= offset.x * 0.5;
            self->velocity.y -= offset.y * 0.5;
            self->velocity.z -= offset.z * 0.5;
...
void func_8018F328(struct ObjBone *b) {
    ...
    b->worldPos.x = (joint1->worldPos.x + joint2->worldPos.x) / 2.0;
    b->worldPos.y = (joint1->worldPos.y + joint2->worldPos.y) / 2.0;
    b->worldPos.z = (joint1->worldPos.z + joint2->worldPos.z) / 2.0;
    ...
    gd_create_origin_lookat(&b->matB0, &b->unk58, 0);
}
...
void func_8018F89C(struct ObjBone *b) {
    ...
    b->worldPos.x = (spAC->worldPos.x + spA8->worldPos.x) / 2.0;
    b->worldPos.y = (spAC->worldPos.y + spA8->worldPos.y) / 2.0;
    b->worldPos.z = (spAC->worldPos.z + spA8->worldPos.z) / 2.0;
...
void func_80190574(s32 a0, struct ObjJoint *a1, struct ObjJoint *a2, f32 x, f32 y, f32 z) {
    struct ObjJoint *sp274;
    struct ObjJoint *sp270;
    struct ObjJoint *sp26C;
    UNUSED u8 filler1[4];
    UNUSED u32 unused = 0;
