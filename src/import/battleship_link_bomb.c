 ...
 *
 * The state machine below is source-shaped; the DS-only adaptations are
 * documented at the reloc lookup, particle scale, and release-ABI seams. */
...
sb32 func_ovl3_801864BC(GObj *item_gobj);
...
/* P2-3 LinkBomb: BattleShip state machine retained verbatim except for two DS
 * adaptation seams documented at their call sites below: reloc token lookup
 * and HAL's intentionally wrong three-argument release prototype. */

uintptr_t llLinkMainBombItemAttributes = 0x40u;
uintptr_t llLinkMainBombAttackEvents = 0x88u;
uintptr_t llLinkMainBombBloatScales = 0xA8u;
...
extern void ndsEFManagerScaleParticle(LBParticle *pc, f32 scale);
extern void func_800269C0_275C0(u16 sfx_id);

static void ndsItLinkBombSetFighterRelease3(GObj *item_gobj, Vec3f *vel,
                                             f32 throw_mul)
{
    ITStruct *ip = itGetStruct(item_gobj);

    /* HAL compiled this TU against a false 3-arg declaration of a 5-arg
     * function. On MIPS the garbage extra registers are overwritten before
     * the explosion becomes active. ARM cannot safely reproduce undefined
     * call ABI state, so pass the item's current source stat lanes explicitly;
     * explosion setup still replaces the attack state exactly as BattleShip. */
    itMainSetFighterRelease(item_gobj, vel, throw_mul,
                            ip->attack_coll.stat_flags.halfword,
                            ip->attack_coll.stat_count);
}
...
void itLinkBombExplodeWaitUpdateScale(GObj *item_gobj)
{
	...
	{
		f32 *scales = lbRelocGetFileData(f32*, *dItLinkBombItemDesc.p_file,
												  &llLinkMainBombBloatScales);
...
void itLinkBombExplodeMakeEffectGotoSetStatus(GObj *item_gobj)
{
	...
	if (pc != NULL)
	{
		ndsEFManagerScaleParticle(pc, ITLINKBOMB_EXPLODE_EFFECT_SCALE);
	}
...
sb32 itLinkBombHoldProcUpdate(GObj *item_gobj)
{
	...
	else
	{
		if (ip->lifetime == 0)
		{
			...
			ndsItLinkBombSetFighterRelease3(item_gobj, &ip->physics.vel_air, 1.0F);
