...
/* Dream Land layer zero is the first immutable owner prepared by the
 * adapter.  Compilation observes one generic traversal.  A whole-owner
 * preflight then allows the exact executor to replace source scanning on
 * later frames without changing the live matrix/material/cache boundary. */
#define NDS_RENDERER_PREPARED_STAGE0_ARENA_BYTES 6144u
#define NDS_RENDERER_PREPARED_STAGE0_DOBJ_COUNT 21u
#define NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT 20u
#define NDS_RENDERER_PREPARED_STAGE0_OPERATION_CAPACITY 91u
#define NDS_RENDERER_PREPARED_STAGE0_EXEC_OPERATION_COUNT 69u
#define NDS_RENDERER_PREPARED_STAGE0_LOGICAL_OPERATION_COUNT 91u
#define NDS_RENDERER_PREPARED_STAGE0_APPLY_COUNT 17u
#define NDS_RENDERER_PREPARED_STAGE0_STATE_GROUP_COUNT 17u
#define NDS_RENDERER_PREPARED_STAGE0_STATE_ACTION_COUNT 123u
#define NDS_RENDERER_PREPARED_STAGE0_STATE_SCRATCH_COUNT 32u
#define NDS_RENDERER_PREPARED_STAGE0_VERTEX_BLOCK_COUNT 26u
#define NDS_RENDERER_PREPARED_STAGE0_VERTEX_COUNT 108u
#define NDS_RENDERER_PREPARED_STAGE0_DRAW_RUN_COUNT 26u
#define NDS_RENDERER_PREPARED_STAGE0_TRI_COMMAND_COUNT 36u
#define NDS_RENDERER_PREPARED_STAGE0_TRIANGLE_COUNT 54u
#define NDS_RENDERER_PREPARED_STAGE0_LINK_ID 4
#define NDS_RENDERER_PREPARED_STAGE0_MODE_GENERIC 1u
#define NDS_RENDERER_PREPARED_STAGE0_MODE_PREPARED 2u

typedef enum NDSRendererPreparedStage0Fallback
{
    NDS_RENDERER_PREPARED_STAGE0_FALLBACK_NONE = 0,
    NDS_RENDERER_PREPARED_STAGE0_FALLBACK_ASSETS = 1,
    NDS_RENDERER_PREPARED_STAGE0_FALLBACK_SELECTION = 2,
    NDS_RENDERER_PREPARED_STAGE0_FALLBACK_ARENA = 3,
    NDS_RENDERER_PREPARED_STAGE0_FALLBACK_UNSUPPORTED = 4,
    NDS_RENDERER_PREPARED_STAGE0_FALLBACK_DEPTH = 5,
    NDS_RENDERER_PREPARED_STAGE0_FALLBACK_RESOLVE = 6,
    NDS_RENDERER_PREPARED_STAGE0_FALLBACK_IMMUTABLE = 7,
    NDS_RENDERER_PREPARED_STAGE0_FALLBACK_GENERIC = 8,
    NDS_RENDERER_PREPARED_STAGE0_FALLBACK_CENSUS = 9
} NDSRendererPreparedStage0Fallback;

typedef enum NDSRendererPreparedStage0ExecutionFailure
{
    NDS_RENDERER_PREPARED_STAGE0_EXEC_FAILURE_NONE = 0,
    NDS_RENDERER_PREPARED_STAGE0_EXEC_FAILURE_PREFLIGHT = 1,
    NDS_RENDERER_PREPARED_STAGE0_EXEC_FAILURE_BINDING = 2,
    NDS_RENDERER_PREPARED_STAGE0_EXEC_FAILURE_RENDERER = 3,
    NDS_RENDERER_PREPARED_STAGE0_EXEC_FAILURE_INCOMPLETE = 4
} NDSRendererPreparedStage0ExecutionFailure;

typedef struct NDSRendererPreparedStage0StateGroup
{
    u16 payload_index;
    u16 count;
    u32 hash;
} NDSRendererPreparedStage0StateGroup;

typedef struct NDSRendererPreparedStage0Image
{
    NDSRendererPreparedList lists[NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT];
    NDSRendererPreparedOp
        operations[NDS_RENDERER_PREPARED_STAGE0_OPERATION_CAPACITY];
    Gfx state_commands[
        NDS_RENDERER_PREPARED_STAGE0_STATE_ACTION_COUNT +
        NDS_RENDERER_PREPARED_STAGE0_STATE_SCRATCH_COUNT];
    NDSRendererInputVertex
        vertices[NDS_RENDERER_PREPARED_STAGE0_VERTEX_COUNT];
    NDSRendererPreparedTriangleCommand
        triangle_commands[NDS_RENDERER_PREPARED_STAGE0_TRI_COMMAND_COUNT];
    NDSRendererPreparedStage0StateGroup
        state_groups[NDS_RENDERER_PREPARED_STAGE0_STATE_GROUP_COUNT];
} NDSRendererPreparedStage0Image;

typedef union NDSRendererPreparedStage0Arena
{
    u32 align;
    u8 bytes[NDS_RENDERER_PREPARED_STAGE0_ARENA_BYTES];
    NDSRendererPreparedStage0Image image;
} NDSRendererPreparedStage0Arena;

_Static_assert(sizeof(NDSRendererPreparedStage0Image) <=
                   NDS_RENDERER_PREPARED_STAGE0_ARENA_BYTES,
               "Dream Land layer-zero prepared program exceeded 6 KiB");

typedef struct NDSRendererPreparedStage0Compiler
{
    u32 active;
    u32 failed;
    u32 valid;
    ...
    u32 operation_count;
    u32 apply_op_count;
    u32 state_group_count;
    u32 state_action_count;
    u32 vertex_block_count;
    u32 source_vertex_count;
    u32 draw_run_count;
    u32 triangle_command_count;
    u32 triangle_count;
    u32 immutable_list_count;
    u32 trusted_command_count;
    u32 validated_command_count;
    u32 branch_count;
    u32 branch_call_count;
    u32 branch_jump_count;
    u32 current_list;
    u32 current_operation_start;
    u32 current_list_active;
    u32 pending_state_start;
    u32 pending_state_count;
    u32 pending_source_index;
    u32 draw_run_start;
    u32 draw_run_count_current;
    u32 draw_run_source_index;
    u32 root_branch_index;
    u32 root_branch_is_jump;
} NDSRendererPreparedStage0Compiler;

typedef struct NDSRendererPreparedStage0CallbackContext
{
    NDSFighterDLDrawState *draw_state;
    NDSRendererCommandCallback base_callback;
} NDSRendererPreparedStage0CallbackContext;

typedef struct NDSRendererPreparedStage0Replay
{
    u32 armed;
    u32 failed;
    u32 next_list;
    DObj *dobjs[NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT];
    const Gfx *roots[NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT];
} NDSRendererPreparedStage0Replay;

static NDSRendererPreparedStage0Arena sNdsRendererPreparedStage0Arena;
static NDSRendererPreparedStage0Compiler sNdsRendererPreparedStage0Compiler;
static NDSRendererPreparedStage0Replay sNdsRendererPreparedStage0Replay;

#define NDS_RENDERER_PREPARED_HASH_SEED 2166136261u
#define NDS_RENDERER_PREPARED_HASH2_SEED 0x9e3779b9u

static u32 ndsRendererPreparedStage0HashBytes(u32 hash, const void *data,
                                               size_t bytes)
{
    ...
    if (hash == 0u)
    {
        hash = NDS_RENDERER_PREPARED_HASH_SEED;
    }
    for (i = 0u; i < bytes; i++)
    {
        hash ^= cursor[i];
...
static u32 ndsRendererPreparedStage0Hash2Bytes(u32 hash, const void *data,
                                                size_t bytes)
{
    const u8 *cursor = data;
    size_t i;
    ...
    if (hash == 0u)
    {
        hash = NDS_RENDERER_PREPARED_HASH2_SEED;
    }
    for (i = 0u; i < bytes; i++)
    {
        hash ^= (u32)cursor[i] + 0x9du;
        hash = (hash << 5) | (hash >> 27);
        hash *= 0x85ebca6bu;
    }
    return (hash != 0u) ? hash : 1u;
...
static u32 ndsRendererPreparedStage0HashU32(u32 hash, u32 value)
{
    return ndsRendererPreparedStage0HashBytes(hash, &value, sizeof(value));
}

static u32 ndsRendererPreparedStage0Hash2U32(u32 hash, u32 value)
{
    return ndsRendererPreparedStage0Hash2Bytes(hash, &value, sizeof(value));
}

static void ndsRendererPreparedStage0Fail(u32 reason)
{
    NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;

    if (compiler->failed == 0u)
    {
        compiler->fallback_reason = reason;
    }
...
static u32 ndsRendererPreparedStage0HashPointer(u32 hash,
                                                const void *pointer)
{
    const NDSRelocLoadedFile *loaded =
        ndsRelocFindLoadedFileContaining(pointer, 1u);
    uintptr_t value = (uintptr_t)pointer;

    if (pointer == NULL)
    {
        return ndsRendererPreparedStage0HashU32(hash, 0u);
    }
    if (loaded != NULL)
    {
        hash = ndsRendererPreparedStage0HashU32(hash, 1u);
        hash = ndsRendererPreparedStage0HashU32(hash, loaded->asset_id);
        hash = ndsRendererPreparedStage0HashU32(
            hash, loaded->owner_generation);
        return ndsRendererPreparedStage0HashU32(
            hash, (u32)(value - (uintptr_t)loaded->data));
    }
    hash = ndsRendererPreparedStage0HashU32(hash, 2u);
    hash = ndsRendererPreparedStage0HashU32(hash, (u32)(value >> 24));
    return ndsRendererPreparedStage0HashU32(
        hash, (u32)(value & 0x00ffffffu));
}

static u32 ndsRendererPreparedStage0Hash2Pointer(u32 hash,
                                                 const void *pointer)
{
    const NDSRelocLoadedFile *loaded =
        ndsRelocFindLoadedFileContaining(pointer, 1u);
    ...
    if (pointer == NULL)
    {
        return ndsRendererPreparedStage0Hash2U32(hash, 0u);
    }
    if (loaded != NULL)
    {
        hash = ndsRendererPreparedStage0Hash2U32(hash, 1u);
        hash = ndsRendererPreparedStage0Hash2U32(hash, loaded->asset_id);
        hash = ndsRendererPreparedStage0Hash2U32(
            hash, loaded->owner_generation);
        return ndsRendererPreparedStage0Hash2U32(
            hash, (u32)(value - (uintptr_t)loaded->data));
    }
    hash = ndsRendererPreparedStage0Hash2U32(hash, 2u);
    hash = ndsRendererPreparedStage0Hash2U32(hash, (u32)(value >> 24));
    return ndsRendererPreparedStage0Hash2U32(
        hash, (u32)(value & 0x00ffffffu));
}

static s32 ndsRendererPreparedStage0RootAccepted(
    const Gfx *dl, u32 *hash, u32 *hash2)
{
    const NDSRelocLoadedFile *loaded;
    ...
    loaded = ndsRelocFindLoadedFileContaining(dl, sizeof(*dl));
    if ((loaded == NULL) ||
        (loaded->asset_id != NDS_RELOC_ASSET_EXTERN_DATA_BANK_104))
    {
        return FALSE;
    }
    *hash = ndsRendererPreparedStage0HashPointer(*hash, dl);
    *hash2 = ndsRendererPreparedStage0Hash2Pointer(*hash2, dl);
    return TRUE;
}

static u32 ndsRendererPreparedStage0HashSelection(
    void *const *collected_dobjs, u32 collected_count, u32 callback_kind,
    u32 *hash, u32 *hash2, DObj **binding_dobjs,
    const Gfx **binding_roots)
{
    u32 list_count = 0u;
    u32 i;
    ...
    {
        DObj *dobj = collected_dobjs[i];
        DObjDLLink *dl_link;
        u32 link;

        *hash = ndsRendererPreparedStage0HashU32(*hash, i);
        *hash2 = ndsRendererPreparedStage0Hash2U32(*hash2, i);
        if (dobj == NULL)
        {
            return 0u;
        }
        /* The first proof has no dynamic MObj material segment.  Reject the
         * whole owner instead of compiling a topology different from the
         * generic submission set. */
        if (dobj->mobj != NULL)
        {
            return 0u;
        }
        if ((dobj->dv == NULL) ||
            ((dobj->flags & DOBJ_FLAG_HIDDEN) != 0))
        {
            continue;
        }
        if (((callback_kind ==
        ...
        case NDS_OPENING_ROOM_DRAW_CALLBACK_DOBJ_DLHEAD1:
            if ((list_count >=
                 NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT) ||
                (ndsRendererPreparedStage0RootAccepted(
                    dobj->dl, hash, hash2) == FALSE)
               )
            {
                return 0u;
            }
            if ((binding_dobjs != NULL) && (binding_roots != NULL))
            {
                binding_dobjs[list_count] = dobj;
                binding_roots[list_count] = dobj->dl;
            }
            list_count++;
            break;
        ...
        case NDS_OPENING_ROOM_DRAW_CALLBACK_DOBJ_DLLINKS:
            ...
            for (link = 0u; link < GC_COMMON_MAX_DLLINKS;
                 link++, dl_link++)
            {
                if (dl_link->list_id ==
                    (s32)NDS_RENDERER_STAGE_DL_HEADS)
                {
                    break;
                }
                    ...
                    (dl_link->dl != NULL))
                {
                    if ((list_count >=
                         NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT) ||
                        (ndsRendererPreparedStage0RootAccepted(
                             dl_link->dl, hash, hash2) == FALSE))
                    {
                        return 0u;
                    }
                    if ((binding_dobjs != NULL) &&
                        (binding_roots != NULL))
                    {
                        binding_dobjs[list_count] = dobj;
                        binding_roots[list_count] = dl_link->dl;
                    }
                    list_count++;
                }
            }
...
#if NDS_RENDERER_PROFILE_LEVEL >= 2
static void ndsRendererPreparedStage0Publish(void)
{
    const NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;

    gNdsPreparedStage0CompileAttemptCount = compiler->attempted;
    gNdsPreparedStage0CompileSuccessCount = compiler->valid;
    gNdsPreparedStage0FallbackReason = compiler->fallback_reason;
    gNdsPreparedStage0ArenaCapacity =
        NDS_RENDERER_PREPARED_STAGE0_ARENA_BYTES;
    gNdsPreparedStage0ProgramBytes = compiler->valid ?
        (u32)sizeof(NDSRendererPreparedStage0Image) : 0u;
    gNdsPreparedStage0KeyHash = compiler->key_hash;
    gNdsPreparedStage0KeyHash2 = compiler->key_hash2;
    gNdsPreparedStage0DObjCount = compiler->dobj_count;
    gNdsPreparedStage0ListCount = compiler->list_count;
    gNdsPreparedStage0SourceCommandCount = compiler->source_command_count;
    /* Twenty list descriptors bind live DObj matrices; owner begin/end bind
     * live event state.  Count those coarse operations with the 69 renderer
     * ops even though the adapter performs them outside the compact op pool. */
    gNdsPreparedStage0OperationCount = compiler->valid ?
        NDS_RENDERER_PREPARED_STAGE0_LOGICAL_OPERATION_COUNT :
        compiler->operation_count;
    gNdsPreparedStage0ApplyOpCount = compiler->apply_op_count;
    gNdsPreparedStage0StateGroupCount = compiler->state_group_count;
    gNdsPreparedStage0StateActionCount = compiler->state_action_count;
    gNdsPreparedStage0VertexBlockCount = compiler->vertex_block_count;
    gNdsPreparedStage0SourceVertexCount = compiler->source_vertex_count;
    gNdsPreparedStage0DrawRunCount = compiler->draw_run_count;
    gNdsPreparedStage0TriangleCommandCount =
        compiler->triangle_command_count;
    gNdsPreparedStage0TriangleCount = compiler->triangle_count;
    gNdsPreparedStage0ImmutableListCount = compiler->immutable_list_count;
    gNdsPreparedStage0TrustedCommandCount = compiler->trusted_command_count;
    gNdsPreparedStage0ValidatedCommandCount =
        compiler->validated_command_count;
    gNdsPreparedStage0BranchCount = compiler->branch_count;
    gNdsPreparedStage0BranchCallCount = compiler->branch_call_count;
    gNdsPreparedStage0BranchJumpCount = compiler->branch_jump_count;
}
#else
#define ndsRendererPreparedStage0Publish() ((void)0)
#endif

static void ndsRendererPreparedStage0RecordExecutionFailure(
    u32 failure, u32 blocker)
{
    if (gNdsPreparedStage0ExecutionFailureCount == 0u)
    {
        gNdsPreparedStage0ExecutionFailureCount = 1u;
        gNdsPreparedStage0ExecutionLastFailure = failure;
        gNdsPreparedStage0ExecutionLastBlocker = blocker;
    }
}

static s32 ndsRendererPreparedStage0TryArm(
    DObj *const *binding_dobjs, const Gfx *const *binding_roots)
{
    NDSRendererConfig config;
    u32 i;

    if ((binding_dobjs == NULL) || (binding_roots == NULL) ||
        (sNdsRendererPreparedStage0Compiler.valid == 0u) ||
        (gNdsPreparedStage0ExecutionFailureCount != 0u))
    {
        return FALSE;
    }

    gNdsPreparedStage0PreflightAttemptCount++;
    ...
    for (i = 0u; i < NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT; i++)
    {
        const NDSRendererPreparedList *list =
            &sNdsRendererPreparedStage0Arena.image.lists[i];
        u32 blocker;

        if ((binding_dobjs[i] == NULL) || (binding_roots[i] == NULL) ||
            (list->storage_base != &sNdsRendererPreparedStage0Arena) ||
            (list->storage_bytes !=
             (u16)sizeof(NDSRendererPreparedStage0Image)))
        {
            ndsRendererPreparedStage0RecordExecutionFailure(
                NDS_RENDERER_PREPARED_STAGE0_EXEC_FAILURE_PREFLIGHT,
                NDS_RENDERER_BLOCKER_BAD_BRANCH);
            return FALSE;
        }
        blocker = ndsRendererPreparedListPreflight(list, &config, NULL);
    ...
    bzero(&sNdsRendererPreparedStage0Replay,
          sizeof(sNdsRendererPreparedStage0Replay));
    for (i = 0u; i < NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT; i++)
    {
        sNdsRendererPreparedStage0Replay.dobjs[i] = binding_dobjs[i];
        sNdsRendererPreparedStage0Replay.roots[i] = binding_roots[i];
    }
    sNdsRendererPreparedStage0Replay.armed = TRUE;
    gNdsPreparedStage0PreflightSuccessCount++;
    gNdsPreparedStage0ArmedOwnerCount++;
    return TRUE;
}
...
{
    NDSRendererPreparedStage0Compiler next;
    NDSRelocLoadedFile *textures;
    NDSRelocLoadedFile *topology;
    u32 hash = 0u;
    u32 hash2 = 0u;
    u32 list_count;
    DObj *binding_dobjs[NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT];
    const Gfx *binding_roots[NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT];

    (void)stage_gobj;
    (void)camera_gobj;
    if ((sNdsRendererAdapterStagePersistentActive == FALSE) ||
        (is_layer == FALSE) || (layer_mask != 1u) ||
        (link_id != NDS_RENDERER_PREPARED_STAGE0_LINK_ID) ||
        (callback_kind !=
         NDS_OPENING_ROOM_DRAW_CALLBACK_DOBJ_TREE) ||
        (collected_dobjs == NULL) ||
        (collected_count != NDS_RENDERER_PREPARED_STAGE0_DOBJ_COUNT))
    {
        return;
    }

    textures = ndsRelocFindLoadedFileByAsset(
        NDS_RELOC_ASSET_EXTERN_DATA_BANK_103);
    topology = ndsRelocFindLoadedFileByAsset(
        NDS_RELOC_ASSET_EXTERN_DATA_BANK_104);
    if ((textures == NULL) || (topology == NULL) ||
        (textures->data == NULL) || (topology->data == NULL) ||
        (textures->data_size == 0u) || (topology->data_size == 0u))
    {
        return;
    }

    hash = ndsRendererPreparedStage0HashU32(
        hash, NDS_RENDERER_PREPARED_FORMAT_VERSION);
    hash2 = ndsRendererPreparedStage0Hash2U32(
        hash2, NDS_RENDERER_PREPARED_FORMAT_VERSION);
    hash = ndsRendererPreparedStage0HashU32(
        hash, NDS_RENDERER_TEXTURE_DATA_O2R_WORD_SWAPPED);
    hash2 = ndsRendererPreparedStage0Hash2U32(
        hash2, NDS_RENDERER_TEXTURE_DATA_O2R_WORD_SWAPPED);
    hash = ndsRendererPreparedStage0HashU32(hash, callback_kind);
    hash2 = ndsRendererPreparedStage0Hash2U32(hash2, callback_kind);
    hash = ndsRendererPreparedStage0HashU32(hash, initial_geometry_mode);
    hash2 = ndsRendererPreparedStage0Hash2U32(
        hash2, initial_geometry_mode);

    /* Loaded-file generations are the immutable-content authority.  Hashing
     * both full asset payloads on every candidate frame would itself cost
     * more than the scan this program is intended to remove; the compiler
     * hashes every executed source command and normalized operand once below. */
#define NDS_RENDERER_PREPARED_HASH_ASSET(file) \
    do { \
        hash = ndsRendererPreparedStage0HashU32(hash, (file)->asset_id); \
        hash = ndsRendererPreparedStage0HashU32( \
            hash, (file)->owner_generation); \
        hash = ndsRendererPreparedStage0HashU32(hash, (file)->data_size); \
        hash2 = ndsRendererPreparedStage0Hash2U32( \
            hash2, (file)->asset_id); \
        hash2 = ndsRendererPreparedStage0Hash2U32( \
            hash2, (file)->owner_generation); \
        hash2 = ndsRendererPreparedStage0Hash2U32( \
            hash2, (file)->data_size); \
    } while (0)

    NDS_RENDERER_PREPARED_HASH_ASSET(textures);
    NDS_RENDERER_PREPARED_HASH_ASSET(topology);
#undef NDS_RENDERER_PREPARED_HASH_ASSET

    list_count = ndsRendererPreparedStage0HashSelection(
        collected_dobjs, collected_count, callback_kind, &hash, &hash2,
        binding_dobjs, binding_roots);
    if (list_count != NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT)
    {
        return;
    }
    if ((sNdsRendererPreparedStage0Compiler.attempted != 0u) &&
        (sNdsRendererPreparedStage0Compiler.base_key_hash == hash) &&
        (sNdsRendererPreparedStage0Compiler.base_key_hash2 == hash2))
    {
        if ((sNdsRendererPreparedStage0Compiler.valid != 0u) &&
            (gNdsPreparedStage0ExecutionMode ==
             NDS_RENDERER_PREPARED_STAGE0_MODE_PREPARED))
        {
            (void)ndsRendererPreparedStage0TryArm(
                binding_dobjs, binding_roots);
        }
        return;
    }
    ...
    next.active = TRUE;
    next.attempted = 1u;
    next.base_key_hash = hash;
    next.base_key_hash2 = hash2;
    next.key_hash = hash;
    next.key_hash2 = hash2;
    next.selected_hash = hash;
    next.dobj_count = collected_count;
    next.root_branch_index = NDS_RENDERER_PREPARED_ROOT_BRANCH;
    bzero(&sNdsRendererPreparedStage0Arena,
          sizeof(sNdsRendererPreparedStage0Arena));
    sNdsRendererPreparedStage0Compiler = next;
    ndsRendererPreparedStage0Publish();
}

static const NDSRendererPreparedList *
ndsRendererPreparedStage0AcquireList(DObj *dobj, const Gfx *dl,
                                      sb32 *claimed)
{
    NDSRendererPreparedStage0Replay *replay =
        &sNdsRendererPreparedStage0Replay;
    u32 list_index;
    ...
    if ((replay->armed == 0u) && (replay->failed == 0u))
    {
        return NULL;
    }
    if (claimed != NULL)
    ...
    list_index = replay->next_list;
    if ((list_index >= NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT) ||
        (replay->dobjs[list_index] != dobj) ||
        (replay->roots[list_index] != dl))
    {
        replay->failed = TRUE;
        ndsRendererPreparedStage0RecordExecutionFailure(
            NDS_RENDERER_PREPARED_STAGE0_EXEC_FAILURE_BINDING,
            NDS_RENDERER_BLOCKER_BAD_BRANCH);
        return NULL;
    }
    replay->next_list++;
    return &sNdsRendererPreparedStage0Arena.image.lists[list_index];
}

static void ndsRendererPreparedStage0FinishReplay(void)
{
    NDSRendererPreparedStage0Replay *replay =
        &sNdsRendererPreparedStage0Replay;
    ...
    if (replay->armed != 0u)
    {
        if ((replay->failed == 0u) &&
            (replay->next_list ==
             NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT))
        {
            gNdsPreparedStage0CompletedOwnerCount++;
        }
        else if (replay->failed == 0u)
        {
            ndsRendererPreparedStage0RecordExecutionFailure(
                NDS_RENDERER_PREPARED_STAGE0_EXEC_FAILURE_INCOMPLETE,
...
static NDSRendererPreparedList *ndsRendererPreparedStage0CurrentList(void)
{
    NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;

    if ((compiler->current_list_active == 0u) ||
        (compiler->current_list >= NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT))
    {
        return NULL;
    }
    return &sNdsRendererPreparedStage0Arena.image.lists[
        compiler->current_list];
}

static NDSRendererPreparedOp *ndsRendererPreparedStage0AppendOperation(
    u32 kind, u32 payload_index, u32 count, u32 source_command_index,
    u32 vertex_start)
{
    NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;
    NDSRendererPreparedOp *operation;

    if ((compiler->active == 0u) ||
        (compiler->operation_count >=
         NDS_RENDERER_PREPARED_STAGE0_OPERATION_CAPACITY) ||
        (payload_index > 0xffffu) || (count > 0xffffu) ||
        (source_command_index > 0xffffu) || (vertex_start > 0xffu))
    {
        ndsRendererPreparedStage0Fail(
            NDS_RENDERER_PREPARED_STAGE0_FALLBACK_ARENA);
        return NULL;
    }
    operation = &sNdsRendererPreparedStage0Arena.image.operations[
        compiler->operation_count++];
    operation->kind = (u8)kind;
    operation->payload_index = (u16)payload_index;
    operation->count = (u16)count;
    operation->source_command_index = (u16)source_command_index;
    operation->vertex_start = (u8)vertex_start;
    return operation;
}

static void ndsRendererPreparedStage0CloseDrawRun(void)
{
    NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;

    if ((compiler->active == 0u) ||
        (compiler->draw_run_count_current == 0u))
    {
        return;
    }
    if (ndsRendererPreparedStage0AppendOperation(
            NDS_RENDERER_PREPARED_OP_DRAW_TRIANGLE_RUN,
            compiler->draw_run_start, compiler->draw_run_count_current,
            compiler->draw_run_source_index, 0u) != NULL)
    {
        compiler->draw_run_count++;
    }
    compiler->draw_run_count_current = 0u;
}

static u32 ndsRendererPreparedStage0StateHash(const Gfx *commands,
                                               u32 count)
{
    return ndsRendererPreparedStage0HashBytes(
        0u, commands, count * sizeof(*commands));
}

static void ndsRendererPreparedStage0FlushState(void)
{
    NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;
    NDSRendererPreparedStage0Image *image =
        &sNdsRendererPreparedStage0Arena.image;
    NDSRendererPreparedStage0StateGroup *group;

    if ((compiler->active == 0u) || (compiler->pending_state_count == 0u))
    {
        return;
    }
    /* Keep one immutable payload per coarse APPLY operation.  Two source
     * groups currently contain the same command bytes, but retaining their
     * separate payloads makes list ownership/provenance explicit and avoids
     * coupling this first execution proof to cross-list aliasing. */
    if ((compiler->state_group_count >=
         NDS_RENDERER_PREPARED_STAGE0_STATE_GROUP_COUNT) ||
        ((compiler->state_action_count +
          compiler->pending_state_count) >
         NDS_RENDERER_PREPARED_STAGE0_STATE_ACTION_COUNT))
    {
        ndsRendererPreparedStage0Fail(
            NDS_RENDERER_PREPARED_STAGE0_FALLBACK_ARENA);
    ...
    group = &image->state_groups[compiler->state_group_count++];
    group->payload_index = (u16)compiler->state_action_count;
    group->count = (u16)compiler->pending_state_count;
    group->hash = ndsRendererPreparedStage0StateHash(
        &image->state_commands[compiler->pending_state_start],
        compiler->pending_state_count);
    compiler->state_action_count += compiler->pending_state_count;
    if (ndsRendererPreparedStage0AppendOperation(
            NDS_RENDERER_PREPARED_OP_APPLY_STATE_GROUP,
            group->payload_index, group->count,
            compiler->pending_source_index, 0u) != NULL)
    {
        compiler->apply_op_count++;
    }
    compiler->pending_state_start = compiler->state_action_count;
    compiler->pending_state_count = 0u;
}

static void ndsRendererPreparedStage0AppendState(
    const NDSRendererCommand *command)
{
    NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;
    NDSRendererPreparedStage0Image *image =
    ...
    ndsRendererPreparedStage0CloseDrawRun();
    if (compiler->active == 0u)
    {
        return;
    }
    if (compiler->pending_state_count == 0u)
    {
        compiler->pending_state_start = compiler->state_action_count;
        compiler->pending_source_index = command->list_index;
    }
    index = compiler->pending_state_start + compiler->pending_state_count;
    if (index >= (NDS_RENDERER_PREPARED_STAGE0_STATE_ACTION_COUNT +
                  NDS_RENDERER_PREPARED_STAGE0_STATE_SCRATCH_COUNT))
    {
        ndsRendererPreparedStage0Fail(
            NDS_RENDERER_PREPARED_STAGE0_FALLBACK_ARENA);
    ...
    image->state_commands[index].words.w0 = command->w0;
    image->state_commands[index].words.w1 = command->w1;
    compiler->pending_state_count++;
    if (compiler->pending_state_count >
        NDS_RENDERER_PREPARED_STAGE0_STATE_SCRATCH_COUNT)
    {
        ndsRendererPreparedStage0Fail(
            NDS_RENDERER_PREPARED_STAGE0_FALLBACK_ARENA);
...
static void ndsRendererPreparedStage0AppendVertex(
    const NDSRendererCommand *command, NDSFighterDLDrawState *draw_state)
{
    NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;
    NDSRendererPreparedStage0Image *image =
    ...
    ndsRendererPreparedStage0FlushState();
    ndsRendererPreparedStage0CloseDrawRun();
    if (compiler->active == 0u)
    {
        return;
    }
    if ((ndsGBIDecodeF3DEX2Vtx(command->w0,
                              NDS_RENDERER_VERTEX_CACHE_SIZE,
                              &v0, &count) == FALSE) ||
        ((compiler->source_vertex_count + count) >
         NDS_RENDERER_PREPARED_STAGE0_VERTEX_COUNT))
    {
        ndsRendererPreparedStage0Fail(
            NDS_RENDERER_PREPARED_STAGE0_FALLBACK_ARENA);
        return;
    }
    source = ndsFighterDLDrawResolveRendererData(
        (const void *)(uintptr_t)command->w1,
        (size_t)count * 16u, draw_state);
    if (source == NULL)
    {
        ndsRendererPreparedStage0Fail(
            NDS_RENDERER_PREPARED_STAGE0_FALLBACK_RESOLVE);
        return;
    }
    ndsRendererDecodePreparedVertexBlock(
        &image->vertices[compiler->source_vertex_count], source, count);
    if (ndsRendererPreparedStage0AppendOperation(
            NDS_RENDERER_PREPARED_OP_LOAD_PREDECODED_VTX_BLOCK,
            compiler->source_vertex_count, count, command->list_index,
            v0) != NULL)
    {
        compiler->vertex_block_count++;
        compiler->source_vertex_count += count;
    }
}

static void ndsRendererPreparedStage0AppendTriangle(
    const NDSRendererCommand *command)
{
    NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;
    NDSRendererPreparedTriangleCommand *triangle;

    ndsRendererPreparedStage0FlushState();
    if ((compiler->active == 0u) ||
        (compiler->triangle_command_count >=
         NDS_RENDERER_PREPARED_STAGE0_TRI_COMMAND_COUNT))
    {
        if (compiler->active != 0u)
        {
            ndsRendererPreparedStage0Fail(
    ...
    if (compiler->draw_run_count_current == 0u)
    {
        compiler->draw_run_start = compiler->triangle_command_count;
        compiler->draw_run_source_index = command->list_index;
    }
    triangle = &sNdsRendererPreparedStage0Arena.image.triangle_commands[
        compiler->triangle_command_count++];
    triangle->command.words.w0 = command->w0;
    triangle->command.words.w1 = command->w1;
    triangle->source_command_index = (u16)command->list_index;
    if (command->depth == 0u)
    {
        triangle->branch_command_index =
            NDS_RENDERER_PREPARED_ROOT_BRANCH;
        triangle->branch_depth = 0u;
        triangle->branch_is_jump = 0u;
    }
    else
    {
        triangle->branch_command_index = (u16)compiler->root_branch_index;
        triangle->branch_depth = (u8)command->depth;
        triangle->branch_is_jump = (u8)compiler->root_branch_is_jump;
    }
    triangle->reserved[0] = 0u;
    triangle->reserved[1] = 0u;
    compiler->draw_run_count_current++;
    compiler->triangle_count +=
        (command->op == NDS_FIGHTER_DL_OP_TRI2) ? 2u : 1u;
}

static void ndsRendererPreparedStage0Omit(sb32 counts_as_skip)
{
    NDSRendererPreparedList *list;

    /* Sync/control/branch/end commands close a source TRI run, but they do
     * not mutate any state retained by this prepared vocabulary.  Keep
     * adjacent supported state actions pending across those omitted commands
     * so execution preserves order in one coarse group.  VTX/TRI and list
     * finalization still flush the group at the exact semantic boundary. */
    ndsRendererPreparedStage0CloseDrawRun();
    list = ndsRendererPreparedStage0CurrentList();
    if (list != NULL)
    {
        list->omitted_command_count++;
        if (counts_as_skip != FALSE)
        {
            list->skip_command_count++;
        }
    }
}

static s32 ndsRendererPreparedStage0CommandUsesPointer(u32 op)
{
    return ((op == NDS_FIGHTER_DL_OP_VTX) ||
            (op == NDS_FIGHTER_DL_OP_DL) ||
            (op == NDS_FIGHTER_DL_OP_SETTIMG)) ? TRUE : FALSE;
}

static void ndsRendererPreparedStage0HashCommand(
    const NDSRendererCommand *command)
{
    NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;

#define NDS_RENDERER_PREPARED_HASH_COMMAND_FIELD(value) \
    do { \
        compiler->key_hash = ndsRendererPreparedStage0HashU32( \
            compiler->key_hash, (u32)(value)); \
        compiler->key_hash2 = ndsRendererPreparedStage0Hash2U32( \
            compiler->key_hash2, (u32)(value)); \
    } while (0)

    NDS_RENDERER_PREPARED_HASH_COMMAND_FIELD(command->depth);
    NDS_RENDERER_PREPARED_HASH_COMMAND_FIELD(command->list_index);
    NDS_RENDERER_PREPARED_HASH_COMMAND_FIELD(command->w0);
    if (ndsRendererPreparedStage0CommandUsesPointer(command->op) != FALSE)
    {
        compiler->key_hash = ndsRendererPreparedStage0HashPointer(
            compiler->key_hash, (const void *)(uintptr_t)command->w1);
        compiler->key_hash2 = ndsRendererPreparedStage0Hash2Pointer(
            compiler->key_hash2, (const void *)(uintptr_t)command->w1);
    }
    else
    {
        NDS_RENDERER_PREPARED_HASH_COMMAND_FIELD(command->w1);
    }
    if (command->op == NDS_FIGHTER_DL_OP_DL)
    {
        NDS_RENDERER_PREPARED_HASH_COMMAND_FIELD(
            command->branch_resolve_kind);
        NDS_RENDERER_PREPARED_HASH_COMMAND_FIELD(command->branch_is_jump);
        compiler->key_hash = ndsRendererPreparedStage0HashPointer(
            compiler->key_hash, command->resolved_branch_dl);
        compiler->key_hash2 = ndsRendererPreparedStage0Hash2Pointer(
            compiler->key_hash2, command->resolved_branch_dl);
    }
#undef NDS_RENDERER_PREPARED_HASH_COMMAND_FIELD
}

static void ndsRendererPreparedStage0BeginList(const Gfx *dl)
{
    NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;
    NDSRendererPreparedList *list;
    ...
    if ((loaded == NULL) ||
        (loaded->asset_id != NDS_RELOC_ASSET_EXTERN_DATA_BANK_104) ||
        (ndsRendererAdapterImmutableCommandSpan(dl, NULL) == 0u) ||
        (compiler->current_list_active != 0u) ||
        (compiler->list_count >=
         NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT))
    {
        ndsRendererPreparedStage0Fail(
            NDS_RENDERER_PREPARED_STAGE0_FALLBACK_IMMUTABLE);
        return;
    }
    compiler->current_list = compiler->list_count++;
    compiler->current_list_active = TRUE;
    compiler->current_operation_start = compiler->operation_count;
    compiler->pending_state_start = compiler->state_action_count;
    compiler->pending_state_count = 0u;
    compiler->draw_run_count_current = 0u;
    compiler->root_branch_index = NDS_RENDERER_PREPARED_ROOT_BRANCH;
    compiler->root_branch_is_jump = 0u;

    list = ndsRendererPreparedStage0CurrentList();
    bzero(list, sizeof(*list));
    list->format_version = NDS_RENDERER_PREPARED_FORMAT_VERSION;
    list->operations =
        &sNdsRendererPreparedStage0Arena.image.operations[
            compiler->current_operation_start];
    list->state_commands =
        sNdsRendererPreparedStage0Arena.image.state_commands;
    list->vertices = sNdsRendererPreparedStage0Arena.image.vertices;
    list->triangle_commands =
        sNdsRendererPreparedStage0Arena.image.triangle_commands;
    list->storage_base = &sNdsRendererPreparedStage0Arena;
}

static void ndsRendererPreparedStage0EndList(u32 generic_blocker)
{
    NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;
    NDSRendererPreparedList *list =
        ndsRendererPreparedStage0CurrentList();

    if (list == NULL)
    {
        return;
    }
    if ((generic_blocker != NDS_RENDERER_BLOCKER_NONE) &&
        (compiler->failed == 0u))
    {
        ndsRendererPreparedStage0Fail(
            NDS_RENDERER_PREPARED_STAGE0_FALLBACK_GENERIC);
    }
    if (compiler->failed == 0u)
    {
        ndsRendererPreparedStage0FlushState();
        ndsRendererPreparedStage0CloseDrawRun();
        list->operation_count = (u16)(compiler->operation_count -
                                      compiler->current_operation_start);
    }
    compiler->current_list_active = FALSE;
}

static void ndsRendererPreparedStage0Observe(
    const NDSRendererCommand *command, NDSFighterDLDrawState *draw_state)
{
    NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;
    NDSRendererPreparedList *list =
        ndsRendererPreparedStage0CurrentList();
    size_t immutable_span;

    if ((compiler->active == 0u) || (command == NULL) || (list == NULL))
    {
        return;
    }
    if (command->depth > 1u)
    {
        ndsRendererPreparedStage0Fail(
            NDS_RENDERER_PREPARED_STAGE0_FALLBACK_DEPTH);
        return;
    }
    immutable_span = ndsRendererAdapterImmutableCommandSpan(
        command->dl, draw_state);
    if (immutable_span == 0u)
    {
        ndsRendererPreparedStage0Fail(
            NDS_RENDERER_PREPARED_STAGE0_FALLBACK_IMMUTABLE);
        return;
    }
    if (command->list_index == 0u)
    {
        compiler->immutable_list_count++;
        compiler->validated_command_count++;
        list->immutable_list_count++;
        list->validated_command_count++;
    }
    compiler->trusted_command_count++;
    list->trusted_command_count++;
    if (list->source_command_count == 0u)
    {
        list->first_opcode = (u8)command->op;
    }
    list->source_command_count++;
    compiler->source_command_count++;
    if (command->depth > list->max_depth_seen)
    {
        list->max_depth_seen = (u16)command->depth;
    }
    if ((command->list_index + 1u) > list->max_source_list_commands)
    {
        list->max_source_list_commands =
            (u16)(command->list_index + 1u);
    }
    ndsRendererPreparedStage0HashCommand(command);

    if (ndsRendererPreparedStateOpcodeSupported(command->op) != FALSE)
    {
        ndsRendererPreparedStage0AppendState(command);
        return;
    }
    switch (command->op)
    {
    case NDS_FIGHTER_DL_OP_VTX:
        ndsRendererPreparedStage0AppendVertex(command, draw_state);
        break;

    case NDS_FIGHTER_DL_OP_TRI1:
    case NDS_FIGHTER_DL_OP_TRI2:
        ndsRendererPreparedStage0AppendTriangle(command);
        break;

    case NDS_FIGHTER_DL_OP_DL:
        if ((command->depth != 0u) ||
            (command->resolved_branch_dl == NULL) ||
            (ndsRendererAdapterImmutableCommandSpan(
                 command->resolved_branch_dl, draw_state) == 0u))
        {
            ndsRendererPreparedStage0Fail(
                NDS_RENDERER_PREPARED_STAGE0_FALLBACK_IMMUTABLE);
            break;
        }
        compiler->root_branch_index = command->list_index;
        compiler->root_branch_is_jump = command->branch_is_jump;
        compiler->branch_count++;
        list->branch_command_count++;
        if (command->branch_is_jump != 0u)
        {
            compiler->branch_jump_count++;
            list->branch_jump_count++;
        }
        else
        {
            compiler->branch_call_count++;
            list->branch_call_count++;
        }
        if (command->branch_resolve_kind == NDS_RENDERER_RESOLVE_SEGMENT)
        {
            list->segment_resolve_count++;
        }
        if (list->first_branch_dl == NULL)
        {
            list->first_branch_dl = command->raw_branch_dl;
            list->first_resolved_branch_dl = command->resolved_branch_dl;
        }
        /* Generic traversal validates the resolved branch immediately and
         * then validates it again on recursive list entry.  The child entry
         * itself was counted above at list_index zero. */
        compiler->validated_command_count++;
        list->validated_command_count++;
        ndsRendererPreparedStage0Omit(FALSE);
        break;

    case NDS_FIGHTER_DL_OP_ENDDL:
        list->end_command_count++;
        ndsRendererPreparedStage0Omit(TRUE);
        break;
    ...
    default:
        ...
        ndsRendererPreparedStage0CloseDrawRun();
        ndsRendererPreparedStage0Fail(
            NDS_RENDERER_PREPARED_STAGE0_FALLBACK_UNSUPPORTED);
        break;
    }
}

static s32 ndsRendererPreparedStage0VisitCommand(
    const NDSRendererCommand *command, void *user)
{
    NDSRendererPreparedStage0CallbackContext *context = user;
    s32 accepted = TRUE;

    if ((context != NULL) && (context->base_callback != NULL))
    {
        accepted = context->base_callback(command, context->draw_state);
    }
    if ((context != NULL) &&
        (sNdsRendererPreparedStage0Compiler.active != 0u))
    {
        ndsRendererPreparedStage0Observe(command, context->draw_state);
    }
    return accepted;
}

static u32 ndsRendererPreparedStage0ProgramBytes(void)
{
    return (u32)sizeof(NDSRendererPreparedStage0Image);
}

static void ndsRendererPreparedStage0Finalize(void)
{
    NDSRendererPreparedStage0Compiler *compiler =
        &sNdsRendererPreparedStage0Compiler;
    NDSRendererPreparedStage0Image *image =
    ...
    if (compiler->attempted == 0u)
    {
        return;
    }
    if (compiler->current_list_active != 0u)
    {
        ndsRendererPreparedStage0EndList(NDS_RENDERER_BLOCKER_NONE);
    }
    if (compiler->failed != 0u)
    {
        ndsRendererPreparedStage0Publish();
    ...
    program_bytes = ndsRendererPreparedStage0ProgramBytes();
    if ((program_bytes > NDS_RENDERER_PREPARED_STAGE0_ARENA_BYTES) ||
        (compiler->dobj_count !=
         NDS_RENDERER_PREPARED_STAGE0_DOBJ_COUNT) ||
        (compiler->list_count !=
         NDS_RENDERER_PREPARED_STAGE0_LIST_COUNT) ||
        (compiler->source_command_count != 297u) ||
        (compiler->operation_count !=
         NDS_RENDERER_PREPARED_STAGE0_EXEC_OPERATION_COUNT) ||
        (compiler->apply_op_count !=
         NDS_RENDERER_PREPARED_STAGE0_APPLY_COUNT) ||
        (compiler->state_group_count !=
         NDS_RENDERER_PREPARED_STAGE0_STATE_GROUP_COUNT) ||
        (compiler->state_action_count !=
         NDS_RENDERER_PREPARED_STAGE0_STATE_ACTION_COUNT) ||
        (compiler->vertex_block_count !=
         NDS_RENDERER_PREPARED_STAGE0_VERTEX_BLOCK_COUNT) ||
        (compiler->source_vertex_count !=
         NDS_RENDERER_PREPARED_STAGE0_VERTEX_COUNT) ||
        (compiler->draw_run_count !=
         NDS_RENDERER_PREPARED_STAGE0_DRAW_RUN_COUNT) ||
        (compiler->triangle_command_count !=
         NDS_RENDERER_PREPARED_STAGE0_TRI_COMMAND_COUNT) ||
        (compiler->triangle_count !=
         NDS_RENDERER_PREPARED_STAGE0_TRIANGLE_COUNT))
    {
        ndsRendererPreparedStage0Fail(
            (program_bytes > NDS_RENDERER_PREPARED_STAGE0_ARENA_BYTES) ?
                NDS_RENDERER_PREPARED_STAGE0_FALLBACK_ARENA :
                NDS_RENDERER_PREPARED_STAGE0_FALLBACK_CENSUS);
        ndsRendererPreparedStage0Publish();
        return;
    }

    for (i = 0u; i < compiler->list_count; i++)
    {
        NDSRendererPreparedList *list = &image->lists[i];
        u32 accounted = list->omitted_command_count;
        u32 operation_index;

        list->state_command_count = (u16)compiler->state_action_count;
        list->vertex_count = (u16)compiler->source_vertex_count;
        list->triangle_command_count =
            (u16)compiler->triangle_command_count;
        list->storage_bytes = (u16)program_bytes;
        for (operation_index = 0u;
             operation_index < list->operation_count;
             operation_index++)
        {
            const NDSRendererPreparedOp *operation =
                &list->operations[operation_index];
        ...
        if (accounted != list->source_command_count)
        {
            ndsRendererPreparedStage0Fail(
                NDS_RENDERER_PREPARED_STAGE0_FALLBACK_CENSUS);
            ndsRendererPreparedStage0Publish();
    ...
    compiler->key_hash = ndsRendererPreparedStage0HashU32(
        compiler->key_hash, compiler->source_command_count);
    compiler->key_hash = ndsRendererPreparedStage0HashU32(
        compiler->key_hash, compiler->operation_count);
    compiler->key_hash2 = ndsRendererPreparedStage0Hash2U32(
        compiler->key_hash2, compiler->source_command_count);
    compiler->key_hash2 = ndsRendererPreparedStage0Hash2U32(
        compiler->key_hash2, compiler->operation_count);
    compiler->active = FALSE;
    compiler->valid = TRUE;
    compiler->fallback_reason = NDS_RENDERER_PREPARED_STAGE0_FALLBACK_NONE;
    ndsRendererPreparedStage0Publish();
}
#if NDS_RENDERER_PROFILE_LEVEL >= 2
...
{
    ndsRendererPreparedStage0FinishReplay();
    ndsRendererPreparedStage0Finalize();
#if NDS_RENDERER_PROFILE_LEVEL >= 2
    gNdsRendererProfileOwners[
                                            ...
                                            u32 initial_geometry_mode)
{
    ...
    void *callback_user;
    NDSRendererPreparedStage0CallbackContext prepared_callback;
    sb32 prepared_observing = FALSE;
    const NDSRendererPreparedList *prepared_list;
    sb32 prepared_claimed;
    ...
    prepared_list = ndsRendererPreparedStage0AcquireList(
        dobj, dl, &prepared_claimed);
    if ((prepared_claimed != FALSE) && (prepared_list == NULL))
    {
        /* Once a whole owner is armed, never mix a partial prepared prefix
         * with generic source replay.  The prior all-list preflight and live
         * binding table make this an invariant failure, surfaced below. */
        return;
    }
    ...
    callback_user = &state;
    if (sNdsRendererPreparedStage0Compiler.active != 0u)
    {
        ndsRendererPreparedStage0BeginList(dl);
        if (sNdsRendererPreparedStage0Compiler.current_list_active != 0u)
        {
            prepared_callback.draw_state = &state;
            prepared_callback.base_callback = callback;
            callback = ndsRendererPreparedStage0VisitCommand;
            callback_user = &prepared_callback;
            prepared_observing = TRUE;
        }
    }
...
#endif
#endif
    if (prepared_list != NULL)
    {
        ndsRendererExecutePreparedListWithVertexCache(
            prepared_list, &config, render_stats,
            &sNdsRendererAdapterStageVertexCache);
        if (render_stats->blocker == NDS_RENDERER_BLOCKER_NONE)
        {
            gNdsPreparedStage0ExecutedListCount++;
        }
        else
        {
            sNdsRendererPreparedStage0Replay.failed = TRUE;
            ndsRendererPreparedStage0RecordExecutionFailure(
                NDS_RENDERER_PREPARED_STAGE0_EXEC_FAILURE_RENDERER,
                render_stats->blocker);
        }
    }
    else
    {
    ...
    if (prepared_observing != FALSE)
    {
        ndsRendererPreparedStage0EndList(render_stats->blocker);
    }
#if NDS_RENDERER_HW_TRIANGLES
#if NDS_RENDERER_PROFILE_LEVEL >= 2
