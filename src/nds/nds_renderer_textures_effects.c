 ...
 *
 * The N64 clamps the shade sum to eight bits and modulates by the primitive
 * colour AFTERWARDS: pixel = clamp8(light2 + light1 * dot) * prim / 255. The DS
 * geometry engine has no inner clamp -- it saturates once, per channel, at 31,
 * after the material fold -- so folding prim into diffuse and ambient cannot
 * reproduce that shape. Fold it and the lit half washes toward white (Purin's
 * pink body measured as pure (31,31,31) at full light); cap the diffuse so it
 * cannot wash, and the ramp reaches prim only at dot = 1 while the source has
 * been sitting on prim since dot = 0.498, which draws the body uniformly dimmer
 * than the face. Those are the owner's two reports of this row, r35 and r36.
 *
 * There is exactly one post-shade multiply on this hardware, and the FACE runs
 * already use it: they are textured, so the DS modulates the shaded vertex by
 * the texel and they inherit the source's clamp order for free. Give the body
 * the same thing -- raw light colours in diffuse/ambient, and a texture every
 * texel of which is prim -- and the pipeline evaluates
 * clamp31(RGB5(l2) + RGB5(l1)*dot) * prim, which IS the source's order of
 * operations.
 *
 * Eight 8x8 PAL16 tiles, 32 bytes of texels and a palette each, created on
 * demand: the roster's whole tinted set is Pikachu 0xFFD933, Purin 0xFFCDD8 and
 * Kirby 0x00FF5A / 0x00B300. Index 1, not 0, because
 * PrepareIFCommonPal16Atlas passes color0_transparent.
 *
 * Every texel of the tile is the same colour, so the DS texcoord register being
 * sticky means ONE GFX_TEX_COORD per run serves every vertex of it: the cost is
 * a bind and one FIFO word per run, never a word per vertex. */
#define NDS_R2_FIGHTER_TINT_SLOTS 8u
...
{
    u32 color;      /* the RRGGBBAA material colour this tile was built for */
    u32 name;
    u32 generation; /* names die with glResetTextures */
} NDSR2FighterTint;
...
/* Epochs that wanted the tint route and did not get it, by reason. [0] is a
 * live colour modulate -- the hurt flash is a second post-shade multiply the
 * texel cannot also carry -- and [1] is a full tile table. A non-zero [1] means
 * NDS_R2_FIGHTER_TINT_SLOTS is too small for the roster. */
volatile u32 gNdsR2FighterTintDeclined[2];
...
static s32 ndsR2FighterTintFill(u8 *pixels, u32 bytes, void *user_data)
{
    ...
    /* 0x11: both 4-bit texels of the byte select palette index 1. */
    memset(pixels, 0x11, (size_t)bytes);
    return TRUE;
}

/* The GL name of a tile whose every texel is `material_color`, or 0. */
static u32 ndsRendererR2FighterTintTexture(u32 material_color)
{
    u16 palette[16];
    u32 generation = gNdsRendererSceneTextureVramResetCount + 1u;
    u32 slot = NDS_R2_FIGHTER_TINT_SLOTS;
    u32 i;

    for (i = 0u; i < NDS_R2_FIGHTER_TINT_SLOTS; i++)
    {
        NDSR2FighterTint *entry = &sNdsR2FighterTints[i];
        ...
        if ((entry->name != 0u) && (entry->color == material_color))
        {
            gNdsR2FighterTintHits++;
            return entry->name;
        }
        if ((slot == NDS_R2_FIGHTER_TINT_SLOTS) && (entry->name == 0u))
        {
            slot = i;
        }
    }
    if (slot == NDS_R2_FIGHTER_TINT_SLOTS)
    {
        gNdsR2FighterTintDeclined[1]++;
        return 0u;
    }
    memset(palette, 0, sizeof(palette));
    palette[1] = (u16)RGB15(((material_color >> 24) & 0xffu) >> 3,
                            ((material_color >> 16) & 0xffu) >> 3,
                            ((material_color >> 8) & 0xffu) >> 3);
    if (ndsRendererHardwarePrepareIFCommonPal16Atlas(
            NDS_R2_FIGHTER_TINT_DIM, NDS_R2_FIGHTER_TINT_DIM, palette,
            ndsR2FighterTintFill, NULL,
            &sNdsR2FighterTints[slot].name) == FALSE)
    {
        sNdsR2FighterTints[slot].name = 0u;
        gNdsR2FighterTintFails++;
        return 0u;
    }
    sNdsR2FighterTints[slot].color = material_color;
    sNdsR2FighterTints[slot].generation = generation;
    gNdsR2FighterTintBuilds++;
    return sNdsR2FighterTints[slot].name;
}
#endif
