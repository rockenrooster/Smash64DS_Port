...
ALPHA_THRESHOLD = 32
BINARY = "assets/effects/task39_hit_sparks.rgb5a1.bin"
NITRO_PATH = "nitro:/effects/task39_hit_sparks.rgb5a1.bin"
...
def write_inc(data: bytes, digest: str) -> None:
    """Emit the payload's identity, not the payload.

    The sheet used to be a linked `static const u8 [22528]`, which is 22,528
    bytes of the ARM9 image spent on data that is read exactly once -- one DMA
    into OBJ VRAM at prepare time and never again. Linked .rodata is taken out
    of the boot-time taskman arena one-for-one (src/port/diagnostics.c searches
    down from 0x150000 in 0x1000 steps and floors at 0x130000), and at 22,528
    bytes this single array was worth five whole steps of that search -- which
    is what made NDS_R2_PARTICLE_RUNTIME=1 unbootable: it pushed the search to
    its floor and the first battle allocation died 864 bytes short.

    The bytes ship as the NitroFS payload written below instead, which the
    generator was already producing for other consumers. Nothing about the
    sheet changes; only where it lives.
    """
    ...
    text = (
        ...
        "\n"
        "/* The sheet itself is NOT here -- see write_inc() in the generator.\n"
        " * ndsTask39PrepareHitSparks streams it from this path into OBJ VRAM. */\n"
        "#define NDS_TASK39_HIT_SPARK_ASSET_PATH \\\n"
        f'    "{NITRO_PATH}"\n'
        f"#define NDS_TASK39_HIT_SPARK_ASSET_BYTES {len(data)}u\n"
    )
