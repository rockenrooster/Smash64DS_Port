...
$verifierContext = Initialize-MelonDSVerifierContext `
    ...
    -NoBuild:$NoBuild
$rom = Join-Path $root 'smash64ds-battle-mariofox-dl-draw.nds'
$elf = Join-Path $root 'smash64ds-battle-mariofox-dl-draw.elf'
$melonDsPath = $verifierContext.MelonDSPath
$melonDsDir = Split-Path -Parent $melonDsPath
$logDir = Get-MelonDSVerifierLogDir -Root $root -RunnerSlot (Get-MelonDSActiveRunnerSlot)
$stdout = Join-Path $logDir 'melonds.battle-mariofox-dl-draw-harness.stdout.log'
$stderr = Join-Path $logDir 'melonds.battle-mariofox-dl-draw-harness.stderr.log'
$configState = $null
$emulator = $null
$scriptName = '_battle_mariofox_dl_draw_harness.gdb'
...
& make -C $root TARGET=smash64ds-battle-mariofox-dl-draw BUILD=build-battle-mariofox-dl-draw-harness NDS_DEV_SCENE_HARNESS=battle_mariofox_dl_draw -j16
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
if (-not (Test-Path $rom) -or -not (Test-Path $elf)) {
    throw 'Battle Mario/Fox DL draw harness build did not produce the expected ROM and ELF.'
}
...
try {
    ...
    Assert-Condition ($harn.Success -and (Convert-MarkerUInt32 $harn.Groups[1].Value) -eq 0x4841524e -and [int]$harn.Groups[2].Value -eq 29 -and [int]$harn.Groups[3].Value -eq 22 -and [int]$harn.Groups[4].Value -eq 21 -and (Convert-MarkerUInt32 $harn.Groups[5].Value) -eq 0) 'Battle Mario/Fox DL draw harness did not select direct VSBattle from Maps.' $gdbStdout
    ...
    Assert-Condition ($scan.Success -and (Convert-MarkerUInt32 $scan.Groups[1].Value) -eq 0x46544c50 -and (Convert-MarkerUInt32 $scan.Groups[2].Value) -eq 0x46544c53 -and ((Convert-MarkerUInt32 $scan.Groups[3].Value) -band 0x7ff) -eq 0x7ff -and [int]$scan.Groups[5].Value -eq 2) 'Prerequisite DL scan proof did not pass before draw.' $gdbStdout
    ...
    Assert-Condition ($exec.Success -and (Convert-MarkerUInt32 $exec.Groups[1].Value) -eq 0x46544c45 -and (Convert-MarkerUInt32 $exec.Groups[2].Value) -eq 0x46544c58 -and ((Convert-MarkerUInt32 $exec.Groups[3].Value) -band 0x7ff) -eq 0x7ff -and (Convert-MarkerUInt32 $exec.Groups[4].Value) -eq 0xff -and [int]$exec.Groups[5].Value -eq 2) 'Prerequisite DL execute proof did not pass before draw.' $gdbStdout
    Assert-Condition ($execStats.Success -and [int]$execStats.Groups[1].Value -eq 0 -and [int]$execStats.Groups[2].Value -eq 0 -and [int]$execStats.Groups[3].Value -ge 59 -and [int]$execStats.Groups[4].Value -ge 69 -and (Convert-MarkerUInt32 $execStats.Groups[7].Value) -eq 0 -and (Convert-MarkerUInt32 $execStats.Groups[8].Value) -eq 0 -and [int]$execStats.Groups[9].Value -eq 0 -and [int]$execStats.Groups[10].Value -eq 0) 'Prerequisite DL execute stats were not clean.' $gdbStdout
    Assert-Condition ($execGeom.Success -and [int]$execGeom.Groups[1].Value -gt 0 -and [int]$execGeom.Groups[2].Value -gt 0 -and [int]$execGeom.Groups[7].Value -gt 0 -and [int]$execGeom.Groups[8].Value -gt 0 -and [int]$execGeom.Groups[9].Value -gt 0 -and [int]$execGeom.Groups[10].Value -gt 0) 'Prerequisite DL execute did not decode vertices and triangles.' $gdbStdout
    Assert-Condition ($draw.Success -and (Convert-MarkerUInt32 $draw.Groups[1].Value) -eq 0x46544c44 -and (Convert-MarkerUInt32 $draw.Groups[2].Value) -eq 0x46544c57 -and ((Convert-MarkerUInt32 $draw.Groups[3].Value) -band 0x7ff) -eq 0x7ff -and (Convert-MarkerUInt32 $draw.Groups[4].Value) -eq 0xff -and [int]$draw.Groups[5].Value -eq 2) 'Mario/Fox DL draw proof did not pass.' $gdbStdout
    Assert-Condition ($preview.Success -and [int]$preview.Groups[1].Value -eq 96 -and [int]$preview.Groups[2].Value -eq 72 -and [int]$preview.Groups[3].Value -ge 96 -and [int]$preview.Groups[4].Value -eq 1 -and [int]$preview.Groups[6].Value -gt [int]$preview.Groups[5].Value -and [int]$preview.Groups[7].Value -eq 1) 'DL draw preview was not committed as a 96x72 retained surface.' $gdbStdout
    Assert-Condition ($platform.Success -and [int]$platform.Groups[1].Value -eq 1 -and [int]$platform.Groups[2].Value -eq 96 -and [int]$platform.Groups[3].Value -eq 72 -and [int]$platform.Groups[4].Value -ge [int]$preview.Groups[6].Value) 'Platform original-DL preview state is not ready after draw.' $gdbStdout
    Assert-Condition ($drawStats.Success -and [int]$drawStats.Groups[1].Value -eq 0 -and [int]$drawStats.Groups[2].Value -eq 0 -and [int]$drawStats.Groups[3].Value -ge 59 -and [int]$drawStats.Groups[4].Value -ge 69 -and (Convert-MarkerUInt32 $drawStats.Groups[5].Value) -ne 0 -and (Convert-MarkerUInt32 $drawStats.Groups[6].Value) -ne 0 -and (Convert-MarkerUInt32 $drawStats.Groups[7].Value) -eq 0 -and (Convert-MarkerUInt32 $drawStats.Groups[8].Value) -eq 0 -and [int]$drawStats.Groups[9].Value -eq 0 -and [int]$drawStats.Groups[10].Value -eq 0) 'DL draw stats were not clean.' $gdbStdout
    Assert-Condition ($drawGeom.Success -and [int]$drawGeom.Groups[1].Value -gt 0 -and [int]$drawGeom.Groups[2].Value -gt 0 -and [int]$drawGeom.Groups[3].Value -gt 0 -and [int]$drawGeom.Groups[4].Value -gt 0 -and [int]$drawGeom.Groups[5].Value -gt 0 -and [int]$drawGeom.Groups[6].Value -gt 0 -and [int]$drawGeom.Groups[7].Value -gt 0 -and [int]$drawGeom.Groups[8].Value -gt 0 -and [int]$drawGeom.Groups[9].Value -gt 0 -and [int]$drawGeom.Groups[10].Value -gt 0 -and [int]$drawGeom.Groups[11].Value -gt 0) 'DL draw did not produce visible software pixels for both fighters.' $gdbStdout
    Assert-Condition ($drawRender.Success -and [int]$drawRender.Groups[1].Value -gt 0 -and [int]$drawRender.Groups[2].Value -gt 0) 'DL draw did not produce real non-degenerate triangles for both fighters.' $gdbStdout
    Assert-Condition ($axis.Success -and [int]$axis.Groups[1].Value -le 2 -and [int]$axis.Groups[2].Value -le 2 -and [int]$axis.Groups[3].Value -gt 0 -and [int]$axis.Groups[4].Value -gt 0 -and (([int]$axis.Groups[6].Value -ne [int]$axis.Groups[5].Value) -or ([int]$axis.Groups[8].Value -ne [int]$axis.Groups[7].Value)) -and (([int]$axis.Groups[10].Value -ne [int]$axis.Groups[9].Value) -or ([int]$axis.Groups[12].Value -ne [int]$axis.Groups[11].Value))) 'DL draw projection axes/bounds were not usable.' $gdbStdout
    Assert-Condition ($screen.Success -and [int]$screen.Groups[1].Value -ge 0 -and [int]$screen.Groups[2].Value -le 95 -and [int]$screen.Groups[3].Value -ge 0 -and [int]$screen.Groups[4].Value -le 71 -and [int]$screen.Groups[5].Value -ge 0 -and [int]$screen.Groups[6].Value -le 95 -and [int]$screen.Groups[7].Value -ge 0 -and [int]$screen.Groups[8].Value -le 71 -and [int]$screen.Groups[2].Value -ge [int]$screen.Groups[1].Value -and [int]$screen.Groups[6].Value -ge [int]$screen.Groups[5].Value -and (Convert-MarkerUInt32 $screen.Groups[9].Value) -ne 0 -and (Convert-MarkerUInt32 $screen.Groups[10].Value) -ne 0) 'DL draw screen bounds/checksums were not valid.' $gdbStdout
    Assert-Condition ($state.Success -and [int]$state.Groups[1].Value -eq 10 -and [int]$state.Groups[2].Value -eq 10 -and [int]$state.Groups[3].Value -eq 4 -and [int]$state.Groups[4].Value -eq 4 -and [int]$state.Groups[5].Value -eq 0 -and [int]$state.Groups[6].Value -eq 0 -and (Convert-MarkerUInt32 $state.Groups[7].Value) -eq (Convert-MarkerUInt32 $state.Groups[8].Value) -and (Convert-MarkerUInt32 $state.Groups[9].Value) -eq (Convert-MarkerUInt32 $state.Groups[10].Value)) 'DL draw mutated fighter state or root position.' $gdbStdout
    Assert-Condition ($safe.Success -and [int]$safe.Groups[1].Value -eq 0 -and [int]$safe.Groups[2].Value -eq 0 -and [int]$safe.Groups[3].Value -eq 0 -and [int]$safe.Groups[4].Value -eq 0 -and [int]$safe.Groups[5].Value -eq 0 -and [int]$safe.Groups[6].Value -eq 0) 'DL draw escaped bounded software-preview behavior.' $gdbStdout
    Assert-Condition ($boundary.Success -and (Convert-MarkerUInt32 $boundary.Groups[1].Value) -eq 0x53434e45 -and [int]$boundary.Groups[2].Value -eq 22) 'VSBattle did not park at the bounded scene boundary after draw.' $gdbStdout
    Write-Output ("Battle Mario/Fox DL draw harness passed: scene=22/21 pixels={0}/{1} tris={2}/{3} real={4}/{5} marker={6}/{7} preview=96x72 commit=1 safe=1" -f $drawGeom.Groups[9].Value, $drawGeom.Groups[10].Value, $drawGeom.Groups[3].Value, $drawGeom.Groups[4].Value, $drawRender.Groups[1].Value, $drawRender.Groups[2].Value, $drawRender.Groups[3].Value, $drawRender.Groups[4].Value)
} finally {
    if ($null -ne $emulator) {
        $emulator.Refresh()
