...
$stdout = Join-Path $logDir 'melonds.menu-chain-mariofox-dl-draw-all-harness.stdout.log'
$stderr = Join-Path $logDir 'melonds.menu-chain-mariofox-dl-draw-all-harness.stderr.log'
$configState = $null
$emulator = $null
$scriptName = '_menu_chain_mariofox_dl_draw_all_harness.gdb'
...
try {
    ...
    Assert-Condition ($harn.Success -and (Convert-MarkerUInt32 $harn.Groups[1].Value) -eq 0x4841524e -and [int]$harn.Groups[2].Value -eq 34 -and [int]$harn.Groups[3].Value -eq 9 -and [int]$harn.Groups[4].Value -eq 1 -and (Convert-MarkerUInt32 $harn.Groups[5].Value) -eq 0) 'Menu-chain Mario/Fox all-DL draw harness did not select VS Mode from Title.' $gdbStdout
