...
& make -C $root TARGET=smash64ds-battle-fd BUILD=build-battle-fd-harness NDS_DEV_SCENE_HARNESS=battle_fd -j4
...
try {
    ...
    if (-not $setup.Success -or
        ...
        ((Convert-MarkerUInt32 $setup.Groups[5].Value) -band 0x7f) -ne 0x7f -or
