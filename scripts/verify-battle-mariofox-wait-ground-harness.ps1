...
try {
    ...
    Assert-Condition ($vsb.Success -and (Convert-MarkerUInt32 $vsb.Groups[1].Value) -eq 0x56425355 -and ((Convert-MarkerUInt32 $vsb.Groups[2].Value) -band 0x6f) -eq 0x6f -and (Convert-MarkerUInt32 $vsb.Groups[3].Value) -eq 0x56425550 -and [int]$vsb.Groups[4].Value -eq 2 -and [int]$vsb.Groups[5].Value -eq 2 -and [int]$vsb.Groups[6].Value -eq 6) 'Imported VSBattle setup did not reach the ground proof boundary.' $gdbStdout
