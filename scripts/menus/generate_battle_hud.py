...
def quantize_portrait(ui, raster, width: int, height: int, cell_width=16, cell_height=16):
    ...
    indices = [0] * (cell_width * cell_height)
    for y in range(height):
        for x in range(width):
            r, g, b, a = raster[y][x]
            if a < 32:
            ...
            best = min(range(len(palette_rgb)),
                       key=lambda i: ((r - palette_rgb[i][0]) ** 2 +
                                      (g - palette_rgb[i][1]) ** 2 +
                                      (b - palette_rgb[i][2]) ** 2))
            indices[y * cell_width + x] = best + 1
    return pack_obj4(indices, cell_width, cell_height), palette
...
def bake(repo_root: Path, output: Path, binary_output: Path) -> None:
    ...
    particle_data = particles.load_o2r_blob(repo_root, *particles.TEXTURE_BANK)
    score_texture = particles.parse_texture_bank(particle_data)[32]
    assert (score_texture["width"], score_texture["height"], score_texture["frames"]) == (64, 16, 2)
    score_gfx, score_palettes = [], []
    for frame in range(2):
        pixels = particles.decode_texture_frame(particle_data, score_texture, frame)
        raster = [pixels[y * 64:(y + 1) * 64] for y in range(16)]
        raster, width, height = scale_raster(ui, raster)
        cell = [[(0, 0, 0, 0)] * 64 for _ in range(32)]
        left, top = (64 - width) // 2, (32 - height) // 2
        for y, row in enumerate(raster):
            cell[top + y][left:left + width] = row
    ...
    if not binary_output.exists() or binary_output.read_bytes() != binary:
...
def main() -> int:
    ...
    parser.add_argument("--binary-output", type=Path, default=None)
    args = parser.parse_args()
    root = args.repo_root.resolve()
    output = args.output or (root / "src" / "nds" / "generated" /
                             "battle_hud.generated.inc")
    binary_output = args.binary_output or (
        output.with_suffix(".bin") if args.output else root / "assets/menus/battle_hud.bin")
    try:
        bake(root, output, binary_output)
