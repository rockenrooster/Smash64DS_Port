...
$verifierContext = Initialize-MelonDSVerifierContext `
    ...
    -NoBuild:$NoBuild
$rom = Join-Path $root 'smash64ds-battle-mariofox-walk-loop.nds'
$elf = Join-Path $root 'smash64ds-battle-mariofox-walk-loop.elf'
$melonDsPath = $verifierContext.MelonDSPath
$melonDsDir = Split-Path -Parent $melonDsPath
$logDir = Get-MelonDSVerifierLogDir -Root $root -RunnerSlot (Get-MelonDSActiveRunnerSlot)
$stdout = Join-Path $logDir 'melonds.battle-mariofox-walk-loop-harness.stdout.log'
$stderr = Join-Path $logDir 'melonds.battle-mariofox-walk-loop-harness.stderr.log'
$configState = $null
$emulator = $null
$scriptName = '_battle_mariofox_walk_loop_harness.gdb'
...
& make -C $root TARGET=smash64ds-battle-mariofox-walk-loop BUILD=build-battle-mariofox-walk-loop-harness NDS_DEV_SCENE_HARNESS=battle_mariofox_walk_loop -j16
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
if (-not (Test-Path $rom) -or -not (Test-Path $elf)) {
    throw 'Battle Mario/Fox Walk loop harness build did not produce the expected ROM and ELF.'
}
...
try {
    ...
    Assert-Condition ($harn.Success -and (Convert-MarkerUInt32 $harn.Groups[1].Value) -eq 0x4841524e -and [int]$harn.Groups[2].Value -eq 37 -and [int]$harn.Groups[3].Value -eq 22 -and [int]$harn.Groups[4].Value -eq 21 -and (Convert-MarkerUInt32 $harn.Groups[5].Value) -eq 0) 'Battle Mario/Fox Walk loop harness did not select direct VSBattle from Maps.' $gdbStdout
    ...
    Write-Output ("Battle Mario/Fox Walk loop harness passed: scene=22/21 frames=4/4 root-dx={0}/{1} release=Wait vel={2}/{3}->{4}/{5} safe=1" -f $move.Groups[11].Value, $move.Groups[12].Value, $vel.Groups[3].Value, $vel.Groups[4].Value, $vel.Groups[5].Value, $vel.Groups[6].Value)
} finally {
    if ($null -ne $emulator) {
        $emulator.Refresh()
