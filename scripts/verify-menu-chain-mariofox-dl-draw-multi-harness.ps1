...
$verifierContext = Initialize-MelonDSVerifierContext `
    ...
    -NoBuild:$NoBuild
$rom = Join-Path $root 'smash64ds-menu-chain-mariofox-dl-draw-multi.nds'
$elf = Join-Path $root 'smash64ds-menu-chain-mariofox-dl-draw-multi.elf'
$melonDsPath = $verifierContext.MelonDSPath
$melonDsDir = Split-Path -Parent $melonDsPath
$logDir = Get-MelonDSVerifierLogDir -Root $root -RunnerSlot (Get-MelonDSActiveRunnerSlot)
$stdout = Join-Path $logDir 'melonds.menu-chain-mariofox-dl-draw-multi-harness.stdout.log'
$stderr = Join-Path $logDir 'melonds.menu-chain-mariofox-dl-draw-multi-harness.stderr.log'
$configState = $null
$emulator = $null
$scriptName = '_menu_chain_mariofox_dl_draw_multi_harness.gdb'
...
& make -C $root TARGET=smash64ds-menu-chain-mariofox-dl-draw-multi BUILD=build-menu-chain-mariofox-dl-draw-multi-harness NDS_DEV_SCENE_HARNESS=menu_chain_mariofox_dl_draw_multi -j16
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
if (-not (Test-Path $rom) -or -not (Test-Path $elf)) {
    throw 'Menu-chain Mario/Fox multi-DL draw harness build did not produce the expected ROM and ELF.'
}
...
try {
    ...
    Assert-Condition ($harn.Success -and (Convert-MarkerUInt32 $harn.Groups[1].Value) -eq 0x4841524e -and [int]$harn.Groups[2].Value -eq 32 -and [int]$harn.Groups[3].Value -eq 9 -and [int]$harn.Groups[4].Value -eq 1 -and (Convert-MarkerUInt32 $harn.Groups[5].Value) -eq 0) 'Menu-chain Mario/Fox multi-DL draw harness did not start at VSMode from Title.' $gdbStdout
