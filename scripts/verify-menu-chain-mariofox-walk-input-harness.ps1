...
$verifierContext = Initialize-MelonDSVerifierContext `
    ...
    -NoBuild:$NoBuild
$rom = Join-Path $root 'smash64ds-menu-chain-mariofox-walk-input.nds'
$elf = Join-Path $root 'smash64ds-menu-chain-mariofox-walk-input.elf'
$melonDsPath = $verifierContext.MelonDSPath
$melonDsDir = Split-Path -Parent $melonDsPath
$logDir = Get-MelonDSVerifierLogDir -Root $root -RunnerSlot (Get-MelonDSActiveRunnerSlot)
$stdout = Join-Path $logDir 'melonds.menu-chain-mariofox-walk-input-harness.stdout.log'
$stderr = Join-Path $logDir 'melonds.menu-chain-mariofox-walk-input-harness.stderr.log'
$configState = $null
$emulator = $null
$scriptName = '_menu_chain_mariofox_walk_input_harness.gdb'
...
& make -C $root TARGET=smash64ds-menu-chain-mariofox-walk-input BUILD=build-menu-chain-mariofox-walk-input-harness NDS_DEV_SCENE_HARNESS=menu_chain_mariofox_walk_input -j16
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
if (-not (Test-Path $rom) -or -not (Test-Path $elf)) {
    throw 'Menu-chain Mario/Fox Walk input harness build did not produce the expected ROM and ELF.'
}
...
try {
    ...
    Assert-Condition ($harn.Success -and (Convert-MarkerUInt32 $harn.Groups[1].Value) -eq 0x4841524e -and [int]$harn.Groups[2].Value -eq 36 -and [int]$harn.Groups[3].Value -eq 9 -and [int]$harn.Groups[4].Value -eq 1 -and (Convert-MarkerUInt32 $harn.Groups[5].Value) -eq 0) 'Menu-chain Mario/Fox Walk input harness did not select VS Mode from Title.' $gdbStdout
    ...
    Write-Output ("Menu-chain Mario/Fox Walk input harness passed: chain final=22/21 status=12/13 motion=6/7 stick=40/80 vel={0}/{1} callbacks=2 safe=1" -f $phys.Groups[3].Value, $phys.Groups[4].Value)
} finally {
    if ($null -ne $emulator) {
        $emulator.Refresh()
