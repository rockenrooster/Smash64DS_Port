...
try {
    ...
    if ($HardwareTriangles) {
        $summary += (" hwtex=bind{0}/upload{1}/ready{2}/reject{3}/fmt{4}/max{5}x{6}" -f $hwTexture.Groups[1].Value, $hwTexture.Groups[2].Value, $hwTexture.Groups[3].Value, $hwTexture.Groups[4].Value, $hwTexture.Groups[5].Value, $hwTexture.Groups[6].Value, $hwTexture.Groups[7].Value)
    }
