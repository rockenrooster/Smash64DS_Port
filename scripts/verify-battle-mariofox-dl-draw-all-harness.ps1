...
$verifierContext = Initialize-MelonDSVerifierContext `
    ...
    -NoBuild:$NoBuild
$rom = Join-Path $root 'smash64ds-battle-mariofox-dl-draw-all.nds'
$elf = Join-Path $root 'smash64ds-battle-mariofox-dl-draw-all.elf'
...
& make -C $root TARGET=smash64ds-battle-mariofox-dl-draw-all BUILD=build-battle-mariofox-dl-draw-all-harness NDS_DEV_SCENE_HARNESS=battle_mariofox_dl_draw_all -j16
