...
try {
    ...
    if ($RequireStageMPPassiveRecoverLoop) {
        ...
        Assert-Condition ($dashRunProcParams.Success -and (($dashRunProcParamsInts[0] -band 0x3ffff) -eq 0x3ffff) -and ($dashRunProcParamsInts[2] -eq ($dashRunProcParamsInts[1] + $dashRunProcParamsInts[3])) -and $dashRunProcParamsInts[3] -gt 0 -and $dashRunProcParamsInts[4] -gt 0 -and $dashRunProcParamsInts[5] -gt 0 -and $dashRunProcParamsInts[6] -eq 1 -and $dashRunProcParamsInts[7] -eq $dashRunProcParamsInts[8]) 'Dash-Run ftMainProcParams damage/hitlag scheduling proof did not pass in the current Passive recover boundary.' $gdbStdout
        ...
        Assert-Condition ($dashRunDamageColAnimUpdate.Success -and (($dashRunDamageColAnimUpdateInts[0] -band 0x1f) -eq 0x1f) -and $dashRunDamageColAnimUpdateInts[1] -eq 0x020e -and $dashRunDamageColAnimUpdateInts[2] -eq 2) 'Dash-Run ftCommonDamageUpdateDamageColAnim/SetDamageColAnim proof did not pass.' $gdbStdout
        ...
        Assert-Condition ($dashRunDamageCommonCallbacks.Success -and (($dashRunDamageCommonCallbacksInts[0] -band 0x3fff) -eq 0x3fff)) 'Dash-Run common damage callback proof did not pass.' $gdbStdout
        ...
        Assert-Condition ($dashRunDamageUpdateCatchResist.Success -and (($dashRunDamageUpdateCatchResistInts[0] -band 0x1f) -eq 0x1f)) 'Dash-Run ftCommonDamageUpdateCatchResist branch proof did not pass.' $gdbStdout
        ...
        Assert-Condition ($dashRunDamageKind.Success -and (($dashRunDamageKindInts[0] -band 0x7f) -eq 0x7f)) 'Dash-Run ftMainProcParams Twister/proc_trap branch plus imported ftCommonDamageInitDamageVars/GotoDamageStatus route and damage-kind preservation proof did not pass.' $gdbStdout
        $dashRunDamageSleepInts = Get-Ints $dashRunDamageSleep
        Assert-Condition ($dashRunDamageSleep.Success -and (($dashRunDamageSleepInts[0] -band 0x7f) -eq 0x7f) -and $dashRunDamageSleepInts[2] -eq 165 -and $dashRunDamageSleepInts[3] -eq 145 -and $dashRunDamageSleepInts[4] -gt 0) 'Dash-Run ftCommonDamageUpdateMain sleep-element status branch proof did not pass.' $gdbStdout
    ...
    if ($RequireStageMPCliffCatchFloor) {
        ...
        Assert-Condition ($stageMPCliffCatchFloorEffects.Success -and $cce[0] -eq 1 -and $cce[1] -eq 1) 'Stage MP cliff-catch effect/capture markers failed.' $gdbStdout
    ...
    if ($RequireStageMPCliffWaitFloor) {
        ...
        Assert-Condition ($stageMPCliffWaitFloorCalls.Success -and $cwc[0] -eq 1 -and $cwc[1] -eq 1 -and $cwc[2] -eq 1 -and $cwc[3] -eq 1 -and $cwc[4] -eq 1 -and $cwc[5] -eq 1 -and $cwc[6] -eq 1 -and $cwc[7] -eq 1 -and $cwc[8] -eq 1 -and $cwc[9] -eq 1 -and $cwc[10] -eq 1 -and $cwc[11] -eq 1 -and $cwc[12] -eq 1 -and $cwc[13] -eq 0) 'Stage MP cliff-wait source-order call counts failed.' $gdbStdout
    ...
    if ($RequireStageMPCliffWaitDamage) {
        ...
        Assert-Condition ($stageMPCliffWaitDamageCliffCatch.Success -and $cwdcc[0] -eq 1 -and $cwdcc[1] -eq 1 -and $cwdcc[2] -eq 1 -and $cwdcc[3] -eq 1 -and $cwdcc[4] -eq 1 -and $cwdcc[5] -eq 1 -and $cwdcc[6] -eq 1 -and $cwdcc[7] -eq 1 -and $cwdcc[8] -eq 84 -and $cwdcc[9] -eq 72 -and $cwdcc[10] -eq 1 -and $cwdcc[11] -eq 1 -and $cwdcc[12] -eq 1 -and $cwdcc[13] -eq 1 -and $cwdcc[14] -eq $cwdf[3] -and $cwdcc[15] -eq -1 -and (($cwdcc[16] -band 0x2000) -ne 0) -and (($cwdcc[17] -band 0x2000) -ne 0) -and $cwdcc[18] -eq 0x4) 'Stage MP cliff-wait DamageFall CliffCatch markers failed.' $gdbStdout
        ...
        Assert-Condition ($stageMPCliffWaitDamageDownWait.Success -and $cwdw[0] -eq 1 -and $cwdw[1] -eq 1 -and $cwdw[2] -eq 70 -and $cwdw[3] -eq -2 -and $cwdw[4] -eq 0 -and $cwdw[5] -eq 180 -and $cwdw[6] -eq 0x33 -and $cwdw[7] -eq 500 -and $cwdw[8] -eq 1 -and $cwdw[9] -eq 2 -and $cwdw[10] -eq 180 -and $cwdw[11] -eq 179 -and $cwdw[12] -eq 0 -and $cwdw[13] -eq 70 -and $cwdw[14] -eq -2 -and $cwdw[15] -eq 0) 'Stage MP cliff-wait DownWait setup/update markers failed.' $gdbStdout
    ...
    if ($RequireStageMPPassiveLoop) {
        ...
        if ($RequireStageMPPassiveRecoverLoop) {
            ...
            Assert-Condition ($stageMPPassiveLoopCatchPull.Success -and (($pcp[0] -band 0xff) -eq 0xff) -and $pcp[1] -eq 1 -and $pcp[2] -eq 1 -and $pcp[3] -eq 1 -and $pcp[4] -eq 1 -and $pcp[5] -eq 1 -and $pcp[6] -eq 1 -and $pcp[7] -eq 2 -and $pcp[8] -eq 1 -and $pcp[9] -eq 1 -and $pcp[10] -eq 167 -and $pcp[11] -eq 147 -and $pcp[12] -eq 0 -and $pcp[13] -eq 168 -and $pcp[14] -eq 4294967294 -and $pcp[15] -eq 0 -and $pcp[16] -eq 60 -and $pcp[17] -eq 59 -and $pcp[18] -eq 1 -and $pcp[19] -eq 1 -and $pcp[20] -eq 1 -and $pcp[21] -eq 0) 'Stage MP Passive CatchPull/CatchWait proof did not pass.' $gdbStdout
            $pcap = Get-Ints $stageMPPassiveLoopCapture
            Assert-Condition ($stageMPPassiveLoopCapture.Success -and (($pcap[0] -band 0x3f) -eq 0x3f) -and $pcap[1] -eq 1 -and $pcap[2] -eq 1 -and $pcap[3] -eq 1 -and $pcap[4] -eq 1 -and $pcap[5] -eq 1 -and $pcap[6] -eq 1 -and $pcap[7] -eq 1 -and $pcap[8] -eq 1 -and $pcap[9] -eq 2 -and $pcap[10] -eq 171 -and $pcap[11] -eq 150 -and $pcap[12] -eq 0 -and $pcap[13] -eq 172 -and $pcap[14] -eq 4294967294 -and $pcap[15] -eq 0 -and $pcap[16] -eq 1 -and ([Math]::Abs($pcap[17]) -eq 1) -and $pcap[18] -eq 0 -and $pcap[21] -eq 0) 'Stage MP Passive CapturePulled/CaptureWait proof did not pass.' $gdbStdout
            $pthr = Get-Ints $stageMPPassiveLoopThrow
            Assert-Condition ($stageMPPassiveLoopThrow.Success -and (($pthr[0] -band 0x3ff) -eq 0x3ff) -and $pthr[1] -eq 2 -and $pthr[2] -eq 2 -and $pthr[3] -eq 2 -and $pthr[4] -eq 4 -and $pthr[5] -eq 4 -and $pthr[6] -eq 169 -and $pthr[7] -eq 148 -and $pthr[8] -eq 0 -and $pthr[9] -eq 1 -and $pthr[10] -eq 186 -and $pthr[11] -eq 161 -and $pthr[12] -eq 1 -and $pthr[13] -eq 1 -and $pthr[14] -eq 1 -and $pthr[15] -eq 0) 'Stage MP Passive ThrowF/ThrowB/ThrownCommon proof did not pass.' $gdbStdout
            ...
            Assert-Condition ($stageMPPassiveLoopThrowCallback.Success -and (($pthrc[0] -band 0x1ff) -eq 0x1ff) -and $pthrc[1] -eq 1 -and $pthrc[2] -eq 1 -and $pthrc[3] -eq 1 -and $pthrc[4] -eq 186 -and $pthrc[5] -eq 161 -and $pthrc[6] -eq 1 -and $pthrc[7] -ge 0 -and $pthrc[8] -eq 1 -and $pthrc[9] -eq 1 -and $pthrc[10] -eq 1 -and $pthrc[11] -eq 1 -and $pthrc[12] -eq 1 -and $pthrc[13] -eq 186 -and $pthrc[14] -eq 161 -and $pthrc[15] -eq 1 -and $pthrc[16] -eq 1 -and $pthrc[17] -eq 1) 'Stage MP Passive ThrownCommon callback proof did not pass.' $gdbStdout
    ...
    if ($RequireStageMPDownWaitLoop) {
        ...
        Assert-Condition ($stageMPDownWaitLoopSetup.Success -and $dws[0] -eq 1 -and $dws[1] -eq 1 -and $dws[2] -eq 1 -and $dws[3] -eq 1 -and $dws[4] -eq 1 -and $dws[5] -eq 70 -and $dws[6] -eq -2 -and $dws[7] -eq 0 -and $dws[8] -eq 180 -and $dws[9] -eq 0x33 -and $dws[10] -eq 500 -and $dws[11] -eq 1) 'Stage MP DownWait setup markers failed.' $gdbStdout
    ...
    if ($RequireStageMPDownRecoverLoop) {
        ...
        Assert-Condition ($stageMPDownRecoverLoopSetup.Success -and $drs[0] -eq 1 -and $drs[1] -eq 1 -and $drs[2] -eq 1 -and $drs[3] -eq 1 -and $drs[4] -eq 1 -and $drs[5] -eq 69 -and $drs[6] -eq -2 -and $drs[7] -eq 0 -and $drs[8] -eq 180 -and $drs[9] -eq 1 -and $drs[10] -eq 0) 'Stage MP down-recover DownWaitD setup markers failed.' $gdbStdout
