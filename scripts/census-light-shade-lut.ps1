...
param(
    ...
    [int]$GdbPort = 4615,
    [int]$RunnerSlot = -1,
    [string]$Build = 'build-task90-shade-census',
    [switch]$NoBuild,
...
try {
    if (-not $NoBuild) {
        ...
        make -C $root "TARGET=$target" "BUILD=$Build" NDS_TASK90_SHADE_CENSUS=1 -j16
    ...
    $gdbLines = @(
        ...
        'printf "LUTDIFF="',
        'output gNdsTask90LutTraceDiffuse',
        'printf "\nLUTAMB="',
        'output gNdsTask90LutTraceAmbient',
        'printf "\n"',
        'detach')
    ...
    Write-Host ""
    Write-Host ("Task 90 -- light-shade LUT cache over $frames presented frames")
    ...
    Write-Host ""
    if ($delta['gNdsTask90LutGetCalls'] -gt 0) {
        Write-Host ("LUT rebuilds:         {0:N0} of {1:N0} get calls = {2:P1}" -f `
            $delta['gNdsTask90LutBuilds'], $delta['gNdsTask90LutGetCalls'],
            ($delta['gNdsTask90LutBuilds'] / $delta['gNdsTask90LutGetCalls']))
    }
    Write-Host ""
    ...
    if ($nextLine -and $diffLine -and $ambLine) {
        ...
        # gNdsTask90LutTraceNext points at the slot the next request will use,
    ...
    if ($JsonOut) {
        $payload = [ordered]@{
            task = 'Task 90 - light-shade LUT cache working set'
            target = $target
            rom = $rom
