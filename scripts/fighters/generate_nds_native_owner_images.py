...
def _kirby_hat_members(
        context: dict[str, object],
        ) -> list[tuple[str, str, list[str], str]]:
    """Image members for one deferred Kirby hat at one source detail."""
    root = context["roots"][0]
    light_index = context["light_preamble_indices"][0]
    preamble_words = [
        f"0x{word:08x}u"
        for pair in context["light_preambles"]
        for word in pair
    ]
    root_members = [
        ("u32", "copy_modelpart_id",
         [f"{int(context['copy_modelpart_id'])}u"], ""),
        ("u32", "root_offset", [f"0x{root[0]:08x}u"], ""),
        ("u16", "root_first_epoch", ["0u"], ""),
        ("u16", "root_tail_state_first", [f"{root[2]}u"], ""),
        ("u16", "root_source_command_count", [f"{root[3]}u"], ""),
        ("u8", "root_epoch_count", [f"{root[4]}u"], ""),
        ("u8", "root_tail_state_count", [f"{root[5]}u"], ""),
        ("u8", "root_tail_sync_count", [f"{root[6]}u"], ""),
        ("u8", "root_light_preamble", [f"{light_index}u"], ""),
        ("u32", "light_preamble_words", preamble_words, ""),
    ]
    return root_members + _member_values(context)


def _kirby_hat_type(modelpart_id: int, detail: str) -> str:
...
def render_header(
    ...
    if len(hat_bindings) != 1:
        raise ValueError(
            f"Kirby deferred hats disagree on logical binding: {hat_bindings}"
        )
    kirby_hat_binding = next(iter(hat_bindings))
    lines = [
    ...
    lines += [
        ...
        f"#define NDS_NATIVE_KIRBY_HAT_MIN_MODELPART {min(owners.KIRBY_COPY_HAT_MODEL_PART_IDS)}u",
        f"#define NDS_NATIVE_KIRBY_HAT_MAX_MODELPART {max(owners.KIRBY_COPY_HAT_MODEL_PART_IDS)}u",
        f"#define NDS_NATIVE_KIRBY_HAT_COUNT {len(owners.KIRBY_COPY_HAT_MODEL_PART_IDS)}u",
        f"#define NDS_NATIVE_KIRBY_HAT_BINDING {kirby_hat_binding}u",
        "",
    ]
    for modelpart_id in owners.KIRBY_COPY_HAT_MODEL_PART_IDS:
        for detail in DETAILS:
            context = hat_contexts[(modelpart_id, detail)]
            members = _kirby_hat_members(context)
            type_name = _kirby_hat_type(modelpart_id, detail)
            macro_prefix = (
                f"NDS_NATIVE_KIRBY_HAT_{modelpart_id}_{detail.upper()}"
            )
            lines += [
                f"/* Kirby copy modelpart {modelpart_id}, {detail} detail. */",
                f"typedef struct {type_name}",
                "{",
                "    u32 abi_tag[1];",
            ]
            ...
            for _ctype, name, values, guard in members:
                macro = f"{macro_prefix}_{name.upper()}_COUNT"
                if guard:
                    lines.append(f"#if {guard}")
                lines.append(f"#define {macro} {len(values)}u")
                if guard:
                    lines.append("#endif")
            lines += [
                f"#define {macro_prefix}_BYTES ((u32)sizeof({type_name}))",
                "",
            ]
    lines += [
    ...
    for modelpart_id in owners.KIRBY_COPY_HAT_MODEL_PART_IDS:
        for detail_index, detail in enumerate(DETAILS):
            ...
            image_rows.append(
                f"    X({modelpart_id}u, {detail_index}u, {type_name}, "
                f"{macro_prefix}, \"nitro:/fighters/"
                f"kirby_hat_{modelpart_id:02d}_{detail}.bin\")"
            )
    for index, row in enumerate(image_rows):
        lines.append(row + (" \\" if index + 1 < len(image_rows) else ""))
    lines.append("")
...
def render_kirby_hat_image(
        modelpart_id: int, detail: str, context: dict[str, object],
        ) -> str:
    """Standalone image for one copy_modelpart_id and source detail."""
    type_name = _kirby_hat_type(modelpart_id, detail)
    members = _kirby_hat_members(context)
    lines = [
        "/* Generated by scripts/fighters/generate_nds_native_owner_images.py.",
        "  Do not edit. */",
        f"/* Kirby deferred copy-hat modelpart {modelpart_id}; {detail} detail. */",
        "",
        "#include <PR/ultratypes.h>",
        "#include <nds/generated/nds_native_fighter_image.generated.h>",
        "",
        f"const {type_name} gNdsNativeKirbyHat{modelpart_id}{detail.title()}Image",
        " __attribute__((section(\".fighter_image\"), used)) =",
        "{",
        f"    .abi_tag = {{ 0x{NDS_NATIVE_OWNER_IMAGE_ABI_TAG:08x}u }},",
    ]
...
def main() -> int:
    ...
    for modelpart_id in owners.KIRBY_COPY_HAT_MODEL_PART_IDS:
        for detail in DETAILS:
            path = (repo_root / "src" / "nds" / "generated"
                    / (f"nds_native_fighter_kirby_hat_{modelpart_id:02d}_"
                       f"{detail}.image.c"))
            products[path] = render_kirby_hat_image(
                modelpart_id, detail, hat_contexts[(modelpart_id, detail)])
    ...
    if stale:
        raise SystemExit("stale generated native-owner images:\n  " +
                         "\n  ".join(stale))
    if args.check:
        print("native-owner image check: generated files are current")
    if not args.check:
        for (owner_name, detail), context in sorted(contexts.items()):
            total = 0
