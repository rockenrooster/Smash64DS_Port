...
$verifierContext = Initialize-MelonDSVerifierContext `
    ...
    -NoBuild:$NoBuild
$rom = Join-Path $root 'smash64ds-menu-chain-mariofox-dl-scan.nds'
$elf = Join-Path $root 'smash64ds-menu-chain-mariofox-dl-scan.elf'
$melonDsPath = $verifierContext.MelonDSPath
$melonDsDir = Split-Path -Parent $melonDsPath
$logDir = Get-MelonDSVerifierLogDir -Root $root -RunnerSlot (Get-MelonDSActiveRunnerSlot)
$stdout = Join-Path $logDir 'melonds.menu-chain-mariofox-dl-scan-harness.stdout.log'
$stderr = Join-Path $logDir 'melonds.menu-chain-mariofox-dl-scan-harness.stderr.log'
$configState = $null
$emulator = $null
$scriptName = '_menu_chain_mariofox_dl_scan_harness.gdb'
...
& make -C $root TARGET=smash64ds-menu-chain-mariofox-dl-scan BUILD=build-menu-chain-mariofox-dl-scan-harness NDS_DEV_SCENE_HARNESS=menu_chain_mariofox_dl_scan -j16
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
if (-not (Test-Path $rom) -or -not (Test-Path $elf)) {
    throw 'Menu-chain Mario/Fox DL scan harness build did not produce the expected ROM and ELF.'
}
...
try {
    ...
    Assert-Condition ($harn.Success -and (Convert-MarkerUInt32 $harn.Groups[1].Value) -eq 0x4841524e -and [int]$harn.Groups[2].Value -eq 26 -and [int]$harn.Groups[3].Value -eq 9 -and [int]$harn.Groups[4].Value -eq 1 -and (Convert-MarkerUInt32 $harn.Groups[5].Value) -eq 0) 'Menu-chain Mario/Fox DL scan harness did not start at VSMode from Title.' $gdbStdout
    ...
    Assert-Condition ($display.Success -and (Convert-MarkerUInt32 $display.Groups[1].Value) -eq 0x46544450 -and (Convert-MarkerUInt32 $display.Groups[2].Value) -eq 0x46544453 -and ((Convert-MarkerUInt32 $display.Groups[3].Value) -band 0x7ff) -eq 0x7ff -and (Convert-MarkerUInt32 $display.Groups[4].Value) -eq 0x3f -and [int]$display.Groups[5].Value -eq 2) 'Display metadata proof did not pass before menu-chain DL scan.' $gdbStdout
    Assert-Condition ($counts.Success -and [int]$counts.Groups[1].Value -eq 25 -and [int]$counts.Groups[2].Value -eq 27 -and [int]$counts.Groups[3].Value -eq 0 -and [int]$counts.Groups[4].Value -eq 0 -and [int]$counts.Groups[5].Value -eq 0 -and [int]$counts.Groups[6].Value -eq 0 -and [int]$counts.Groups[7].Value -eq 14 -and [int]$counts.Groups[8].Value -eq 18) 'Display metadata counts changed before menu-chain DL scan.' $gdbStdout
    Assert-Condition ($scan.Success -and (Convert-MarkerUInt32 $scan.Groups[1].Value) -eq 0x46544c50 -and (Convert-MarkerUInt32 $scan.Groups[2].Value) -eq 0x46544c53 -and ((Convert-MarkerUInt32 $scan.Groups[3].Value) -band 0x7ff) -eq 0x7ff -and (Convert-MarkerUInt32 $scan.Groups[4].Value) -eq 0xff -and [int]$scan.Groups[5].Value -eq 2) 'Menu-chain Mario/Fox DL scan proof did not pass.' $gdbStdout
    Assert-Condition ($ptrs.Success -and (Convert-MarkerUInt32 $ptrs.Groups[1].Value) -ne 0 -and (Convert-MarkerUInt32 $ptrs.Groups[2].Value) -ne 0 -and [uint32]$ptrs.Groups[3].Value -ne [uint32]::MaxValue -and [uint32]$ptrs.Groups[4].Value -ne [uint32]::MaxValue -and [uint32]$ptrs.Groups[5].Value -ne [uint32]::MaxValue -and [uint32]$ptrs.Groups[6].Value -ne [uint32]::MaxValue) 'Menu-chain DL scan did not find owned display lists.' $gdbStdout
    Assert-Condition ($stats.Success -and [int]$stats.Groups[1].Value -eq 0 -and [int]$stats.Groups[2].Value -eq 0 -and [int]$stats.Groups[3].Value -ge 59 -and [int]$stats.Groups[4].Value -ge 69 -and (Convert-MarkerUInt32 $stats.Groups[5].Value) -ne 0 -and (Convert-MarkerUInt32 $stats.Groups[6].Value) -ne 0 -and (Convert-MarkerUInt32 $stats.Groups[7].Value) -eq 0 -and (Convert-MarkerUInt32 $stats.Groups[8].Value) -eq 0) 'Menu-chain DL scan parser did not finish cleanly.' $gdbStdout
    Assert-Condition ($shape.Success -and [int]$shape.Groups[1].Value -gt 0 -and [int]$shape.Groups[2].Value -gt 0 -and [int]$shape.Groups[3].Value -gt 0 -and [int]$shape.Groups[4].Value -gt 0 -and [int]$shape.Groups[5].Value -gt 0 -and [int]$shape.Groups[6].Value -gt 0) 'Menu-chain DL scan did not record bounded geometry shape.' $gdbStdout
    Assert-Condition ($families.Success -and [int]$families.Groups[1].Value -eq 0 -and [int]$families.Groups[2].Value -eq 0 -and [int]$families.Groups[3].Value -gt 0 -and [int]$families.Groups[4].Value -gt 0 -and [int]$families.Groups[5].Value -gt 0 -and [int]$families.Groups[6].Value -gt 0 -and [int]$families.Groups[11].Value -gt 0 -and [int]$families.Groups[12].Value -gt 0 -and [int]$families.Groups[13].Value -gt 0 -and [int]$families.Groups[14].Value -gt 0) 'Menu-chain DL scan command-family counters were not populated cleanly.' $gdbStdout
    Assert-Condition ($state.Success -and [int]$state.Groups[1].Value -eq 10 -and [int]$state.Groups[2].Value -eq 10 -and [int]$state.Groups[3].Value -eq 4 -and [int]$state.Groups[4].Value -eq 4 -and [int]$state.Groups[5].Value -eq 0 -and [int]$state.Groups[6].Value -eq 0 -and (Convert-MarkerUInt32 $state.Groups[7].Value) -eq (Convert-MarkerUInt32 $state.Groups[8].Value) -and (Convert-MarkerUInt32 $state.Groups[9].Value) -eq (Convert-MarkerUInt32 $state.Groups[10].Value)) 'Menu-chain DL scan changed fighter state or root position.' $gdbStdout
    Assert-Condition ($safe.Success -and [int]$safe.Groups[1].Value -eq 0 -and [int]$safe.Groups[2].Value -eq 0 -and [int]$safe.Groups[3].Value -eq 0 -and [int]$safe.Groups[4].Value -eq 0) 'Menu-chain DL scan escaped into object creation, draw, matrix, or gameplay behavior.' $gdbStdout
    Assert-Condition ($boundary.Success -and (Convert-MarkerUInt32 $boundary.Groups[1].Value) -eq 0x53434e45 -and [int]$boundary.Groups[2].Value -eq 22) 'VSBattle did not park at the bounded scene boundary after menu-chain DL scan.' $gdbStdout
    Write-Output ("Menu-chain Mario/Fox DL scan harness passed: chain final=22/21 dl={0}/{1} asset={2}/{3} commands={4}/{5} blocker={6}/{7} safe=1" -f $ptrs.Groups[1].Value, $ptrs.Groups[2].Value, $ptrs.Groups[3].Value, $ptrs.Groups[4].Value, $stats.Groups[3].Value, $stats.Groups[4].Value, $stats.Groups[1].Value, $stats.Groups[2].Value)
} finally {
    if ($null -ne $emulator) {
        $emulator.Refresh()
