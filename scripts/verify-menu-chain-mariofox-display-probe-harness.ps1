...
$verifierContext = Initialize-MelonDSVerifierContext `
    ...
    -NoBuild:$NoBuild
$rom = Join-Path $root 'smash64ds-menu-chain-mariofox-display-probe.nds'
$elf = Join-Path $root 'smash64ds-menu-chain-mariofox-display-probe.elf'
$melonDsPath = $verifierContext.MelonDSPath
$melonDsDir = Split-Path -Parent $melonDsPath
$logDir = Get-MelonDSVerifierLogDir -Root $root -RunnerSlot (Get-MelonDSActiveRunnerSlot)
$stdout = Join-Path $logDir 'melonds.menu-chain-mariofox-display-probe-harness.stdout.log'
$stderr = Join-Path $logDir 'melonds.menu-chain-mariofox-display-probe-harness.stderr.log'
$configState = $null
$emulator = $null
$scriptName = '_menu_chain_mariofox_display_probe_harness.gdb'
...
& make -C $root TARGET=smash64ds-menu-chain-mariofox-display-probe BUILD=build-menu-chain-mariofox-display-probe-harness NDS_DEV_SCENE_HARNESS=menu_chain_mariofox_display_probe -j16
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
if (-not (Test-Path $rom) -or -not (Test-Path $elf)) {
    throw 'Menu-chain Mario/Fox display probe harness build did not produce the expected ROM and ELF.'
}
...
try {
    ...
    $groundSafe = [regex]::Match($gdbStdout, 'FTR_GROUND_SAFE=([0-9]+),([0-9]+),([0-9]+),([0-9]+),([0-9]+),([0-9]+),([0-9]+)')
    ...
    $displayState = [regex]::Match($gdbStdout, 'FTR_DISPLAY_STATE=([0-9]+),([0-9]+),([0-9]+),([0-9]+),([0-9]+),([0-9]+)')
    $displayRoot = [regex]::Match($gdbStdout, 'FTR_DISPLAY_ROOT=(0x[0-9a-fA-F]+|0),(0x[0-9a-fA-F]+|0),(0x[0-9a-fA-F]+|0),(0x[0-9a-fA-F]+|0)')
    $displaySafe = [regex]::Match($gdbStdout, 'FTR_DISPLAY_SAFE=([0-9]+),([0-9]+),([0-9]+),([0-9]+)')
    $boundary = [regex]::Match($gdbStdout, 'BOUNDARY=(0x[0-9a-fA-F]+|0),([0-9]+)')
    Assert-Condition ($harn.Success -and (Convert-MarkerUInt32 $harn.Groups[1].Value) -eq 0x4841524e -and [int]$harn.Groups[2].Value -eq 24 -and [int]$harn.Groups[3].Value -eq 9 -and [int]$harn.Groups[4].Value -eq 1 -and (Convert-MarkerUInt32 $harn.Groups[5].Value) -eq 0) 'Menu-chain Mario/Fox display probe harness did not start at VSMode from Title.' $gdbStdout
    ...
    Assert-Condition ($groundSafe.Success -and [int]$groundSafe.Groups[1].Value -eq 0 -and [int]$groundSafe.Groups[2].Value -eq 0 -and [int]$groundSafe.Groups[3].Value -eq 0 -and [int]$groundSafe.Groups[4].Value -eq 0 -and [int]$groundSafe.Groups[5].Value -eq 0 -and [int]$groundSafe.Groups[6].Value -eq 0 -and [int]$groundSafe.Groups[7].Value -eq 0) 'Ground proof escaped the bounded safety contract after menu chain.' $gdbStdout
    Assert-Condition ($display.Success -and (Convert-MarkerUInt32 $display.Groups[1].Value) -eq 0x46544450 -and (Convert-MarkerUInt32 $display.Groups[2].Value) -eq 0x46544453 -and ((Convert-MarkerUInt32 $display.Groups[3].Value) -band 0x7ff) -eq 0x7ff -and (Convert-MarkerUInt32 $display.Groups[4].Value) -eq 0x3f -and [int]$display.Groups[5].Value -eq 2) 'Mario/Fox display metadata probe did not pass after menu chain.' $gdbStdout
    Assert-Condition ($counts.Success -and [int]$counts.Groups[1].Value -eq 25 -and [int]$counts.Groups[2].Value -eq 27 -and [int]$counts.Groups[3].Value -eq 0 -and [int]$counts.Groups[4].Value -eq 0 -and [int]$counts.Groups[5].Value -eq 0 -and [int]$counts.Groups[6].Value -eq 0 -and [int]$counts.Groups[7].Value -eq 14 -and [int]$counts.Groups[8].Value -eq 18) 'Display metadata counts differ from the direct Mario/Fox proof.' $gdbStdout
    Assert-Condition ($displayState.Success -and [int]$displayState.Groups[1].Value -eq 10 -and [int]$displayState.Groups[2].Value -eq 10 -and [int]$displayState.Groups[3].Value -eq 4 -and [int]$displayState.Groups[4].Value -eq 4 -and [int]$displayState.Groups[5].Value -eq 0 -and [int]$displayState.Groups[6].Value -eq 0) 'Display probe changed fighter status, motion, or GA state after menu chain.' $gdbStdout
    Assert-Condition ($displayRoot.Success -and (Convert-MarkerUInt32 $displayRoot.Groups[1].Value) -eq (Convert-MarkerUInt32 $displayRoot.Groups[2].Value) -and (Convert-MarkerUInt32 $displayRoot.Groups[3].Value) -eq (Convert-MarkerUInt32 $displayRoot.Groups[4].Value)) 'Display probe moved fighter root DObjs after menu chain.' $gdbStdout
    Assert-Condition ($displaySafe.Success -and [int]$displaySafe.Groups[1].Value -eq 0 -and [int]$displaySafe.Groups[2].Value -eq 0 -and [int]$displaySafe.Groups[3].Value -eq 0 -and [int]$displaySafe.Groups[4].Value -eq 0) 'Display probe escaped into draw, matrix, gameplay, or object creation behavior after menu chain.' $gdbStdout
    ...
    Write-Output ("Menu-chain Mario/Fox display probe harness passed: chain final=22/21 display={0} dobj={1}/{2} ready={3}/{4} stable=1" -f $display.Groups[3].Value, $counts.Groups[1].Value, $counts.Groups[2].Value, $counts.Groups[7].Value, $counts.Groups[8].Value)
} finally {
    if ($null -ne $emulator) {
        $emulator.Refresh()
