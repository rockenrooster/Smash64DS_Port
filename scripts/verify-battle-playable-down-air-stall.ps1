...
try {
    ...
    $commands = @(
        ...
        'set $frame_start = gNdsFrameCounter',
        'set $reads_start = gNdsControllerPlaybackReadCount',
        'set $status_tics_start = $actor_fp->status_total_tics',
        'set $last_logic = $logic_start',
        'set $last_cpu = $cpu_start',
        'set $last_frame = $frame_start',
        'set $last_anim_bits = $entry_anim_bits',
        ('set {{unsigned int}}0x{0:x8} = 0' -f $actorPads),
        'else',
        'set $samples = $samples + 1',
        'if (gNdsBattlePlayablePacingLogicFrames > $last_logic) && (gNdsFTComputerProcessCount > $last_cpu) && (gNdsFrameCounter > $last_frame)',
        'set $heartbeat_samples = $heartbeat_samples + 1',
        'end',
        'set $current_anim_bits = *(unsigned int *)&$actor_gobj->anim_frame',
        'if $current_anim_bits != $last_anim_bits',
        'set $anim_changes = $anim_changes + 1',
        'end',
        'set $last_logic = gNdsBattlePlayablePacingLogicFrames',
        'set $last_cpu = gNdsFTComputerProcessCount',
        'set $last_frame = gNdsFrameCounter',
        'set $last_anim_bits = $current_anim_bits',
        'if gNdsRendererProfileUpdateTicks > $max_update',
        'set $max_update = gNdsRendererProfileUpdateTicks',
        'end',
        'if $samples >= 8',
        'set $outcome = 1',
        'disable $input_breakpoint',
        ...
        'continue',
        ('set {{unsigned int}}0x{0:x8} = 0' -f $actorPads),
        'printf "DOWN_AIR_RESULT=%u,%u,%u,%u,%u,%u,%u\n", $actor_kind, $outcome, $frame, $seen, $samples, $heartbeat_samples, $anim_changes',
        ...
        'printf "DOWN_AIR_PROGRESS=%u,%u,%u,%u,%d,%u\n", gNdsBattlePlayablePacingLogicFrames - $logic_start, gNdsFTComputerProcessCount - $cpu_start, gNdsFrameCounter - $frame_start, gNdsControllerPlaybackReadCount - $reads_start, $actor_fp->status_total_tics - $status_tics_start, $max_update',
    ...
    $result = [regex]::Match($gdbStdout,
        'DOWN_AIR_RESULT=([0-9]+),([0-9]+),([0-9]+),([0-9]+),([0-9]+),([0-9]+),([0-9]+)')
    ...
    $progress = [regex]::Match($gdbStdout,
        'DOWN_AIR_PROGRESS=([0-9]+),([0-9]+),([0-9]+),([0-9]+),(-?[0-9]+),([0-9]+)')
    ...
    if ($rv[1] -eq 2 -and $Actor -eq 'Fox') {
        throw "Temporary P2 Fox did not enter Down-Air through controller playback.`n$gdbStdout"
    }
    Assert-Condition ($rv[1] -eq 1 -and $rv[3] -eq 1 -and
        $rv[4] -eq 8 -and $rv[5] -ge 4 -and $rv[6] -gt 0) `
        "$Actor Down-Air did not retain eight updates across four presented frames." `
        $gdbStdout
    ...
    Assert-Condition ($progress.Success -and $pv[0] -ge 8 -and
        $pv[1] -ge $minimumCpuUpdates -and
        $pv[2] -ge 4 -and $pv[3] -gt 0 -and
        $pv[4] -gt 0 -and $pv[5] -gt 0) `
    ...
    Write-Output ((
        '{0} Down-Air passed: knee={1} updates={2} presentedHeartbeats={3}/8 animChanges={4} ' +
        'reloc={5}/{6} logic/cpu/frame={7}/{8}/{9} updateMax={10} ' +
        'reserve={11} screenshot={12}') -f
        $Actor, $kv[0], $rv[2], $rv[5], $rv[6], $ev[3], $ev[4],
        $pv[0], $pv[1], $pv[2], $pv[5], $mv[2],
        $screenshotPath)
} finally {
    if ($null -ne $emulator) {
        $emulator.Refresh()
