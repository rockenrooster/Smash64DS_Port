...
$verifierContext = Initialize-MelonDSVerifierContext `
    ...
    -NoBuild:$NoBuild
$rom = Join-Path $root 'smash64ds-menu-chain-mariofox-dl-execute.nds'
$elf = Join-Path $root 'smash64ds-menu-chain-mariofox-dl-execute.elf'
$melonDsPath = $verifierContext.MelonDSPath
$melonDsDir = Split-Path -Parent $melonDsPath
$logDir = Get-MelonDSVerifierLogDir -Root $root -RunnerSlot (Get-MelonDSActiveRunnerSlot)
$stdout = Join-Path $logDir 'melonds.menu-chain-mariofox-dl-execute-harness.stdout.log'
$stderr = Join-Path $logDir 'melonds.menu-chain-mariofox-dl-execute-harness.stderr.log'
$configState = $null
$emulator = $null
$scriptName = '_menu_chain_mariofox_dl_execute_harness.gdb'
...
& make -C $root TARGET=smash64ds-menu-chain-mariofox-dl-execute BUILD=build-menu-chain-mariofox-dl-execute-harness NDS_DEV_SCENE_HARNESS=menu_chain_mariofox_dl_execute -j16
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
if (-not (Test-Path $rom) -or -not (Test-Path $elf)) {
    throw 'Menu-chain Mario/Fox DL execute harness build did not produce the expected ROM and ELF.'
}
...
try {
    ...
    Assert-Condition ($harn.Success -and (Convert-MarkerUInt32 $harn.Groups[1].Value) -eq 0x4841524e -and [int]$harn.Groups[2].Value -eq 28 -and [int]$harn.Groups[3].Value -eq 9 -and [int]$harn.Groups[4].Value -eq 1 -and (Convert-MarkerUInt32 $harn.Groups[5].Value) -eq 0) 'Menu-chain Mario/Fox DL execute harness did not start at VSMode from Title.' $gdbStdout
    ...
    Assert-Condition ($scan.Success -and (Convert-MarkerUInt32 $scan.Groups[1].Value) -eq 0x46544c50 -and (Convert-MarkerUInt32 $scan.Groups[2].Value) -eq 0x46544c53 -and ((Convert-MarkerUInt32 $scan.Groups[3].Value) -band 0x7ff) -eq 0x7ff -and [int]$scan.Groups[5].Value -eq 2) 'Prerequisite DL scan proof did not pass before menu-chain execute.' $gdbStdout
    ...
    Assert-Condition ($exec.Success -and (Convert-MarkerUInt32 $exec.Groups[1].Value) -eq 0x46544c45 -and (Convert-MarkerUInt32 $exec.Groups[2].Value) -eq 0x46544c58 -and ((Convert-MarkerUInt32 $exec.Groups[3].Value) -band 0x7ff) -eq 0x7ff -and (Convert-MarkerUInt32 $exec.Groups[4].Value) -eq 0xff -and [int]$exec.Groups[5].Value -eq 2) 'Menu-chain Mario/Fox DL execute proof did not pass.' $gdbStdout
    Assert-Condition ($stats.Success -and [int]$stats.Groups[1].Value -eq 0 -and [int]$stats.Groups[2].Value -eq 0 -and [int]$stats.Groups[3].Value -ge 59 -and [int]$stats.Groups[4].Value -ge 69 -and (Convert-MarkerUInt32 $stats.Groups[7].Value) -eq 0 -and (Convert-MarkerUInt32 $stats.Groups[8].Value) -eq 0 -and [int]$stats.Groups[9].Value -eq 0 -and [int]$stats.Groups[10].Value -eq 0) 'Menu-chain DL execute stats were not clean.' $gdbStdout
    Assert-Condition ($geom.Success -and [int]$geom.Groups[1].Value -gt 0 -and [int]$geom.Groups[2].Value -gt 0 -and [int]$geom.Groups[3].Value -gt 0 -and [int]$geom.Groups[4].Value -gt 0 -and (Convert-MarkerUInt32 $geom.Groups[5].Value) -ne 0 -and (Convert-MarkerUInt32 $geom.Groups[6].Value) -ne 0 -and [int]$geom.Groups[7].Value -gt 0 -and [int]$geom.Groups[8].Value -gt 0 -and [int]$geom.Groups[9].Value -gt 0 -and [int]$geom.Groups[10].Value -gt 0) 'Menu-chain DL execute did not decode vertices and triangles.' $gdbStdout
    Assert-Condition ($bounds.Success -and ([int]$bounds.Groups[1].Value -ne [int]$bounds.Groups[2].Value -or [int]$bounds.Groups[3].Value -ne [int]$bounds.Groups[4].Value -or [int]$bounds.Groups[5].Value -ne [int]$bounds.Groups[6].Value) -and ([int]$bounds.Groups[7].Value -ne [int]$bounds.Groups[8].Value -or [int]$bounds.Groups[9].Value -ne [int]$bounds.Groups[10].Value -or [int]$bounds.Groups[11].Value -ne [int]$bounds.Groups[12].Value) -and (Convert-MarkerUInt32 $bounds.Groups[13].Value) -ne 0 -and (Convert-MarkerUInt32 $bounds.Groups[14].Value) -ne 0) 'Menu-chain DL execute bounds/checksum were not populated.' $gdbStdout
    Assert-Condition ($families.Success -and [int]$families.Groups[5].Value -gt 0 -and [int]$families.Groups[6].Value -gt 0 -and [int]$families.Groups[7].Value -gt 0 -and [int]$families.Groups[8].Value -gt 0 -and (Convert-MarkerUInt32 $families.Groups[11].Value) -ne 0 -and (Convert-MarkerUInt32 $families.Groups[12].Value) -ne 0) 'Menu-chain DL execute command-family counters were not populated.' $gdbStdout
    Assert-Condition ($safe.Success -and [int]$safe.Groups[1].Value -eq 0 -and [int]$safe.Groups[2].Value -eq 0 -and [int]$safe.Groups[3].Value -eq 0 -and [int]$safe.Groups[4].Value -eq 0 -and [int]$safe.Groups[5].Value -eq 0 -and [int]$safe.Groups[6].Value -eq 0) 'Menu-chain DL execute escaped bounded decode-only behavior.' $gdbStdout
    Assert-Condition ($boundary.Success -and (Convert-MarkerUInt32 $boundary.Groups[1].Value) -eq 0x53434e45 -and [int]$boundary.Groups[2].Value -eq 22) 'VSBattle did not park at the bounded scene boundary after menu-chain execute.' $gdbStdout
    Write-Output ("Menu-chain Mario/Fox DL execute harness passed: chain final=22/21 commands={0}/{1} verts={2}/{3} tris={4}/{5} safe=1" -f $stats.Groups[3].Value, $stats.Groups[4].Value, $geom.Groups[1].Value, $geom.Groups[2].Value, $geom.Groups[7].Value, $geom.Groups[8].Value)
} finally {
    if ($null -ne $emulator) {
        $emulator.Refresh()
