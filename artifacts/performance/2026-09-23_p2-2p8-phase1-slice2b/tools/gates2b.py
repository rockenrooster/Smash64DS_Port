"""Slice 2b gate tables: percentiles, bands, per-range means, VBlank shares.

Usage: gates2b.py CONTROL_ARM CANDIDATE_ARM [more arms]
Arms are artifact names (ARM-rows.csv + ARM.json in the slice 2b folder).
...
def load(arm):
    rows = list(csv.DictReader(open(os.path.join(ART, arm + '-rows.csv'))))
