"""Per-kind lean counters of slice 5 run JSONs (run-s5.ps1 output).

Usage: kinds5.py ARM [ARM...]   (ARM = run name in the slice 5 artifact dir,
s4:NAME for the slice 4 dir, or a path without .json)
...
def load(arm):
    ...
    if arm.startswith('s4:'):
        base = HERE.replace('slice5', 'slice4')
        arm = arm[3:]
