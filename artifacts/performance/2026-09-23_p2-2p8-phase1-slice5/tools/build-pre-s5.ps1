param(
    ...
    [string]$Build = 'build-p2p8-s5-pre',
    [string]$SeedFrom = '',
    [string[]]$MakeArgs = @()
)
# P2-2p8 Phase 1 slice 5 A/B baseline (from slice 4's build-base-s4.ps1):
# build the PRE-slice-5 sources of the files this slice edits (the start-of-
# work backups) on TODAY's tree, in their own build dir, under the shared build
# lock; then put the slice 5 sources back and prove them byte-identical. The
...
$bk = 'C:\Users\Tyler\AppData\Local\Temp\claude\D--Stuff-DevFolder-Smash64DS-Port\31e5c78d-38d0-4d75-b683-91723230d2cc\scratchpad\backup-slice5'
...
foreach ($k in $pre.Keys) {
    if (-not (Test-Path -LiteralPath $pre[$k])) { throw "missing pre-slice-5 source for $k" }
}
while (Test-Path -LiteralPath $lock) { Start-Sleep -Seconds 5 }
Set-Content -LiteralPath $lock -Value 'phase1-slice5-pre' -NoNewline
$hashes = @{}
try {
    ...
    Set-Content -LiteralPath $log -Value ("make " + ($all -join ' ') + "   (PRE-slice-5 lean sources)")
    ...
    Add-Content -LiteralPath $log -Value ("RESTORED slice 5 sources; mismatches: " + $bad.Count + " " + ($bad -join ','))
