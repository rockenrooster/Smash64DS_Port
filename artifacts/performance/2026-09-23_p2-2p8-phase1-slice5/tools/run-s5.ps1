...
# P2-2p8 Phase 1 slice 5 sampler wrapper (from slice 4's run-s4.ps1): runner
...
$leanScalars = @('draws', 'shadow_runs', 'attempts', 'entry_hits', 'entry_switches', 'materializations',
    ...
    'plan_reuses', 'plan_reuse_restarts', 'verify_plan_runs',
    'fighter_uploads', 'fighter_uploads_after_go', 'go_frame')
$lean = @()
foreach ($f in $leanScalars) { $lean += "gNdsFtrLean.$f" }
foreach ($i in 0..3) { $lean += "gNdsFtrLean.variant_reject[$i]"; $lean += "gNdsFtrLean.verify_mismatch[$i]";
