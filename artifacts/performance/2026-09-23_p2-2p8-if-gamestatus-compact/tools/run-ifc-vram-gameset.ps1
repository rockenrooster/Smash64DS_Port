param(
    ...
    # Poke the announcement's game_set argument to 1 so the TIME UP match bakes
    # and shows GAME SET instead (a register poke; the stress match never ends
    # on stocks). Dumps go to <Arm>-gameset.
    [switch]$ForceGameSet
)
...
$out = Join-Path $art ($(if ($ForceGameSet) { "$Arm-gameset" } else { $Arm }))
...
try {
    ...
    [System.IO.File]::WriteAllLines($script, @(
        ...
        'continue',
        $(if ($ForceGameSet) { 'set variable $r0 = 1' } else { 'echo' }),
