"""Slice 4 gate tables: percentiles, bands, per-range means, VBlank shares.

Usage: gates4.py CONTROL_ARM CANDIDATE_ARM [more arms]
Arms are artifact names (ARM-rows.csv + ARM.json in the slice 4 folder; prefix
s3: for the slice 3 folder, 2c: / 2b: for the slice 2c / 2b folders).
...
ART = r'D:\Stuff\DevFolder\Smash64DS_Port\artifacts\performance\2026-09-23_p2-2p8-phase1-slice4'
ART3 = ART.replace('slice4', 'slice3')
ART2C = ART.replace('slice4', 'slice2c')
ART2B = ART.replace('slice4', 'slice2b')
...
def load(arm):
    ...
    if arm.startswith('s3:'):
        base, arm = ART3, arm[3:]
    elif arm.startswith('2b:'):
        base, arm = ART2B, arm[3:]
    elif arm.startswith('2c:'):
        base, arm = ART2C, arm[3:]
