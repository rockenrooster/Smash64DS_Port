param(
    ...
    [int]$Route = 0,
    [int]$Census = 0,
    [string]$Build = 'build-p2p8-s4-base',
...
# P2-2p8 Phase 1 slice 4 BASELINE sampler wrapper (the slice 3 counter set, for the pre-slice-4 build): runner slot 9 / GDB 3423 only.
...
# slice 3 (lab): carved residency and the identity's collision checks
...
# slice 3 (lab): texture ids past a material's sprite list
# slice 3: the lean path counters (gNdsFtrLean), every scalar and array field
...
$leanScalars = @('draws', 'shadow_runs', 'attempts', 'adopts', 'kernel_joints', 'kernel_slow_joints', 'kernel_fail',
    'head_ticks', 'guard_ticks', 'kernel_ticks', 'patch_ticks', 'submit_ticks', 'dma_wait_ticks', 'dma_wait_spins',
    'oracle_runs', 'oracle_words', 'oracle_unconsumed', 'oracle_source_miss', 'oracle_fence_rekey',
    'adopt_tinted_sites', 'adopt_needs_fence', 'adopt_words', 'adopt_roots', 'adopt_textures',
    'tint_patch_binds', 'tint_patch_moved', 'tint_patch_miss', 'tint_patch_white', 'tint_patch_shape',
    ...
    'ident_watch_miss', 'adopt_watch', 'adopt_all_pinned', 'retuples', 'rekeys', 'adopt_ticks', 'adopt_copy_ticks', 'adopt_kept', 'projection_patches', 'variant_learns', 'variant_words', 'variant_switches', 'variant_learn_fail', 'variant_learn_ticks', 'retuple_ticks', 'kernel_rebuilds', 'kernel_part_joints')
...
foreach ($i in 0..9) { $extra += "gNdsFtrLean.adopt_refuse[$i]" }
foreach ($i in 0..7) { $extra += "gNdsFtrLean.oracle_mismatch[$i]" }
foreach ($i in 0..7) { $extra += "gNdsFtrLean.oracle_key_moved[$i]" }
foreach ($i in 0..1) { $extra += "gNdsFtrLean.oracle_record_under_hit[$i]" }
foreach ($i in 0..8) { $extra += "gNdsFtrLean.oracle_record_diff[$i]" }
foreach ($i in 0..2) { $extra += "gNdsFtrLean.oracle_max_lsb[$i]" }
...
foreach ($f in 'k_attempts', 'k_draws', 'k_adopts', 'k_head_ticks', 'k_guard_ticks', 'k_kernel_ticks', 'k_patch_ticks',
    'k_submit_ticks', 'k_dma_wait_ticks', 'k_kernel_joints', 'k_high_draws', 'k_oracle_runs') {
    $extra += @(0..3 | ForEach-Object { "gNdsFtrLean.$f[$_]" })
}
foreach ($k in 0..3) {
    foreach ($i in 0..19) { $extra += "gNdsFtrLean.k_decline[$k][$i]" }
    foreach ($i in 0..9) { $extra += "gNdsFtrLean.k_adopt_refuse[$k][$i]" }
...
foreach ($k in 0..3) { foreach ($i in 0..7) { $extra += "gNdsFtrLean.k_tint_witness[$k][$i]" } }
$extra += @(
