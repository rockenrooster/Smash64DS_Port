param(
    [string]$LogName = 'build-base.log',
    [string]$Target = 'smash64ds-p2-fourcpu-tickhud-hwtri',
    [string]$Build = 'build-p2p8-s4-base',
    # Seed a fresh build dir from this one (objects + generated outputs), so
    # only the swapped translation units recompile.
    [string]$SeedFrom = 'build-p2-fourcpu-tickhud',
    [string[]]$MakeArgs = @()
)
# P2-2p8 Phase 1 slice 4 A/B baseline: build the PRE-slice-4 lean sources (the
# slice's start-of-work backups of the five runtime files) on TODAY's tree, in
# its own build dir, under the shared build lock; then put the slice 4 sources
# back and prove them byte-identical. The lock is held for the whole swap, so
# no other build can see the swapped tree.
...
$bk = 'C:\Users\Tyler\AppData\Local\Temp\claude\D--Stuff-DevFolder-Smash64DS-Port\31e5c78d-38d0-4d75-b683-91723230d2cc\scratchpad\backup-slice4'
...
foreach ($k in $pre.Keys) {
    if (-not (Test-Path -LiteralPath $pre[$k])) { throw "missing pre-slice-4 source for $k" }
}
while (Test-Path -LiteralPath $lock) { Start-Sleep -Seconds 5 }
Set-Content -LiteralPath $lock -Value 'phase1-slice4-base' -NoNewline
$hashes = @{}
try {
    ...
    Set-Content -LiteralPath $log -Value ("make " + ($all -join ' ') + "   (PRE-slice-4 lean sources)")
    ...
    Add-Content -LiteralPath $log -Value ("RESTORED slice 4 sources; mismatches: " + $bad.Count + " " + ($bad -join ','))
