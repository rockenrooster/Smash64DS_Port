param(
    ...
    # P2-2p8 Phase 1 slice 7: the words DEFAULT to route 1 / admission 2 in the
    # ROM. Leave a word unset (-1) to measure the ROM as it ships; set it to
    # poke it at BOOT (sampler -BootSetGlobals, at `main`: before the battle is
    # built, so the creation-time admission sees it too). Route 0 + admit 0 is
    # the old path on the same ROM.
    [int]$Admit = -1,
    [int]$Route = -1,
    [int]$Slow = -1,
...
# P2-2p8 Phase 1 slice 7 sampler wrapper (from slice 6's run-s6.ps1): runner
# slot 9 / GDB 3423 only. Unlike slices 1-6, words are poked at boot (or not at
# all), never at the first frame-complete marker. Every counter name is checked
# against the ELF's symbols first, so a stale list fails before the run, not
# inside GDB.
$ErrorActionPreference = 'Continue'
$root = 'D:\Stuff\DevFolder\Smash64DS_Port'
$art = Join-Path $root 'artifacts\performance\2026-09-24_p2-2p8-phase1-slice7'
...
foreach ($n in 'gNdsFtrLeanAdmitFail', 'gNdsRendererNativeFailure.count', 'gNdsRendererNativeFailure.status',
    ...
    # slice 7: the admission as the ROM runs it by default (creation time)
    'gNdsFtrAdmitLab.runs', 'gNdsFtrAdmitLab.word', 'gNdsFtrAdmitLab.frame', 'gNdsFtrAdmitLab.regions',
    'gNdsFtrAdmitLab.bg3_not_empty', 'gNdsFtrAdmitLab.records', 'gNdsFtrAdmitLab.applied',
    'gNdsFtrAdmitLab.fails', 'gNdsFtrAdmitLab.entries_admitted', 'gNdsFtrAdmitLab.bytes_admitted',
    'gNdsFtrAdmitLab.locked', 'gNdsFtrAdmitLab.lock_frame', 'gNdsFtrAdmitLab.done',
    'gNdsFtrAdmitLab.pruned', 'gNdsFtrAdmitLab.absent', 'gNdsFtrAdmitLab.libc_skipped',
    'gNdsFtrAdmitLab.slot_skipped', 'gNdsFtrAdmitLab.outside_count',
    'gNdsFtrAdmitLab.outside[0]', 'gNdsFtrAdmitLab.outside[1]', 'gNdsFtrAdmitLab.outside[2]',
    'gNdsFtrAdmitLab.outside[3]', 'gNdsFtrAdmitLab.exit_safety', 'gNdsFtrAdmitLab.ticks',
    'gNdsFtrAdmitLab.read_ticks', 'gNdsFtrAdmitLab.apply_ticks', 'gNdsFighterPacketWordsMax') {
    $base = ($n -split '\.')[0]
...
$sets = @()
if ($Route -ge 0) { $sets += "gNdsFtrLeanRoute=$Route" }
if ($Admit -ge 0) { $sets += "gNdsFtrLeanAdmit=$Admit" }
if ($Slow -ge 0) { $sets += "gNdsFtrLeanSlow=$Slow" }
...
$argsList = @('-NoProfile', '-File', (Join-Path $root 'scripts\sample-tick-hud-buckets.ps1'), '-NoBuild',
    ...
    '-Samples', "$Samples", '-StartFrame', "$StartFrame", '-ExtraGlobals', ($kept -join ','),
    '-RowsCsv', $rows, '-JsonOut', $json, '-RunnerSlot', '9', '-GdbPort', '3423', '-TimeoutSeconds', '3600')
if ($sets.Count -gt 0) { $argsList += @('-BootSetGlobals', ($sets -join ',')) }
Set-Content -LiteralPath $log -Value ("boot pokes: " + $(if ($sets.Count -gt 0) { $sets -join ',' } else { 'none (ROM defaults)' }))
& pwsh @argsList *>> $log
$code = $LASTEXITCODE
