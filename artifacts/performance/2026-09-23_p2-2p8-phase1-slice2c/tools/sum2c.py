"""Print the slice 2b/2c gate / lab counters of one or more sampler runs."""
import json
import os
import sys
...
KEYS = [
    ...
    'gNdsVramBankDReturns', 'gNdsVramBankDMissedExits', 'gNdsFtrAdmitLab.exit_runs', 'gNdsFtrAdmitLab.exit_released',
    'gNdsFtrAdmitLab.exit_orphans', 'gNdsFtrAdmitLab.exit_safety',
    'gNdsFtrCarveLab.opens', 'gNdsFtrCarveLab.carrier_fail', 'gNdsFtrCarveLab.regions[0]', 'gNdsFtrCarveLab.regions[1]',
    'gNdsFtrCarveLab.reserved[0]', 'gNdsFtrCarveLab.reserved[1]', 'gNdsFtrCarveLab.used[0]', 'gNdsFtrCarveLab.used[1]',
    'gNdsFtrCarveLab.carved[0]', 'gNdsFtrCarveLab.carved[1]', 'gNdsFtrCarveLab.fails[0]', 'gNdsFtrCarveLab.fails[1]',
    'gNdsFtrCarveLab.lazy', 'gNdsFtrCarveLab.shrink_freed[0]', 'gNdsFtrCarveLab.shrink_freed[1]',
    'gNdsFtrCarveLab.retires', 'gNdsFtrCarveLab.retire_regions', 'gNdsFtrCarveLab.region_returns',
    'gNdsFtrCarveLab.exit_released', 'gNdsFtrCarveLab.libc_before', 'gNdsFtrCarveLab.libc_after',
    'gNdsFtrCarveLab.census_runs', 'gNdsFtrCarveLab.census_records', 'gNdsFtrCarveLab.census_keys',
    'gNdsFtrCarveLab.census_dups', 'gNdsFtrCarveLab.census_skipped', 'gNdsFtrCarveLab.census_noprobe',
    'gNdsFtrCarveLab.census_collisions', 'gNdsFtrCarveLab.census_overflow', 'gNdsFtrCarveLab.census_ticks',
    'gNdsTexIdentShadowOn', 'gNdsTexIdentLab.checks', 'gNdsTexIdentLab.collisions', 'gNdsTexIdentLab.shadow_overflow',
    'gNdsTexIdentLab.journal_count', 'gNdsTexIdentLab.journal_overflow', 'gNdsTexIdentLab.journal_collisions',
    'gNdsRendererTextureKeyPoolEntriesHighWater', 'gNdsFtrTidDrawCount', 'gNdsFtrTidSetCount', 'gNdsFtrTidInitCount',
]


def load(arm):
    if arm.startswith('2b:'):
        return load(os.path.join(ART2B, arm[3:] + '.json'))
...
def main():
    ...
    for k in KEYS:
        ...
        if all(v is None for v in vals):
            continue
        print('%-44s' % k.replace('gNdsFtrAdmitLab.', 'lab.').replace('gNdsVramCensus.', 'vc.').replace('gNdsFtrCarveLab.', 'carve.').replace('gNdsTexIdentLab.', 'ident.') +
    ...
    for a, d, ex in runs:
        ...
        cf = [ex.get('gNdsFtrCarveLab.census_first[%d]' % j) for j in range(4)]
        if cf[0] or cf[1]:
            print('   census first collision', cf)
        icf = [ex.get('gNdsTexIdentLab.collision_first[%d]' % j) for j in range(4)]
        if any(icf):
            print('   ident first collision', icf)
        for i in range(8):
            row = [ex.get('gNdsFtrTidDraw[%d][%d]' % (i, j)) for j in range(6)]
            if row[0]:
                print('   tid draw', [hex(x) if isinstance(x, int) else x for x in row])
        n = ex.get('gNdsNativeFailureLabCount')
        if n:
            print('   native failure classes (first 16 distinct identity/status/reason): %d%s' % (
                n, ' (table full)' if n >= 16 else ''))
            for i in range(min(n, 16)):
                r = [ex.get('gNdsNativeFailureLab[%d][%d]' % (i, j)) or 0 for j in range(5)]
                print('     identity %08x status %3d reason %d count %5d first frame %d' % tuple(r))
